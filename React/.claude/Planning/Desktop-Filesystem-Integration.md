...
**Phase 4 — Live watcher (robust).** Add `chokidar`. `main/watcher.ts`: watch the session root, ignore `.nexus` / `.trash` / dotfiles, `awaitWriteFinish` + ~200 ms debounce; on settle → re-read tree → `webContents.send('nexus:changed', tree)`. No pause flag (idempotent re-read). Start on nexus open, stop on switch/quit. Renderer replaces the tree (selection is id-based, preserved). ⌘R Reload is the manual fallback.
