...
- **Add the missing color tokens.** The token layer (`design-system/tokens/color.css.ts`) already has the structural ones the spec maps to — `color.label.primary` (= body text), `color.label.secondary` (= muted), `color.label.tertiary`, `color.fill.tertiary` (= blockquote card), `color.separator.line` (= HR). **Missing and must be added** as named **semantic color tokens — the labels side, NOT the tint system** (they're semantic text colors, the same category as `label.*`; tints are an opacity *mechanism* for fills/chips). Add as siblings of the `label` group in `color.css.ts`, published as global `--var`s. **Both link types are accent-driven but use *different* accents** (defaults, provisional/swappable):
  - **`link`** — external `[text](url)` → the **system (OS) accent**, `var(--system-accent)`. This is a **new standing var**: today the OS accent is read only to resolve `--accent` when the setting is `system` (`accent.ts` `accentValue`); add `--system-accent` seeded in `theme-vars.css.ts` and populated on load/refresh via the existing `window.nexus.systemAccent()` (Electron) / `readCssAccentColor()` (web) read.
  - **`connection`** — `[[Title]]` → the **Pommora accent**, `var(--accent)` (the app's settings accent — already exists; re-themes with the user's chosen accent).
  - So: external links wear the OS accent, internal connections wear Pommora's. **`code`** — inline/fenced code text (systemRed @ 0.85). Plus an explicit `accent` binding. Heading-marker gray reuses `color.label.tertiary`.

  **Link/connection rendering is GLOBAL, not editor-local.** Because connections render **off-page** too (views, backlinks, future surfaces), the link/connection *rendering treatment* (color + underline + hover) lives in the global `renderer/src/styles.css` (or a shared recipe over the tokens), so every surface reuses one look. The editor's `MarkdownPM/Styles.css` *applies* it; it does not own it. (This is the one explicit exception to "all editor appearance lives in `MarkdownPM/Styles.css`" — see §5.)
...
- **Display-only state lives in dedicated local UI-state files, never source and never frontmatter.** Heading-fold state goes in its **own** file — `.nexus/folds.json` — keyed by entity **id**, **sync-excluded** (per-machine — Obsidian's `workspace.json` model). Each per-page UI-state *concern* gets its **own dedicated JSON** rather than one shared `workspace.json`, so a map that scales with page count (folds today; future column widths, view prefs) can't bloat an unrelated store. This keeps the `.md` portable (no Pommora UI noise leaking to Obsidian) *and* the SQLite index pure (it stays a regeneratable, no-user-data, Swift-schema-identical index — fold state would violate all three). **Divergence from Swift** (which stores `folded_headings` in frontmatter) is deliberate and recorded as a shared-paradigm change in `History.md`; Swift should align. If cross-device fold sync is later wanted, this graduates to the cloud UI-state layer.
...
| --------------------- | ------------------------------------------------------------------------------------------- | ---------------------------------------------------- |
| Image embed           | `!\[\[([^\]\r\n]*)\]\]`                                                                     | 1 = name                                             |
| Wikilink / connection | **reuse `pageLinkPattern()` from `@shared/connections`** (title-only; excludes `![[ ]]`) | 1 = title (`\|alias` dropped) |
| Markdown link         | `\[([^\]\r\n]+)\]\(([^\)\r\n]+)\)`                                                          | 1 = text, 2 = url                                    |
...
> **Connection syntax (React model is authoritative).** `[[Title]]` is the only connection — **title-only, no id, no alias** (`shared/connections.ts`: a legacy `|…` segment is dropped, resolution is computed at read time from normalized title → unique page). `![[ ]]` is an image embed (separate construct), and `{{ }}` is **not** a connection. The editor **reuses `@shared/connections`** (`pageLinkPattern`, `normalizeTitle`) for detection so it can never disagree with the scanner/resolver/rewrite (DRY) — do **not** port the Swift `[[Name|alias]]` regex.
...
**Editor-construct appearance lives in one file — `MarkdownPM/Styles.css`.** The behavior layer never holds a color/size literal; it only assigns CSS class names. Styles.css is the single place an editor construct's look is authored (headings, blockquote, callout, code, checkbox, …), and every color/fill/border in it resolves from the **root design-system tokens via the `--var` bridge** — so values stay DRY-sourced from the token layer, never duplicated. **Exception:** treatments that also render *outside* the editor — notably link/connection coloring (links exist off-page: views, backlinks) — live in the **global** `styles.css`/token layer and the editor merely applies them (§1.1). Token mapping: body text → `color.label.primary`; muted markers → `color.label.secondary`; further-muted (phantom link) → `color.label.tertiary`; blockquote/callout fill → `color.fill.tertiary`; borders → `color.separator.*`; HR rule → `color.separator.line`; external link → `link` (= system accent, `var(--system-accent)`, to add); connection/wikilink → `connection` (= Pommora accent, `var(--accent)`, to add); code text → `code` (to add); checkbox-checked → `accent` (to bind). See §1.1.
...
- **Code — its own styling identity.** Inline and fenced code share one dedicated `code` style block in `Styles.css` (a distinct visual treatment, authored as its own section — not reused generic tokens): monospace family, the `code` text color (= `systemRed` @ 0.85 token), and a dedicated code background fill. Inline code additionally gets inset padding + a small corner radius; fenced code its block treatment (below). This is a first-class look, defined once and shared by both code forms.
- **Inline code** — content uses the code style block (monospace, `code` color, code background, inline padding/radius). Markers (backticks): active → `mutedText`; inactive → `mutedText` @ `inlineCodeMarkerAlpha` (0.5), reduced size.
- **Strikethrough** (`~~…~~`) — `text-decoration: line-through`, color `bodyText` (`.labelColor`). From the supplemental AST pass.
- **Markdown link** `[text](url)` — inactive: `link` color (= **system/OS accent**, `var(--system-accent)`), underline, the `(url)` parenthetical hidden. Active: `link` color @ `activeLinkAlpha` (0.55), no underline, url revealed. URL normalized (prepend `https://` if no scheme). Spell-check disabled on the range.
- **Wikilink / connection** `[[Title]]` — title-only (§4). Markers `[[`/`]]` always `mutedText`. Resolution comes from the **existing connections layer** (§7) and has **three states** (`shared/connections.ts` `LinkStatus`): **resolved** → the **`connection`** color (= **Pommora accent**, `var(--accent)`) + underline (navigates to the target); **phantom** (no page holds the title) → muted (`label.tertiary`), no link; **ambiguous** (≥2 pages hold it) → a distinct warning treatment. Active: content unstyled (raw editable source). A live "connections changed" refresh re-styles so a phantom lights up the moment its target is created. **Styled colored inline text, never a chip.** Spell-check disabled.
...
- **Headings** — exact scale (multiplier × base body size), bold, with em-relative top spacing:
  ...
  - **Foldable.** A fold chevron in the left gutter toggles collapse of the section (down to the next equal-or-higher heading, or document end). **Visibility rule:** when the heading is **open**, the chevron shows **on hover only**; when the heading is **closed (folded)**, the chevron **stays visible in the gutter even when not hovered** — a persistent affordance signalling the section is collapsed (closed/right-pointing orientation). It **reuses the sidebar's exact disclosure language** (not Swift's values): the Lucide **`chevron-right`** icon at size 12, rotating to 90° on expand via the `.twisty` transition; and the collapse/expand animates with the sidebar's **`Reveal`** mechanism — CSS grid `grid-template-rows` `0fr ↔ 1fr` over **180ms `ease`** (`--duration-fast` / `--ease-standard`). This **supersedes the Swift 200ms ease-in-out + SF-Symbol spec** — Pommora-React uses one disclosure standard across sidebar and editor. Fold state persists in its **own dedicated local file** (`.nexus/folds.json`, keyed by page id; §2), **not frontmatter** — the fold keys are still ordinal-disambiguated for duplicate-text headings. **CM6 note:** CM6's native fold doesn't animate; wrap the folded range's collapse in a height/grid transition (or animate a widget) to match the sidebar — the animation parity is a requirement, not incidental.
    - **Chevron color (measured, non-blocking):** the sidebar chevron inherits the row's leading-glyph color `var(--label-secondary)` = `color.label.secondary` (white @ **65%** opacity in dark; `#222224` in light). A **closed** heading may optionally tint its title/chevron toward this `label.secondary` value to read as "collapsed" — an open option, not a requirement.
...
- **Wikilink resolver — wired to the existing connections layer (NOT a no-op).** React already ships connections (`shared/connections.ts` + `main/connections/*`), and `shared/connections.ts` is renderer-importable expressly for "inline styling." The editor resolves via `resolveTitle(normalizeTitle(text), linkIndex) → { status: 'resolved' | 'phantom' | 'ambiguous', targetId? }`. **LinkIndex delivery (recommended): build it renderer-side from the already-loaded page tree** via a shared `buildLinkIndex` (move it from `main/connections/resolve.ts` to `shared/` — it's a pure `{id,title}[] → Map` function) so it stays in sync with the tree's live-refresh and needs **no new IPC**. A tree change re-derives the index → a "connections changed" restyle. **Click-routing** navigates to `targetId`; **rename cascade** rides the existing `main/connections/rewrite.ts` + `crud/cascade.ts`. Only a backlinks *panel* (separate UI) stays out of editor scope.
...
### 12. Connection autocomplete panel (`[[`) — v1

Ports the **existing Swift autocomplete design** (`Connections/AutoCompleteWindow.swift`) to React. A pure presentation panel over a candidate list + callbacks — no index/resolver coupling; the editor maps the connections index → candidates and feeds them in.

- **Trigger + content.** Typing `[[` opens a small popup **above the caret** listing pages **nexus-wide**, filtered live by the text typed inside the brackets. Selecting inserts the page **title** (the connection resolves by title — §4/§7). A convenience, not load-bearing: a bare typed title resolves identically.
- **Surface.** The app's existing **glass material** (`GlassSurface`/`GlassControls`) at corner radius **12**. An empty candidate list renders **nothing** (the panel simply isn't presented).
- **Rows.** Entity **icon** (Lucide) + **title** in body font, tight inline padding at **chip density** (mirrors the chip primitives). Fixed row height **28px**, horizontal padding **8px** (+4px outer). The selected row gets a subtle **quaternary fill** at radius **6**.
- **Match = prefix** (starts-with, case-insensitive): the matched leading `query.length` characters render in **`label.primary`**, the remainder in **`label.secondary`**. This positional split (`highlightSplit`) is the one piece worth unit-testing.
- **Keyboard + selection.** ↑/↓ move the **clamped** selection, **Enter** selects, **Esc** cancels, click selects. Selection **resets to 0** whenever the candidate list changes (a new query = a new list).
- **Sizing.** Height = `min(count, 4) × 28px`; beyond 4 candidates it **scrolls**, keeping the selected row visible. Width **min 160 / max 320**.
- **Candidates** come from the same source that drives resolution (§7) — the loaded page tree, filtered by the in-bracket query; the panel itself stays index-decoupled (it receives a `{ id, icon, title }[]` + `query` + `onSelect`/`onCancel`).
