...
### Sidebar drag-and-drop adopted — the "sidebar" behavior (2026-06-19)

After an 8-variant feel spike (a standalone `sidebar-dnd.html` showcase comparing active displacement at 230ms vs Apple-style insertion lines), Nathan chose **insertion-line-solid + grab-ghost** as the canonical **"sidebar"** drag behavior: an accent line marks the exact drop, the picked-up row stays **muted in place**, and a ghost rides the cursor — no row displacement. Revisitable, but settled for now.

Adopted into the real app, resolving the prior deferred "tree cross-level + real-app adoption" note: the sidebar tree now **reorders pages within a folder *and* reparents cross-set / cross-collection**, position-aware. It's a bespoke behavior (`components/sidebarDnd.tsx` — live-rect hit-test since nothing displaces, portal ghost to escape the glass `backdrop-filter` containing block; pure, unit-tested model in `sidebarDndModel.ts`) that **replaced the container-only `useTreeMove`** (since removed — the sidebar was its only consumer); the PommoraDND sort engines + Interaction Lab are untouched.

**Cleanup (same day):** `SetRow` / `CollectionRow` / `VaultRow` collapsed onto a shared `ContainerRow`; the drag gesture became a discriminated union (`idle` / `pending` / `active`); and the 8-variant spike (`sidebar-dnd.html` + folder) plus the now-unused `useTreeMove` were deleted.

**Extended to all entities (same day):** reorder generalized so **every sidebar entity reorders within its parent heading** — collections / sets (parent-sidecar `collection_order` / `set_order` via a new `reorderChildren` op), vaults + the three context tiers (`.nexus/state.json` via `reorderTop`), pages (`page_order` via `movePage`). The engine derives the right sibling group + commit from each row's kind; only the code-fixed Saved pins stay inert. Also fixed a drag-up bug (an insertion point could resolve to the dragged page itself, appending it to the bottom). The write→read path is **proven correct end-to-end** by a round-trip test (write `page_order` to a Swift-era sidecar → `readNexus` re-applies it); an in-app non-persist seen during testing traced to a **stale main process**, not the persistence code.

**Contract:** `movePage` gained an optional `order` — the destination container's full page-id list after the drop. Main moves the file (when reparenting) then persists `page_order` to the destination sidecar via `setPageOrder` (container kind detected on disk); a stale source id self-drops on the next read (`resolveOrder`), so only the destination is rewritten. Same parent + order = a pure reorder. `page_order` was already a ratified on-disk field — this only wires it to drag. (Adopted/unadopted-import pages have path-derived ids, so a moved one falls back to title order; frontmatter-id pages — all Pommora-created — reorder exactly.)
