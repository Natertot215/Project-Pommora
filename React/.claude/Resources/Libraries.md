...
- **@dnd-kit/core 6** — linear / nested drag-reorder (sidebar move; the deferred Contexts-as-blocks composer). A flat-array `[id, depth, parentId]` tree handles cross-level drag; nested arrays don't compose with dnd-kit sortables. **Not-yet-needed.** (NOT `@dnd-kit/react`, which is pre-1.0.)
- **react-grid-layout** — responsive, draggable + resizable **grid** layout with breakpoints; React-only, MIT (`/react-grid-layout/react-grid-layout`). Fits 2-D **dashboard / widget** composition — the Homepage composed-blocks dashboard, and a candidate for context-level block editing. Complements dnd-kit (linear / nested reorder) rather than replacing it. **Candidate** — verify React 19 compatibility + maintenance health at adoption time.
