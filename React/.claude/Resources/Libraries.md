...
- **PommoraDND** — the **in-house drag-and-drop engine**, behind the `interactions/drag.tsx` seam; replaces `@dnd-kit`. Built for our scope (Chromium-only, React-only, known surfaces): measure-once, no mid-drag array churn, pointer-capture single sensor, closest-centre + hysteresis, decide-then-animate. **Decided + in build** (Phases 0–2 in the Interaction Lab). Spec → `Features/DragAndDrop.md`; plan + dnd-kit dissection → `Planning/PommoraDND-Research.md`.
- **@dnd-kit/core 6** (+ `/sortable`, `/utilities`) — the reference engine PommoraDND was dissected from and is replacing. **Retained** in `package.json` until PommoraDND reaches parity in the Lab, then removed at cutover. (NOT `@dnd-kit/react`, which is pre-1.0.)
