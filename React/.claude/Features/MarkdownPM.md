...
- **Aliased connections collide with cell pipes** — a `|` in `[[Title|alias]]` is escaped to `\|` like any in-cell pipe, so the table stays intact but the alias separator is lost; autocomplete inserts alias-free `[[Title]]`, manual aliases degrade (open pipe-vs-alias paradigm call).
