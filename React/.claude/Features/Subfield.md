...
Per-nexus, in `.nexus/settings.json` under a React-owned **`subfield`** foreign key (Swift ignores unknown keys, so it round-trips): `{ order: per-view item ids, expanded }`. Read/written by `main/settings.ts` `readSubfield`/`writeSubfield`; surfaced over `subfield:get` / `subfield:set` IPC (mirrors the `folds` seam). The store loads it once on nexus open and persists on every change. Item ids from disk are validated against the registry before render.
