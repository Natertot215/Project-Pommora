...
Lean current-state snapshot. Read first at session start. Deep docs: editor → `Features/MarkdownPM.md` (+ exhaustive build spec `Planning/MarkdownPM.md`); data/IPC → `Features/Architecture.md`; design system → `Features/Design.md` + `Features/Typography.md`; drag-and-drop → `Features/DragAndDrop.md`; banner → `Planning/Banner-Design.md`; parked ideas → `Prospects.md`; locked decisions → `History.md`.
...
**▶ ASAP — clear these three, in order:**

1. **Tables rendering** — GFM pipe-tables as styled, caret-aware text (pipes + the `|---|` separator hidden off-caret, revealed on entry; inline plain-text editing). No grid engine. Spec → `Features/MarkdownPM.md`.
2. **Stats footer on pages** — the hover bar: `Vault › Collection › Page` breadcrumb + line / word / char counts. The `editor/textStats.ts` stub exists, unwired.
3. **Icon picker** — build the real `Components/IconPicker` (the Edit-Icon menu routes to a stub) + wire the icon's frontmatter save. The Swift `IconPicker` is the spec (SF-Symbols 6-wide glass grid, pill search, saved-on-top, Remove row).

**Then — finish the editor:** the last constructs — `::` callouts (→ portable `> [!type]` via the codec) and the image + latex render seams (detected/styled today, drawn by a no-op seam).

Beyond that the next phase is page properties + frontmatter inspector, then Agenda surfacing and the Homepage's dynamic widgets — roadmap in `Framework.md`.
