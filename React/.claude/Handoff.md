...
- **Widget-table editing — WORKS; the widget is now the committed default.** The Tables rebuild replaced the old decoration approach (removed in `915e07d`): each cell is a live nested CodeMirror editor — click → focus → type → sync to canonical GFM, focus persists across keystrokes (`tableSelfEdit` remap-not-rebuild), and Tab / Shift-Tab / Enter navigation all verified on disk (commits `5ae1f28` → `b7de2e7`). The earlier "focus blocker" was a test artifact — automated clicks landed ~75px high on the heading; **lesson: verify on-screen coordinates before trusting a UI-automation negative.** **Latent items (from the cleanup review — handle with their feature):** (1) pasting multi-line text into a cell isn't newline-escaped → would break GFM; fix when multi-line `<br>` cells ship. (2) `cellCommitChange` re-locates a table by index — fine now (external edits rebuild), worth a table-ID once structural ops shift indices. **Still to do:** multi-line cells, `[[…]]` connections resolving in-cell, then the drag / resize / structural-menu layer.
