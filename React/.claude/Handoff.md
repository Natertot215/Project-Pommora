...
- **Widget-table editing — WORKS; the widget is the committed default.** Each cell is a live nested CodeMirror editor over canonical GFM (the old decoration approach was removed in `915e07d`). Shipped this session: backslash+pipe escaping with a display round-trip, in-cell line breaks (Shift+Enter → newline, serialized as `<br>` so the row never splits), `atomicRanges` boundary protection, the caret-in-cell scroll fix, the banner-park fix (cells declined the header's named scroll-timeline), and `[[…]]` connections rendering + autocomplete inside cells. **Carry-forwards:** (a) the cell autocomplete machinery near-duplicates the main editor's — extract a shared `useConnectionAutocomplete` hook as its OWN commit, re-verifying the main editor (per the simplification review); (b) the `### Tables` section of `Features/MarkdownPM.md` still describes the superseded decoration-grid, not the widget — reconcile it; (c) aliased `[[A|B]]` vs cell-pipe is an open paradigm call. **Still to do:** the drag / resize / structural-menu layer (headless ops already staged in `operations.ts`).
