...
- **Widget-table cell editing — WORKS (the earlier "focus blocker" was a false alarm).** The slice-2 widget (commits `5ae1f28` render, `945c0c8` data, `78a012f` cell editors): cells are live nested CodeMirror editors; click → focus → type → sync to canonical GFM all verified on disk, and focus persists across keystrokes (the `tableSelfEdit` remap-not-rebuild sync holds). The prior "cells don't focus" diagnosis was a *test artifact* — automated CGEvent clicks were landing ~75px high on the heading, never the table; a correctly-aimed click edits the cell fine. **Lesson:** verify on-screen coordinates before trusting a UI-automation negative. **Still to do:** cell navigation (Tab / Enter / arrows), multi-line cells (Shift+Enter → `<br>`), connections (`[[…]]`) resolving inside cells, then the drag / resize / structural-menu layer. Live behind the uncommitted `index.tsx` swap.
