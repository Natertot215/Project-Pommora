...
### The nested-quote bar — a constraint mistaken for a value

A blockquote nested inside a callout draws a left **bar** over a tint **fill**. For many live rounds the bar's cap was wrong — clipped flat, or square where it should round, or the box's bottom torn open. Each round produced a new CSS variant; each got rejected; the next swapped one flaw for the opposite one. The thrash had a tell nobody named for hours: **the two leading attempts each failed for the *other's* strength.** Put the bar+fill in one `::after` and the fill rounds beautifully but the bar's cap is clipped by that same radius. Move the bar to the line's `background` and the cap is unclipped but now square — a background can't round a corner. Round-trips between those two were not "picking wrong"; the picking *was* the problem.

**The root cause was a count, not a value.** A CodeMirror line gives exactly **two** pseudo-elements. The callout's border already owns `::before` (it must, to close the box on every inner line). That leaves one `::after` for **both** the fill and the bar — and a single element carries a single `border-radius` that clips everything painted into it (gradient, inset box-shadow, all of it). Three things that each need their own rounding — border, fill, bar — cannot fit in two pseudo-elements plus an un-roundable line background. The goal wasn't mis-tuned; it was **provably unreachable** inside the layer budget. No radius, no offset, no `box-shadow` trick was ever going to land it, because the missing thing was a *surface*, not a number.

**What ended it: a review asked to enumerate the option space, not validate an attempt.** Instead of grading the latest CSS, a CSS-review agent was told to list *every* way to get rounded-fill + unclipped-rounded-bar + intact-border under CM6's constraints, with tradeoffs, and pick one. It named the wall in a single pass — two pseudos, one already spent — and the only escape: **add a paint layer.** Make the bar a real DOM element (a per-line widget, the exact `WidgetType` mount the drag-grip already uses), so it's a genuinely separate surface with its own `border-radius` that nothing can clip. The fill keeps its rounded `::after`; the border keeps `::before`; the bar is the third layer the line couldn't otherwise give. Nathan had said the same thing in plain words rounds earlier — *"it's a layer thing, put the bar separate"* — while the code kept trying to spend a budget that was already gone.

**The fix is minimal by construction:** the widget rides the pass that *already* detects nested-quote lines (one new `lineWidget` intent + one generic `LineWidget`), and its caps reuse the `md-bq-in-first/last` classes that already existed for corner rounding — no separate extension, no re-detection, no scaffolding. The historically-broken callout-LAST case (an empty quote as the box's final element) renders clean on the first try, because the border was never the thing being fought.

**The lesson — distinguish a tuning problem from a constraint problem.** A tuning problem yields to nudging a value; a constraint problem does not, and nudging it forever is the tell. Two specific signals were screaming and went unread: (1) **each attempt failed for the other's success** — when your two best options are exact opposites and each is rejected for what the other does well, the axis you're choosing on *is* the constraint, and you need a different axis (here, a layer count); (2) you're **changing the value and the failure only moves, never shrinks.** The tiniest thing went unnoticed the longest — that "we're out of pseudo-elements" is a countable, checkable fact you can establish in one minute, and it makes every subsequent CSS round a guaranteed waste. Adversarial review pays off most not by grading the current attempt but by being pointed at the *space of attempts* — asked "enumerate what's possible here and why," it collapses a dozen live rounds into one. Green was never going to arrive by tuning; the review that mapped the constraint is what earned it.
