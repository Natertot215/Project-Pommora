...
const seedType = (db: Db, id: string) =>
  db.prepare("INSERT INTO page_types (id, title, modified_at) VALUES (?, 'T', '2026-01-01T00:00:00Z')").run(id)
const typeIds = (db: Db) => (db.prepare('SELECT id FROM page_types').all() as { id: string }[]).map((r) => r.id)
...
describe('openIndex', () => {
  ...
  it('deletes + recreates on a version mismatch, dropping stale data', () => {
    ...
    expect(second && typeIds(second.db)).toEqual([]) // stale data gone
    second?.db.close()
  })
