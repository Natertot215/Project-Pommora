...
private func makeFolder(typeID: String, collectionID: String, title: String = "Topic A") -> Folder {
    let folderURL = URL(fileURLWithPath: "/tmp/dummy-\(UUID().uuidString)")
    return Folder(
        id: ULID.generate(),
        typeID: typeID,
...
struct IndexUpdaterTests {
    ...
    @Test func upsertFolderIndexesARow() async throws {
        let nexus = try TempNexus.make()
        defer { TempNexus.cleanup(nexus) }
        let idx = try makeIndex(at: nexus)
        let updater = IndexUpdater(idx)
        ...
        let folder = makeFolder(typeID: pt.id, collectionID: pc.id)
        try updater.upsertFolder(folder)

        let count = try countRows(in: "folders", db: idx)
        #expect(count == 1)
        let row = try firstRow(in: "folders", db: idx)
        #expect(row?["id"] as String? == folder.id)
    ...
    @Test func upsertFolderTwiceIsIdempotent() async throws {
        let nexus = try TempNexus.make()
        defer { TempNexus.cleanup(nexus) }
        let idx = try makeIndex(at: nexus)
        let updater = IndexUpdater(idx)
    ...
    @Test func deleteFolderRemovesRow() async throws {
        let nexus = try TempNexus.make()
        defer { TempNexus.cleanup(nexus) }
        let idx = try makeIndex(at: nexus)
        let updater = IndexUpdater(idx)
        ...
        try updater.deleteFolder(id: folder.id)

        let count = try countRows(in: "folders", db: idx)
        #expect(count == 0)
    }

    @Test func upsertPageWithFolderIDRoutesPageFolderID() async throws {
        let nexus = try TempNexus.make()
        defer { TempNexus.cleanup(nexus) }
        let idx = try makeIndex(at: nexus)
        let updater = IndexUpdater(idx)
        ...
        try updater.upsertPage(
            meta,
            pageTypeID: pt.id,
            pageCollectionID: pc.id,
            pageFolderID: folder.id
        )

        let row = try firstRow(in: "pages", db: idx)
        #expect(row?["page_type_id"] as String? == pt.id)
        #expect(row?["page_collection_id"] as String? == pc.id)
        #expect(row?["page_folder_id"] as String? == folder.id)
    }

    @Test func upsertPageWithoutFolderIDLeavesFolderColumnNull() async throws {
        let nexus = try TempNexus.make()
        defer { TempNexus.cleanup(nexus) }
        let idx = try makeIndex(at: nexus)
        let updater = IndexUpdater(idx)
        ...
        let meta = makePageMeta()
        try updater.upsertPage(meta, pageTypeID: pt.id, pageCollectionID: pc.id)
    ...
    @Test func deleteFolderViaCascadeIsNotAutomatic_FKEnforcedAtAppLayer() async throws {
        // The `addPageFolderIDColumnIfMissing` ALTER cannot include a REFERENCES
        // clause (SQLite restriction). The fresh-DB `pagesDDL` does declare
        // `REFERENCES folders(id) ON DELETE SET NULL`, but FK enforcement is
        // pragma-controlled and we don't rely on it across all index instances.
        // Application code clears `pages.page_folder_id` explicitly when a
        // Folder is deleted (PageTypeManager.deleteFolder + IndexUpdater).
        //
        // This test pins the contract: deleteFolder removes the folders row;
        // it does NOT cascade-update the pages.page_folder_id. The page
        // is left with a now-orphan FK that the next IndexBuilder.populate
        // will fix.
        let nexus = try TempNexus.make()
        defer { TempNexus.cleanup(nexus) }
        let idx = try makeIndex(at: nexus)
        ...
        let folderCount = try countRows(in: "folders", db: idx)
        #expect(folderCount == 0)
        // Page row still exists; its page_folder_id value may or may not
        // remain — both are acceptable. Just assert the page row survives
        // (no cascade-delete of pages).
        let pageCount = try countRows(in: "pages", db: idx)
        #expect(pageCount == 1)
    }
}
