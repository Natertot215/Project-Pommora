...
/// one Collection / Collection, so a duplicate title across DIFFERENT collections slips
...
struct NexusWideUniquenessTests {
    ...
    // MARK: - Test 1: cross-collection Page dup rejected

    @Test("createPage rejects a title already used in a DIFFERENT vault")
    func crossCollectionPageDupRejected() async throws {
        let nexus = try TempNexus.make()
        defer { TempNexus.cleanup(nexus) }
        let (index, _) = try PommoraIndex.open(at: nexus.rootURL)

        let v1 = try makeCollection(in: nexus, index: index, title: "V1")
        let v2 = try makeCollection(in: nexus, index: index, title: "V2")
