...
@Suite("TierValueAdapter — Item")
struct TierValueAdapterItemTests {
    ...
    @Test func tier1ReadFromRoot() {
        let item = makeItem(tier1: ["01A"])
        #expect(item.relationIDs(forPropertyID: ReservedPropertyID.tier1) == ["01A"])
    }

    @Test func tier2ReadFromRoot() {
        let item = makeItem(tier2: ["01B"])
        #expect(item.relationIDs(forPropertyID: ReservedPropertyID.tier2) == ["01B"])
    }

    @Test func tier3ReadFromRoot() {
        let item = makeItem(tier3: ["01C"])
        #expect(item.relationIDs(forPropertyID: ReservedPropertyID.tier3) == ["01C"])
    }

    @Test func tierReadReturnsEmptyWhenRootIsEmpty() {
        let item = makeItem()
        #expect(item.relationIDs(forPropertyID: ReservedPropertyID.tier1) == [])
        #expect(item.relationIDs(forPropertyID: ReservedPropertyID.tier2) == [])
        #expect(item.relationIDs(forPropertyID: ReservedPropertyID.tier3) == [])
    }
    ...
    @Test func tier1WriteUpdatesRootField() {
        var item = makeItem()
        item.setRelationIDs(["01A", "01B"], forPropertyID: ReservedPropertyID.tier1)
        #expect(item.tier1 == ["01A", "01B"])
        #expect(item.properties[ReservedPropertyID.tier1] == nil)
    }

    @Test func tier2WriteUpdatesRootField() {
        var item = makeItem()
        item.setRelationIDs(["01A", "01B"], forPropertyID: ReservedPropertyID.tier2)
        #expect(item.tier2 == ["01A", "01B"])
        #expect(item.properties[ReservedPropertyID.tier2] == nil)
    }

    @Test func tier3WriteUpdatesRootField() {
        var item = makeItem()
        item.setRelationIDs(["01A", "01B"], forPropertyID: ReservedPropertyID.tier3)
        #expect(item.tier3 == ["01A", "01B"])
        #expect(item.properties[ReservedPropertyID.tier3] == nil)
    }
    ...
    @Test func userRelationWriteThenRead() {
        var item = makeItem()
        item.setRelationIDs(["01T"], forPropertyID: "prop_rel")
        #expect(item.relationIDs(forPropertyID: "prop_rel") == ["01T"])
        #expect(item.properties["prop_rel"] == .relation(["01T"]))
    }

    @Test func emptyUserRelationOmitsKey() {
        var item = makeItem(properties: ["prop_rel": .relation(["01T"])])
        item.setRelationIDs([], forPropertyID: "prop_rel")
        #expect(item.properties["prop_rel"] == nil)
    }

    @Test func unknownPropertyIDReturnsEmpty() {
        let item = makeItem()
        #expect(item.relationIDs(forPropertyID: "prop_unknown") == [])
    }
}
