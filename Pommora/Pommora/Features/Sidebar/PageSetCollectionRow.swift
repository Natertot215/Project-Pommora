...
struct CollectionSetRow: View {
    ...
    // list PageCollectionRow uses one level up.
        ...
        DisclosureGroup(isExpanded: $expanded) {
            ForEach(disclosureItems) { item in
                ...
                case .page(let page):
                    PageRow(
                        page: page,
                        parent: .collection(collection, pageCollection: parentPageCollection),
                        selection: $selection,
                        editingID: $editingID,
                        justCreatedID: $justCreatedID
                    )
            ...
            SidebarRow(
                ...
                editingID: $editingID,
                justCreatedID: $justCreatedID,
                onRename: { try await collectionManager.renamePageCollection(collection, to: $0) }
            ) {
                ...
                _ = try await CreateWithInlineEdit.run(
                    create: {
                        try await contentManager.createPage(
                            name: title, in: collection, pageCollection: parentPageCollection
                        )
                    },
                    onCreate: { newPage in
    ...
    /// translation, one level down from PageCollectionRow.reorder. Cross-zone drags
