...
extension PageContentManager {
    ...
    // MARK: - Page CRUD (Folder-scoped, F.1.h)

    /// Folder-scoped create. Writes the `.md` inside `folder.folderURL`,
    /// upserts into SQLite with the full FK trio (type / collection / folder),
    /// and inserts into `pagesByFolder[folder.id]`. Folder-scoped Pages are
    /// never in `pagesByCollection` — the parent Collection's loadAll walk
    /// excludes Folder-tagged sub-folders.
    @discardableResult
    func createPage(
        name: String, icon: String? = nil, in folder: Folder, vault: PageType
    ) async throws -> PageMeta {
        do {
            let existing = pagesByFolder[folder.id] ?? []
            try PageValidator.validate(
                title: name,
                tier1: [], tier2: [], tier3: [],
                properties: [:],
            ...
            let page = PageFile(frontmatter: frontmatter, body: "", title: name)
            let url = NexusPaths.pageFileURL(forTitle: name, in: folder.folderURL)
            try page.save(to: url)
            ...
            if let updater = indexUpdater {
                do {
                    try updater.upsertPage(
                        meta,
                        pageTypeID: vault.id,
                        pageCollectionID: folder.collectionID,
    ...
    func renamePage(
        _ page: PageMeta, to newName: String, in folder: Folder, vault: PageType
    ) async throws {
        do {
            let existing = pagesByFolder[folder.id] ?? []
            try PageValidator.validate(
            ...
            let newURL = NexusPaths.pageFileURL(forTitle: newName, in: folder.folderURL)
            // Single-step atomic rename via FileManager.moveItem; no rollback
            // needed (no follow-up metadata write).
            try Filesystem.renameFile(from: page.url, to: newURL)
    ...
    func deletePage(_ page: PageMeta, in folder: Folder) async throws {
        do {
            try Filesystem.moveToTrash(page.url, in: nexus)
            if let updater = indexUpdater {
                do { try updater.deletePage(id: page.id) } catch { self.pendingError = error }
    ...
    /// Folder-scoped variant of `updatePage`. Same contract: body-only write,
    /// frontmatter preserved, atomic, in-memory cache mutated after success.
    func updatePage(
        _ page: PageMeta, body: String, in folder: Folder, vault: PageType
    ) async throws {
        do {
            let existing = pagesByFolder[folder.id] ?? []
            try PageValidator.validate(
