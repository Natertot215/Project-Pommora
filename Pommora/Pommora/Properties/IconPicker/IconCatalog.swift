...
/// The complete SF Symbols catalog (SF Symbols 6 — the full system set).
/// Bundled as source so `IconPicker` can browse + search EVERY symbol with
/// no runtime resource dependency. Names are Apple's SF Symbol identifiers
/// (the strings `Image(systemName:)` takes), generated 2026-05-30 from the
/// SF Symbols 6 identifier list — regenerate on an SF Symbols version bump.
///
/// Split into fixed-size chunks purely to keep the Swift type-checker fast on
/// the large literal; `all` concatenates them in catalog order.
enum IconCatalog {
    private static let c0: [String] = [
        "square.and.arrow.up",
    ...
    /// Every SF Symbol, in catalog order (6195 total).
    static let all: [String] = c0 + c1 + c2 + c3 + c4 + c5 + c6 + c7 + c8 + c9 + c10
}
