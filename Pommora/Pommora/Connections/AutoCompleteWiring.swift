...
enum AutoCompleteWiring {
    ...
    /// pair (non-empty placeholder). An empty pair (`[[]]`), an image embed, or
    /// `nil` (caret left the token) all suppress the popup.
    ...
    /// connection syntax (PagesV2 decision #3).
    static func fragment(title: String) -> String {
        "[[\(title)]]"
    }
