...
// resolved icon glyph.
export function EntityGlyph({ item, size, className }: { item: ResolvedNav; size: number; className?: string }): React.JSX.Element {
  const profileImage = useSession((s) => s.tree?.nexus.profileImage ?? null)
  if (item.kind === 'homepage' && profileImage) {
    return <img className={cx('entity-glyph-photo', className)} style={{ width: size, height: size }} src={assetUrl(profileImage)} alt="" />
  }
