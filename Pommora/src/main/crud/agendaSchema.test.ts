...
const defs = async (): Promise<PropertyDefinition[]> =>
  parseDefinitions((await readSidecar(tasks, 'taskConfig', agendaConfigSidecar))?.property_definitions)
...
describe('agenda config schema CRUD', () => {
  it('adds + renames a property on the task config (sidecar-only)', async () => {
    const a = await addAgendaProperty(tasks, 'task', { id: '', name: 'Effort', type: 'number' } as PropertyDefinition)
  ...
  it('gates a lossy type change, then strips on confirm', async () => {
    ...
    expect((await changeAgendaPropertyType(tasks, 'task', a.value.id, 'url', { dropConflictingValues: true })).ok).toBe(true)
