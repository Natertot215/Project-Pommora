...
- **2026-05-17 end-of-session — Pages + NavDropdown ship as v0.2.x patches, NOT v0.3.0 / v0.4.0 minor versions.** Initially structured as v0.3.0 = Pages editor (with internal phases a/b/c) + v0.4.0 = Tabs (separate minor). Restructured to ship as v0.2.x patches: v0.2.7 = Pages editor (prose + standard Markdown), v0.2.8 = NavDropdown, v0.2.9 = directives + heading fold + slash menu (Pages addition), v0.2.10 = wikilinks + rename cascade (Pages addition). **Order between v0.2.7 and v0.2.8 is interchangeable** — whichever ships first is `.7`, the other `.8`. v0.3.0 becomes Properties — the next substantial capability after Pommora is writable + multi-instance. Rationale: Pages and the navigation surface are tightly coupled (writing without history is a regression people notice immediately); shipping inside one patch family is more cohesive than spreading across two minor versions. (v0.2.8 was originally scoped as "Tabs" — pivot to Liquid Glass NavDropdown locked 2026-05-18, structural lock unchanged; see `// Features//NavDropdown.md`.)
