...
| **Content elision** | `NativeTextViewCoordinator` conforms to `NSTextContentStorageDelegate`; `textContentStorage(_:textParagraphWith:)` returns an empty `NSTextParagraph` for source ranges intersecting `foldedRanges`. The layout manager sees zero content for those ranges — no fragments created, no layout space occupied, and selection/find/spell-check route through the content manager so the elided range is unreachable. Wired in `NativeTextViewWrapper.makeNSView` BEFORE the first layout pass so cold-open of a page with pre-existing folds opens already-collapsed (no flash). | [`+HeadingFolding.swift`](External/MarkdownEngine/Sources/MarkdownEngine/TextView/Coordinator/NativeTextViewCoordinator+HeadingFolding.swift), [`NativeTextViewWrapper.swift`](External/MarkdownEngine/Sources/MarkdownEngine/TextView/NativeTextViewWrapper.swift) |
| **Service** | Sole writer of `coordinator.foldedRanges`. `syncHeadingFolding` rebuilds the set from AST + `foldedHeadings` binding; when it changes, calls shared `invalidateFoldLayout(in:union:)` over the affected range so the content-storage delegate is re-queried. `applyFoldStateIfChanged` is the fold-toggle path (chevron click) — delegates to `syncHeadingFolding` and handles the engine-specific overscroll recalc. Owns the chevron rotation animation timer. | [`+HeadingFolding.swift`](External/MarkdownEngine/Sources/MarkdownEngine/TextView/Coordinator/NativeTextViewCoordinator+HeadingFolding.swift) |
...
Hiding fragments at the layout layer was tried in two rounds and failed both times. Root cause: `NSTextViewportLayoutController` is aggressive about re-flowing downstream fragments when fragments GROW but lazy when they SHRINK. Combined with `layoutFragmentFrame` being a stored property the layout manager writes during layout (so a getter override only affects external queries, not internal positioning), neither layout-level approach delivered true collapse.

The canonical TextKit 2 primitive — `NSTextContentStorageDelegate.textContentStorage(_:textParagraphWith:)` returning an empty `NSTextParagraph` for source ranges intersecting `foldedRanges` — elides the content from the layout model entirely. Zero fragments → zero height naturally → no patches needed for caret-skip, selection-collapse, post-toggle restyle, or ghost-pixel cleanup. A single `textLayoutManager.invalidateLayout(for: textRange)` on toggle is the entire propagation primitive.

Knock-on cleanup that fell out of switching primitives:
- `skipCaretOutOfFoldedRangesIfNeeded` deleted — selection can't enter elided ranges in the first place.
- `unfocusCaretIfInsideFoldedRange` survived but in a cleaner shape — only fires on chevron click when the caret was already inside the section being folded (Decision 2).
- `isInsideFoldedRange` renderer guards deleted — folded fragments don't reach the renderer.
- Force-layout chain (`ensureLayout` → `layoutViewport` → `needsDisplay` → `makeFirstResponder(nil)` as a redisplay pump) deleted — `invalidateLayout` alone is enough.
