...
**v0.6.0 is underway** — the view renderers that only ever existed as names over the finished filter → group → sort pipeline. **Cards** is first, and it's complete, hardened, and verification-swept on `cards-view`: the ratified plan executed across all eight phases, then a full interaction-hardening campaign — an 8-agent per-type audit whose value-loss and link-alias fixes landed, the number Bar look, the add-menu redesigned around "list what's not shown, picking reveals," and an 11-agent verification sweep whose confirmed catches (a calendar sub-menu z-burial, a dead empty-cell menu, a blur-committing Back row, compact's un-settable blank tiers) are all folded. Pickers now live at ONE grid-level host so row churn can never tear one open, every picker Blooms in and out (dev-enforced), and the chip remove-× is inert until hover-revealed. Gates are green (1738 tests); what's left is my live pass over the new picker architecture + the banner menu, then merge to `main`. After that: the perf identity-stabilization bundle (one commit currently re-renders the whole grid) and the cross-view merge top-3. Table still ships alone on `main`; Gallery, List, Calendar, and Timeline are still just names.
