...
- **Never do expensive work "on every X," never "reload the entire Y."** No O(N), allocating, layout-reading (`getBoundingClientRect` / `getComputedStyle`), or full-recompute work run synchronously on a high-frequency trigger — every keystroke / input, every render, every pointer-move / scroll / resize, every list item. And never re-walk / refetch / rebuild an *entire* structure when an incremental or cached update suffices. These are THE lag sources (a per-keystroke doc reparse, a per-move rect read over every row, a per-open fs re-walk, a whole-grid re-render on one selection). Instead: cache + memoize, read-once + snapshot, incrementalize, subscribe narrowly, rAF / debounce the hot path. If you catch yourself writing "on every …" or "the whole / entire …", stop and make it incremental.
