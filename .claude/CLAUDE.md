...
| `fa51430` → `b13f9a5` | v0.2.7.2 | Session-10 NavDropdown **SHIPPED + TAGGED `v0.2.7.2`** on `main` (22 commits): docs reconciliation (heart icon + drag in-scope + v0.2.8→v0.2.7.2 sweep) → data layer (EntityStateRef + NexusState + RecentsManager + FavoritesManager, 29 unit tests) → EntityRef enum + WindowGroup + EntityWindowHost + AppGlobals/MainWindowRouter bridge → Recents triggers (sidebar selection + ItemWindow.onAppear) → dropdown UI (NavDropdownButton + EntityRow + segmented Picker + popover) → Back/Forward arrows (⌘[/⌘]) + cursor + Intent enum → multiple toolbar chrome iterations → snapshot pattern for Recents/Favorites (bypasses @Observable-through-popover-host edge case) → empty-state copy. **227 unit tests pass.** Full feature spec at `// Features//NavDropdown.md`. **Functional layer complete; UIX polish deferred** (standalone-window chrome, popover panel fill, toolbar segmented controls, system sidebar toggle replacement) — see `Handoff.md → Next session priorities → v0.2.7.2 UIX polish`. |

**Currently working toward v0.3.0 Properties.** Full implementation spec at `// Planning//v0.3.0-Properties-implementation.md`; companion uncertainty log at `// Planning//v0.3.0-Properties-uncertainty-log.md`. **v0.3.x sub-sequence locked RC-2026-05-19:** .0 Properties / .1 Items pane / .2 Page-wikilinks / .3 SQLite + querying. v0.2.7.x patches continue: `.1` Page editor polish (blockquote + HR), `.2` UIX polish, `.3` Tables custom grid, `.4` sidebar reorder+drag.
...
**Reference plan:** `// Planning//v0.2.7-engine-swap.md` — Phases 0/1/2/4/4.5-basic shipped; Phases 3/4.5-polish/6 deferred to v0.2.7.1 patch. **Engine vendor docs:** `External/MarkdownEngine/NOTICE.md`.
