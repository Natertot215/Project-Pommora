...
| `1c6e270` → `b7a2535` | v0.2.7-h.0 → v0.2.7-h.4 | Session-9 engine swap **SHIPPED**: docs repair → Pallepadehat strip → vendor swift-markdown-engine as local SPM at `External/MarkdownEngine/` + Apple swift-markdown 0.8.0 SPM dep → wire `PageEditorView` to `NativeTextViewWrapper` → character-pair auto-pair (`**`/`__`/`[[`/`` `` ``). **197/197 tests pass** (prior 198 count was off-by-one). Phase 3 (AST tokenizer/styler rewrite) + Phase-4.5 polish (selection-wrap + auto-exit) deferred to v0.2.7.1. |
