...
- **Vercel hosting — built, deploy deferred.** `vercel.json` + `npm run build:showcase` are in; the static build is verified (all three pages, no errors). Decided: deploy to a free `*.vercel.app` URL — the domain `pommora-design-system.com` isn't owned yet — with the deploy method TBD (Vercel↔GitHub dashboard connect recommended, so it auto-redeploys on push). Nothing deployed yet.
