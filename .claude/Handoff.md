...
-**PER 6-23:** Tables are **done** — basic functionality complete after a long fight (the "prove it's sustainable or restart" fork resolved by continuing; the foundation held). Full GFM table editing in React: a block-replace CM6 widget over canonical source, live nested cell editors, drag-reorder of rows/columns, an OS-native grip menu, **column width-resize** (drag a boundary; dashes redistribute), and keyboard-uncorruptable structure (one shape `transactionFilter` + a two-table merge guard). Built off **ckant/codemirror-markdown-tables** (MIT). Detail on the React side → `React/.claude/Handoff.md` + `React/.claude/Features/MarkdownPM.md` § Tables.
-**PER 6-25 (React):** Window-chrome + footer session — the **Subfield** (Swift's footer: breadcrumb with a dimmed forward "ghost" crumb + per-view stats/add-menu, app-level slide toggle, persisted per-nexus in `.nexus/settings.json` under a `subfield` foreign key), a full-height **Inspector** pane (pushes/reflows content), **locked-header content-views** (banner + title pinned while the table scrolls — also fixed banner-less views, e.g. Areas, showing no title), the Add-Banner button unified into one shared component, **`glass-controls` → Apple "Liquid Glass"** (`@samasante/liquid-glass`, real refraction), and scrollbars hidden app-wide. Detail → `React/.claude/Handoff.md` + `Features/Subfield.md`. Next React focus: **MarkdownPM performance** (long pages with tables slow to render; caret jitter on long docs).
