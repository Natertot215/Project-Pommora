...
1. **Relations Redesign execution** — Phases 0–14 + 16 shipped; 17–22 remain (Phase 15 + relation picker/chip UI = parallel session). Next backend step: tier-emit-into-`relations` fix (IndexUpdater/IndexBuilder — tiers currently only hit `tier_links`), THEN Phase 18 cascade, 19 migration, 20 finalize (`tier_links` drop), 21 docs, 22 deferred-log. Pending fixes: legacy-sheet Relation dead-end (Fix Log #10); edit-side relation mirror name/icon (parallel-session-adjacent — coordinate).
