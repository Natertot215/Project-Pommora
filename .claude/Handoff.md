...
11. **Index rebuild needed for the cascade on EXISTING nexuses (important).** The tier-emit→`relations` fix (`4ec3430`) writes tier rows into `relations` only on new/incremental indexing + full rebuild. An existing index DB already at `PommoraIndex.currentSchemaVersion == 2` does NOT auto-rebuild, so its `relations` lacks tier rows for un-edited entities → the Phase-18 Context-delete cascade silently MISSES tier references on pre-existing entities until a rebuild. **Fix:** bump `PommoraIndex.currentSchemaVersion` (2→3) so every existing DB does a one-time delete+rebuild (which now emits tiers to `relations`). Make it the FIRST step of Phase 19/20 (or a quick standalone fix). NOTE: this is the INDEX DB version, distinct from the per-Type sidecar `schemaVersion` that Phase 19.10 bumps.
12. **Relation panel surfaces may show title-only.** Tier-row displays (PropertyPanel/PropertiesPulldown/FrontmatterInspector/ItemWindow) render plain text but may not show the target's icon alongside the title (RelationChip cells/picker now do icon+title). Minor — fold the icon into those surfaces during Phase 21.
