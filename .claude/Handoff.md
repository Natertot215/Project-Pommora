...
**1. Interaction testing + foolproofing (Nathan's #1).** Continue live-hands testing — Task 4 shipped view embeds and the in-app handle menu, so the probe list is now: (a) **long page embed at rest** — the tail is unreachable under caret-priority scroll; adjudicate H-2's open scroll clause; (b) **borderless half-step** — body-hover reveals the grip while the border stays hidden; (c) **view embeds under real hands** — config divergence held on-disk in verification, but drag-heavy gestures (column resize, group drags, row drags between groups) want his mouse; (d) **the picker flows** — drill depth, Duplicate landing, Delete confirm, Source on page embeds; (e) **chrome-strip scroll feel** — wheel over an editing tile's handle/edge strip scrolls the page, its gutter scrolls the tile.

**2. The Navigation system (Nathan's #2).** The prior arc's next: Navigation Window + Dropdown + Inspector, → [[Navigation]] + [[Inspector]].

**3. Remaining plumbing — Task 5 + Task 6 leftovers.** Task 5 (link-graph host passes: indexer + the id-less block-body rename pass — cascade.ts:38's guard means a dedicated pass, NOT an extension) is untouched. Task 6 leftovers after the handle menu shipped early: the G-9 Insert menu (background right-click → Page/View/Block through the same picker), the three lock tiers (G-3 host lock · B-5 kind-specific tile locks · G-5 container view-lock — the scoped-settings footer lock is rendered but inert), embed-header completion (page ⋮ toggles — the view header shipped with the H-5 switcher: pills/dropdown, chrome menus, inline rename; pill drag-reorder rides the interaction pass), and the G-16 Link Page SEARCH pane (typed search over recents-descending; the page-open MRU record is new plumbing with a reserved `state.json` home, shared with Navigation).
