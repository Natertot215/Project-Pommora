...
**Conditional inspector (verified live):** a fitting table compresses for the inspector as before; an overflowing one keeps its width and h-scrolls beneath the hovering glass (`Detail.css` `:has(.table-view.overflowing)` drops the compressed padding). The first cut latched: fit was measured via `scrollWidth`, which floors at `clientWidth`, so once lifted the flag could never release — fixed by comparing the KNOWN column sum (`reflowRef`, zoom-adjusted) against the hypothetical compressed pane (`e4e1b54`). Both states CDP-screenshot-verified. Reworked 07-02 after Nathan's live report (the in-between band — fits open pane, not compressed — read as broken): TWO signals now, `overflowing` (flatten look, current box) + `pane-overflowing` (compression release, full pane). Kill switch: delete the `:has` rule + the `paneOverflowing` signal in TableView's `check()` — the core overflow mechanics don't depend on it.
...
- **Conditional-inspector kill switch** — if it ever fights the overflow mechanics: delete the `Detail.css` `:has(.table-view.pane-overflowing)` rule + the `paneOverflowing` signal in TableView's `check()`. Two-signal rework 07-02 (compress whenever the table fits the FULL pane; lift only past it), pending Nathan's visual confirm.
