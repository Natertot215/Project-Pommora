...
**3. Remaining plumbing — Task 5 + Task 6 leftovers.** Task 5 (link-graph host passes: indexer + the id-less block-body rename pass — cascade.ts:38's guard means a dedicated pass, NOT an extension) is untouched. Task 6 leftovers after the handle menu shipped early: the G-9 Insert menu (background right-click → Page/View/Block through the same picker), the three lock tiers (G-3 host lock · B-5 kind-specific tile locks · G-5 container view-lock — the scoped-settings footer lock is rendered but inert), embed-header completion (page ⋮ toggles + view title right-click toggles), and the G-16 Link Page SEARCH pane (typed search over recents-descending; the page-open MRU record is new plumbing with a reserved `state.json` home, shared with Navigation).
