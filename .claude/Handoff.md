...
### Next Session — Plan + build the ViewPane Properties Flow (7-2, ratified)

**The 7-2 ViewPane Properties Flow log is RATIFIED (07-02, studio-brainstorm + two adversarial rounds folded)** — `Planning/7-2 - ViewPane Properties Flow — Decision Log.md` is the contract: assigned properties + "All Properties" registry disclosure (+ promote, sidebar-style left chevron), the DRY nav-stack slide at every depth, Remove = strip-to-sidecar-cache with per-value schema-currency reconciliation on restore (REOPENS the 07-01 "values stay in frontmatter" ratification — provenance logged), pane-gated Delete in the editor's ⋮, nexus-wide property order inside `properties.json` `{order, defs}`, region-owned two-zone drag, 350px cap + hoisted sidebar edge-fade, top-right circle-plus create, net-new inline-rename channel (NOT a RenamableTitle reuse — round-1 catch), duplicate names allowed on BOTH write paths (round-2 catch). Max Properties killed; clash nudge cut. Next: `superpowers:writing-plans` against the log, then build. Nathan's uncommitted `ViewPane.tsx` 245 floor commits with the build (D-4).
...
After the 7-2 build, the remaining Phase 3 leftovers: the capsule's floating × (**~4px RIGHT of the chip**) if Nathan still wants it post-menu-Clear ruling, slide verification post-OverflowScroll, the D-section "+" wiring. Open thread: **file chips deliberately got no ×** (removal = deleting an attachment; unasked) — confirm with Nathan.
