...
1. **Relations Redesign execution** — Phases 0–14 + 16 shipped; the index tier-emit→`relations` fix + Agenda incremental reconcile shipped (`4ec3430`, `1f76f9b`), so `IndexQuery.incomingRelations` now sees tier→Context links for all 4 entities (incremental + rebuild). **Next: Phase 18b cascade.** Design: add `unlinkTier(contextID:tier:index:)` to the 4 content managers — query `IndexQuery(index).incomingRelations(targetID: contextID)`, filter to that manager's kind, and for each referencing entity remove `contextID` from its tierN array via the Phase-6.5 `setRelationIDs` adapter (NOT `properties["_tierN"]`) + save. **Orchestrate at the CALL SITE** (`SidebarView` delete handlers ~:146 Space / ~:155 Topic / ProjectRow Project; pass `nexusManager.currentIndex`) — the Context managers (`SpaceManager`, `TopicManager` owns Topics+Projects; NO separate ProjectManager) hold no content-manager refs, so call `unlinkTier` ×4 BEFORE the context file delete. **Crux:** `incomingRelations` yields id+kind+title only (no URL); the managers' in-memory caches hold just loaded entities — so `unlinkTier` must resolve each referencing entity's container from the index (`pages`/`items`/`agenda_*` tables carry id+title+container ids) → derive its file URL → load → mutate → atomic save → refresh the in-memory cache + index. High-stakes file mutation: demand a strong integration test. Then 19 migration, 20 finalize (`tier_links` drop), 21 docs, 22 deferred-log. Pending fixes: legacy-sheet Relation dead-end (Fix Log #10); edit-side relation mirror name/icon (parallel-adjacent).
