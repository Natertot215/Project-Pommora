...
**The chip hover (×) SHIPPED (07-02, Nathan-directed live):** every PILL chip (status-pill, select, multi, context/tier — Chip AND ContextChip) hover-reveals an × at its right edge in the chip's TEXT color, the label tail blurring INTO THE FILL beneath it: two stacked copies of the text crossfade over one ramp ending at the ×'s left edge — the crisp copy masks out while a `blur(2px)` twin painted in `--chip-fill` masks in (ContextChip overrides the var inline with its neutral fill). The effect touches TEXT ONLY — fill and border never; the design is Nathan-ratified from live zoomed screenshots. One reusable chip-level mechanism (`chipRemovable`/`chipRemove`/`chipFrost` tokens + `ChipRemoveButton`), opt-in via `onRemove`; Cell routes per-chip removal through the ref-stable `cellApi.remove` (tier → setTier, rest → setProperty). Capsule/checkbox looks carry NO × — menu Clear only (already shipped). **No-empties rule locked in `applyPropertyValue`**: null OR empty (`[]`/`''`) deletes the key — a page without a value carries no key (checkbox false / number 0 stay; tier `area: []` deliberately KEEPS writing — Nathan: context-wiping fights the nexus indexing; one-line prospective in Contexts.md). Live-proven on the real Nexus: × click → `properties: {}` on disk → picker-restored exactly.
