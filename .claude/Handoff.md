...
### Next Session — Phase 3 (chips) plan → review → build

Phase 2 is SHIPPED and fully reconciled; the wake-up report was delivered in-chat. Next up, in Nathan's standing order: **Phase 3 — chip mechanics** (the decision log §E, still in `Planning/7-1 - Tables Next-Parts … Decision Log.md`): the DRY'd slide (extract the sidebar's capture-phase scrolled-flag + `slideScrollBack` rAF into the shared hook — most of this shipped with OverflowScroll, verify what's left) + the chip-native hover **(×)** with the E-3 context table (**on a capsule the × floats ~4px RIGHT of the chip** — it can't sit inside), plus the D-section "+" wiring if it rides along. Own plan → adversarial review → build, per the Review Discipline. After that: the View-Settings property pane (Pending Focuses, Figma-designed + Swift-proven — stop and ask on "Max Properties").
...
- **(Perf) Table-view architecture redo — now QUANTIFIED (07-02 audit).** The audit benched real modules: (1) **the watcher re-walks the whole nexus + swaps `source` identity on ANY fs event** — a cell edit still costs a full `readNexus` ~200ms later, and a mutation in an unrelated container re-runs the open table's whole pipeline (the value-op `load()` skip is an immediacy win, not a cost win); fix levers: content-stable `source` identity (tree diff/hash) or an incremental watcher re-read. (2) **`React.memo` + virtualization on the row/cell path** — a full re-render's JS floor: 0.65ms @100 rows, 13.9ms @2k, 34ms @5k (per-cell `styleFor` allocations + `parsePropertyValue` re-parse). (3) **The property-grouped pipeline re-parses every row's value shape per run** (5.3ms @2k, 13.3ms @5k — crosses a frame ~6k grouped rows) and Cells parse the SAME values again per render — parse once into a cache both read. Fixed already: band hit-testing snapshot index (`e4759d0`) + column drag's per-move re-render (`05b654e`, the cursor-follow rides a grid CSS var).
