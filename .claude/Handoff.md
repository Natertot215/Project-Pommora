...
- **F.1.h — PageContentManager pages-in-folders.** ✅ SHIPPED this session. `pagesByFolder` storage + `pages(in folder:)` accessor + `loadAll(for: folder:)` + Collection-walk exclusion of Folder sub-folders + `resolveParent` 3-tuple signature + full Folder-scoped CRUD overloads (`createPage` / `renamePage` / `deletePage` / `updatePage`) routing FK trio through `upsertPage(pageFolderID:)` + `reorderPages(in folder:)` persisting via `OrderPersister.setPageOrder(_:in folder:)`. 11 PageContentManagerFolderTests, all green; 110-unique-test sweep across Page/Folder/Index suites also green.
...
> "Pommora at `/Users/nathantaichman/The Studio/Projects/Project Pommora`. **F.0 + F.1.a–F.1.h SHIPPED on `main`. `origin/main` synced.** This session's F.1.h commit added `pagesByFolder` storage + `pages(in folder:)` accessor + `loadAll(for: folder:)` + Collection-walk exclusion of Folder sub-folders + `resolveParent` 3-tuple + Folder-scoped `createPage` / `renamePage` / `deletePage` / `updatePage` overloads + `reorderPages(in folder:)` + `OrderPersister.setPageOrder(_:in folder:)` + 11 new PageContentManagerFolderTests. Build green across 110-unique-test sweep (Page/Folder/Index suites). **Next: F.1.i — NexusAdopter + auto-tagging paradigm shift.** Extend `AdoptedSidecarKind` with `.folder` case (filename → `folderSidecarFilename`); add `_folder.json` to the recognized-sidecar set at [NexusAdopter.swift:260–267](Pommora/Pommora/Nexus/NexusAdopter.swift#L260-L267); extend `cleanupLegacyOrphans` at [NexusAdopter.swift:808–819](Pommora/Pommora/Nexus/NexusAdopter.swift#L808-L819) to walk one tier deeper. Add new `autoTagMissingSidecars(at: nexusRoot)` method — silent three-level walk. Depth-aware kind selection per plan spec (lines 376–384). Then F.1.j wires it into `NexusManager.runAdoptionIfNeeded` step 3 unconditionally. F.1.k adds `NexusAdopter+AutoTagTests.swift` + `FolderIndexSyncTests.swift`. **DO NOT touch** `Pommora/Properties/*` or `Pommora/ViewSettings/*` (parallel v0.3.1 work in working tree). Approved plan at `.claude/Planning/2026-05-26-folders-and-stub-crud-refactor-plan.md`."
