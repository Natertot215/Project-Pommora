...
Zustand v5+ vanilla (`useSyncExternalStore`), hand-rolled table-keyed pub/sub for `better-sqlite3` reactivity (~80 LOC), `@parcel/watcher` v2.5+ with APFS / atomic-save gotchas, FTS5 `unicode61` (`remove_diacritics=2`) external-content mode for vault-scale search.
