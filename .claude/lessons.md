...
**Incidents:**
- **2026-04-30** — Implementing the right-side inspector with hover-reveal toggles (commit `8d4974b`). Tried the documented Apple pattern: `.windowResizability(.contentSize)` on WindowGroup + dynamic `.frame(idealWidth:)` on ContentView + `.windowResizeAnchor(.leading)`. Built fine; on toggle, the inspector animated in but the window stayed at its current width — the existing columns compressed instead. Removed the dead-code modifiers; accepted compression as the shipped behavior. Per Nathan's "official method wins" rule (framework.md hard constraint addendum), no AppKit fallback was attempted. **The entire inspector feature was subsequently rewound** because the toggle UX wasn't where Nathan wanted it (commits `8d4974b` and `bd0471a` reverted). This lesson stands as the "we tried, deferred" record so a future inspector attempt knows up front: pure-SwiftUI window-grow on inspector toggle is not achievable in macOS 26; either accept compression or scope the inspector before building.
