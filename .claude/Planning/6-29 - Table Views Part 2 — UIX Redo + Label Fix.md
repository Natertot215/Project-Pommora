...
> Status: V2 — review round 1 folded (compile-grounding clean; logic + UIX→data findings resolved inline). Pending Nathan's read, then ratify. Predecessor: `superpowers:brainstorming`. Successor: `writing-plans`.
...
- [ ] Adversarial spec review against real code (compile-grounding + logic/coverage + UIX→data) before ratification — per `rules/Review-Discipline.md`. NOT ratified until a round comes back clean.

#### E — Must-Haves (interaction surface)

**E-1:** [confirmed] **Hide Property** — right-click a column header → menu → writes `hidden_properties`; un-hide via the Part-3 Visibility pane.
**E-2:** [confirmed] **Column reorder** — drag a header (PommoraDND horizontal `SortableZone`) → commits `property_order`; whole column animates live.
**E-3:** [confirmed] **Row drag** — left-gutter handle (PommoraDND vertical `SortableZone`), MarkdownPM feel via the SAME engine (the CM6 table extension itself is not reused — wrong layer). Persists per D-2…D-5.
**E-4:** [confirmed] **Disclosures** — Set / Sub-Set / Collection group headers are chevron disclosure rows; collapse driven by `collapsed_groups` (built); reuse the existing disclosure animation (no new motion).
...
- **H-1:** [confirmed] **One header row per table** (never repeated per group) + **one shared column-width set** table-wide. Disclosed groups are body sections under the single header — no per-group `<thead>`.
- **H-2:** [confirmed] **Column resize** by dragging the column edge; persists to `column_widths` (already on `SavedView`). Column width + order persist always, independent of sort/group.
...
- **Q-6:** [confirmed] Nested Sub-Sets indent the **group header only**; rows stay column-aligned table-wide.
