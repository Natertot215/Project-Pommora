...
- **H-5 — View-embed header + configuration surface** [confirmed — Nathan, by Notion screenshot + anatomy follow-up]: the embed's header is up to two rows — an optional **title row** (the in-line title, G-6) above the **switcher row**: the view switcher on the left (**tabs or a dropdown** — the existing per-container `view_style` Dropdown/Toolbar duality, → [[Views]]; the toolbar rendering itself is the pending ViewBar), the config affordance on the right, a divider beneath, the view body below. No toolbar row. **Right-clicking the title hides it; right-clicking the empty strip between the switcher and the config affordance shows it again.** The config affordance rides the tile's hover chrome (G-4's top-right family) and opens the existing **settings dropdown scoped to the embed** — the view-shaped leaves (Layout · Visibility · Group · Sort) reading the embed's per-instance resolution (H-4), never the global slot. [confirmed — Nathan] The header's icon + title are **the view's own** — payload edits, never `submitRename`/`setIcon` on the source folder (G-6's law; you can't rename a Collection or its icon from a view). The **Properties leaf renders and wires to the source Collection — deliberately**: a property assigned from an embed must register in the collection schema or pages carry properties the schema doesn't know; it's the one intentionally global leaf. The Configuration leaf waits on B-8's preview surface. The pane's **footer**: the source path breadcrumb on the left — each container with its icon, `(icon) Collection › (icon) Set`, custom icon or the kind default (never empty) — the lock on the right; both label-secondary `footnote.emphasized` `[assumed: the lock is the tile's B-5 config lock — the G-5 ViewPane-footer precedent]`. Per-view source management lives with the view's config surfaces, never the tile handle (G-14) `[open — exact affordance rides the design pass]`.
  - **Switcher pills** [confirmed — Nathan]: each view pill displays **icon-only (default) or icon + title** — the existing per-view `view_button` show/hide-title concept — and pills **drag to reorder** (reordering the payload's `views[]`). **Toolbar mode** reveals a hover **+** to the right of the pills that creates a new view in the tile `[assumed: via the same creation drill as Source › — pick a view to copy, or + Custom]`; **dropdown mode copies the ViewPane's logic** (the list + its CRUD, operating on the tile's copied views, never the source's). The config affordance is the toolbar **SettingsPane button — same icon, same dropdown — except its (icon)(title) header references the *view*, not the source Collection/Set.**
...
3. **Multi-view embeds** [pattern + payload + header anatomy confirmed — Nathan]: one view-embed tile carries multiple switchable views — each bound to its own source per D-5a, the entry contract list-shaped from day one so the on-disk format never migrates, the switcher rendering as tabs or a dropdown in the H-5 header. The visual design pass (Figma, or in-chat sketch) follows Tasks 4–6; plumbing ships single-view against the list shape.
4. **Parked by design:** B-2 (contexts as block surfaces), D-10 (sidebar), the contexts resolution, Homepage's final role.
