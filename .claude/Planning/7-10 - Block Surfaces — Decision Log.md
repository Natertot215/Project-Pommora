...
- **D-5:** [confirmed — Nathan] **View-embed tiles come in two kinds** (vocabulary locked): a **Linked View** references an existing saved view of a Collection/depth-1 Set and follows its future edits; a **Custom View** is embed-owned config that diverges freely — **scoped to a single source container exactly like a Linked View** (Notion's one-database-per-view model). The inline config is `SavedView`-shaped, resolved against that source's schema; a Custom View additionally carries its own Open In (B-8, gated). **Authoring-pane UIX pending design (G-8); the plumbing must exist.**
...
- **D-12:** [confirmed — Nathan] **Embedded views are fully interactive — controlling a Collection's content from a block surface is the point.** Two embed kinds with locked vocabulary:
  ...
  - **Custom View** — per-block config against its single source container (D-5a). Its view configuration is block-owned and diverges freely; its **data edits still write through to the source** like any view's.
...
- **H-1 — Creation** [confirmed — restates G-9/G-7]: embeds are **references, not files** — Insert → *Page* searches every Collection's pages and stores the pick as `page_id` (ULID, rename-proof); Insert → *View* runs the source drill (Collections → Sets chevron → that container's views, **+ Custom** footer) and stores `source_id` + `view_id` (Linked) or `source_id` + the inline config (Custom — the source ref is never optional, D-5a). Only markdown blocks mint files; an embed's tile payload is the entire artifact.
...
- **H-7 — Custom view runtime** [confirmed — D-5a]: resolves against its source container through the exact same rows/schema/values pipeline as a Linked View — the only differences are where config persists (the block payload, never the source sidecar) and the extra Open In knob (inert until the preview surface, like every Open In). New pages minted from either kind are born in the source container — creation always has a home.
...
3. **Multi-view embeds:** whether one view-embed tile can carry multiple switchable views (Notion's tabbed pattern), each view bound to its own source per D-5a — or v1 stays one view per tile with H-5's switcher covering the swap. Nathan floated it with the scoping decision; unadjudicated.
