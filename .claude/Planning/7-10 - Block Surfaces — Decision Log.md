...
- **G-16:** [confirmed — Nathan] **The block menu is an in-app `PickerMenu`, never a native menu** — native menus can't hold search fields, pinned rows, or styled drill panes. The handle menu (and the G-9 Insert menu when it lands) composes existing primitives only: a `PickerMenu` shell notch-anchored to the drag handle — dropping down or opening out the side via its existing collision flip, native-feel placement — hosting the menu family's DRY pane mechanics: `PaneSlider` between panes, the ‹ `MenuPaneTopRow` back header, the pinned-edge scroll frame (pinned top/bottom rows, one scrollable middle). Built **after** the plumbing; the native `blockHandleMenu`/`blockPagePicker` are interim scaffolding until it lands.
  - **Link Page pane** [confirmed — Nathan]: search-first and scrolling. Each result row: (icon)(title) in label at `control` size, over a tight `Collection › Set` path breadcrumb in label-secondary `footnote.emphasized`. The list shows recently-opened pages in descending order until the search bar is used. [open] Page-level recents don't exist yet (only app-level recent nexuses) — the MRU record is new plumbing, shared with the Navigation arc's needs.
  ...
  - **Root rows are state-dependent** [confirmed — Nathan]: an unconfigured markdown block shows **Link View ›** · **Link Page ›** (the conversion moment); once a page is linked or a view configured, both collapse to a single **Source ›** — the source can change, the type never can. Style › · ─ · Duplicate · Delete ride both states. [open] Delete's confirm style (inline two-step vs native dialog); [open] a view embed's Source › semantics — re-seed via the full drill vs re-point `source_id` under the tile's existing copied config.
...
3. **Multi-view embeds** [pattern + payload confirmed — Nathan]: one view-embed tile carries multiple switchable views, Notion's tabbed pattern — each view bound to its own source per D-5a, the entry contract list-shaped from day one so the on-disk format never migrates. The tab *chrome* design (Figma, or in-chat sketch) follows Tasks 4–6; plumbing ships single-view against the list shape.
4. **G-16's leftovers:** Delete's confirm style (inline two-step vs native dialog), and a view embed's Source › semantics — re-seed via the full drill vs re-point the source under the tile's existing copied config.
5. **Parked by design:** B-2 (contexts as block surfaces), D-10 (sidebar), the contexts resolution, Homepage's final role.
