...
- **I-6:** [confirmed] **Pinned tabs never change content in place — including Back/Forward.** On an unpinned tab, Back/Forward navigates in place. On a pinned tab, the first Back/Forward spawns one new unpinned tab that inherits the history stack and navigates there; the pinned tab stays put.
...
- **I-14:** [confirmed] A pin-Back-spawned tab (I-6) appends right; a container/context tab's warm state = scroll only (undo/fold are page-editor-only); close is the hover-`×` (+ tab menu) only, no middle-click.
