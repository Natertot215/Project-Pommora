...
Keep system colors via named slots (brand palette deferred to v0.4.0; Service doc D16 dark-mode-adaptive). Heading multipliers: apply the **new scale** `[H1 2.0, H2 1.75, H3 1.5, H4 1.25, H5 1.15, H6 1.0]` (Nathan 2026-06-02; supersedes shipped `[2.0, 1.5, 1.17, 1.0, 0.83, 0.67]`; no heading below body). Intentional change vs the Phase-2-pinned current values (divergence D-HEAD-2); scale heading padding/spacing proportionally (H6 = 1.0, body size). The `HeadingStyle` sub-struct is the **only** one with behavior (clamp+lookup helpers, `MarkdownEditorConfiguration.swift:264-272`) — preserve it.
