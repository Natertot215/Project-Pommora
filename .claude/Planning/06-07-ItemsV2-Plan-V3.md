...
- **Design-gate (§17.2).** Build from existing components + Design.md; STOP and ASK before any step needing Nathan's net-new visual. **Pre-identified stop: D2 (header chrome)** — get sign-off before building. Other view tasks proceed; stop on any net-new visual that surfaces mid-task.
...
### Task F3: prose-level doc-sweep (final, per standard)
