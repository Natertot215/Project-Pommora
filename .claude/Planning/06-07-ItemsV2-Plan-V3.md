...
>
> **Pre-execution amendments (2026-06-07, Nathan — folded before build):** (1) Property Field = **segmented-style row** (`PropertyFieldBar` CL asset; D4); (2) **§17.2 design-gated — gate EACH visual** (D2/D4/D5/E1); ask when unsure, build non-visual parts first so a gate never stalls the plan; (3) **inline text-field commit model** standard (Enter + blur + dismissal; idempotent; field-sized) — B4/D1/D2/D5; (4) **inspector row = `(icon)(title)──(field)`**, no double-name (D5); (5) `Design.md` added to conventions; (6) **per-task evidence-verification + report→adjust/PAUSE** protocol; (7) **post-functional UIX review** standing phase (F2) + **F4 prose doc-sweep** (final); (8) implementers = **Opus 4.8**, Sonnet for trivial.
>
> **EXECUTION STATUS (2026-06-07): Phase A SHIPPED.** Branch `itemsv2-interactive-window`, commits `14760b0..0def549` (A0–A10 + a cleanup commit), full test target **1283/1283 green**, each task independently builder-verified (non-zero counts). Post-Phase-A code-review pass: **clean — no blockers, no majors**; minor follow-ups fixed (cap-engine test coverage for Pool C + under-cap; `displayIcon` DRY fold at 3 stable sites; legacy-`layout` decode already covered). Phase A confirmed every downstream anchor against real source (`renameItem→Item`, shared `.null` gate, `validate(isBodyEdit:)`, additive `property_layout`, `ItemWindowZoneConfig`, paired `promotedForField`, `nexusManager` injection, `PropertyDefinition.displayIcon`) — **no downstream task (B–F) needs rewriting.** Next: **Phase B (ViewModel)** — paused pending Nathan's background Figma work (see `Handoff.md`).
>
> **EXECUTION UPDATE (2026-06-08): Phases B + C SHIPPED.** Branch `itemsv2-interactive-window`. **Phase B** (`ItemWindowViewModel`, B1–B9) = `22ed49c..2355f1e`, then a clean code-review pass + a `fireSave` DRY consolidation (`48015ae`); full target **1302** green. **Phase C** = `560d855` (C1) + `0c821c6` (C2); full target **1290** green (12 retired tests removed). Next: **Phases D/E — DESIGN-GATED, paused for Nathan's Figma.** Carry these corrections into D/E/F:
> - **`-only-testing` selector = the TYPE name (`<Name>Tests`), NOT the `@Suite("string")` label.** The string label silently runs **0 tests** (verified via xcresult). The "Verified `@Suite` names" list below is WRONG on this point — use the struct/type names and ALWAYS confirm a non-zero executed count.
> - **C1 retired MORE than its task text listed** — a complete display-path retirement: also deleted `ItemTemplatePane.applyDisplay`/`setDisplay`/`syntheticItem` (+ the orphaned `ItemContentManager` env + `ResolvedScope.representativeItem`) and the `ItemWindowDisplayResolutionTests`, archetype-`label(for:)`, and `applyDisplay` tests. The renderer is now a bare stub (`header` + `stubBody` + footer); `partition`/`reorderPromoted` KEPT (D re-wires them).
> - **C2 mechanism = `PreviewWindow(showsDefaultHeader: false)`, NOT a `header:` param** (non-breaking opt-out; default `true` preserves current behavior). **D1/D2 must set `showsDefaultHeader: false`** on the scene's `PreviewWindow` and supply the card's own header inside `content`. Esc-to-close lives in `PreviewWindow.body` and survives regardless.
> - **E1 legacy-pin-collapse (its source was DELETED in C1 — RE-IMPLEMENT it in E1's checkbox pin-write path; the old `:379-396` reference is dead):** on the FIRST `promoted_properties` write to a Collection whose `templateConfig?.promotedProperties == nil`, seed the new list from `TemplateResolver.promoted(type:collection:)`, write it via `updateTemplateConfig { $0.promotedProperties = <seeded+edited> }`, THEN clear the legacy pins — `if establishesPromoted, let collection { try? await itemTypeManager.updateItemCollectionPinnedProperties(collection, to: []) }` (where `establishesPromoted = collection?.templateConfig?.promotedProperties == nil`, captured before the write).
>
...
- **`Design.md` governs all visuals.** Every view task also reads `// Guidelines // Design.md` (Component Library = source of design; semantic colors; PUI tokens; Liquid Glass continuity) alongside `swiftui-expert-skill`.
- **Inline TextField commit model (`Design.md`).** Every inline-edit TextField (title; inspector Number/URL) commits on **Enter + focus-loss + dismissal** (`@FocusState` false-transition + `.onChange` + an `.onDisappear` safety net); commits are **idempotent** (guard `trimmed != current`); hit-target is the field via `.fixedSize(horizontal: true, vertical: false)`.
- **Design-gate (§17.2) — gate EACH visual task** (D2 header, D4 property bar, D5 inspector row, E1 pane). Before building a step's visual, **ASK Nathan for the design when unsure** (no existing pattern fully dictates it). **To avoid stalling the plan, build the non-visual parts first** — each gated task's scaffolding, data wiring, and logic proceed; only the specific visual waits on Nathan, and smaller well-defined pieces never block on a gate. **A post-functional, verified UIX review of the working window always runs afterward** (Phase F2), no matter how clean the build. The 4 visual gates MAY be batched into ONE design checkpoint after all non-visual scaffolding is built (recommended — honors "don't interrupt the plan").
- **Per-task verification + report protocol.** After each green commit: (a) **independently** verify the agent's claim against evidence it can't fake — a real background build with a **non-zero executed test count**, the actual `git diff`, and source/docs (never trust the agent's word); (b) analyze the report + re-assess downstream tasks (CLAUDE.md hard rule), rewriting affected later tasks; (c) on a design-gate or blocking ambiguity, **PAUSE and ASK**.
...
### Task A10: `PropertyDefinition.displayIcon` (single icon source — DRY)
**Files:** Modify `Vaults/PropertyDefinition.swift` (or extend beside `PropertyType.pickerIcon` in `Properties/PropertyTypePicker.swift`); Test add to `PropertyDefinitionTests`.
- [ ] **Step 1: Failing test** — `displayIcon` returns `icon` when set, else `type.pickerIcon` (two cases).
- [ ] **Step 2: Run — FAIL.**
- [ ] **Step 3:** Add `var displayIcon: String { icon ?? type.pickerIcon }` on `PropertyDefinition`. **DRY HARD RULE:** D4 (segment) + D5 (inspector row) call `definition.displayIcon` — never inline `icon ?? type.pickerIcon` at a call site. (Optional in-scope DRY cleanup: fold the 5 existing inline sites — `PropertiesListPane:175`, `EditPropertyPane:156`, `PropertyVisibilityPane:297`, `ItemWindowRenderer:368`, and the soon-deleted `ItemTemplatePane:174` — if cheap.)
- [ ] **Step 4: Run — PASS.** **Step 5: Commit** `feat(properties): PropertyDefinition.displayIcon (single icon source; DRY)`.
...
- [ ] **Step 1:** tests — changed title → `onRename` called, `vm.item` re-held to the returned Item (id same, title new); `onRename` throws (collision) → `inlineError` set + `draftTitle` reverts to `item.title`; unchanged → no-op. **Step 2: FAIL. Step 3:** `handleTitleCommit() async` { let `trimmed = draftTitle.trimmingCharacters(in: .whitespacesAndNewlines)`; guard `trimmed != item.title` (**idempotent** — Enter / blur / disappear may all fire); do `let renamed = try await onRename(trimmed); self.item = renamed; inlineError = nil` catch `inlineError = …; draftTitle = item.title` }. (D2 fires this on `.onSubmit` + `@FocusState` false-transition; D1's `.onDisappear` is the dismissal safety net — `Design.md` inline-commit model.) **Step 4: PASS. Step 5:** Commit `feat(items): VM title commit + re-hold + inline collision`.
...
- [ ] **Build `PropertyFieldBar`** — the **segmented-style** Property Field container, a reusable Component-Library asset (`swiftui-expert-skill` + `Design.md`). **⛔ DESIGN-GATED:** build the non-visual structure first (per-pool slice + data wiring), then ASK Nathan to confirm the segment visual before finalizing styling. Render `TemplateResolver.promotedForField(type: vm.itemType, collection: vm.collection)` (paired entries). **Per-pool slice (plan-review fix):** group entries by `ItemWindowZoneConfig.pool(for: def.type)` and defensively keep at most each pool's cap (V1: combined-total 4 across select+multi) — **not** a single global `.prefix`. Lay the kept entries out as ONE rounded segmented container with dividers between segments; each **segment** = the property's icon (`definition.displayIcon`, A10) + current value; tapping a segment opens a `ChipDropdown` — local `@State private var opts: [PropertyChipOption]` seeded `.onAppear` from `definition.selectOptions?.map { $0.asChipOption() }`; selected from the item's current value; `onPick` → `vm.handlePropertyChange(def.id, …)`. A pinned segment shows **even when empty**. Honor `property_layout == .standard`. **Quirk #12:** hoist the pool-grouping into plain-value helpers / a private subview; `first(where:)` not `contains` inside `@ViewBuilder` (the `ContextPicker.swift` pattern). **Ask-list (gate):** segment divider style; empty-segment affordance; selected/hover state; corner-radius token. Build green; manually verify assignment persists + the segment shows when empty + each segment opens its own dropdown. Commit.

### Task D5: Inspector zone (contexts + filled-non-pinned + Add-Property)
- [ ] **⛔ DESIGN-GATED** (build the row composition + filtering logic first; ASK Nathan to confirm row styling when unsure). Create `ItemInspectorRow` laid out as **`HStack { Image(systemName: definition.displayIcon); Text(definition.name); Spacer(); field }`** — property identity shown **ONCE**, **no** type-as-text label, and **do NOT** wrap it in an outer `LabeledContent(name)` (that re-prints the name — the `FrontmatterInspector.swift:188-199` + `PropertyEditorRow.swift:16` double-name bug Nathan flagged). The `field` composes per type: `.select/.multiSelect/.status` → `ChipDropdown` (local opts); `.date/.datetime` → `DateTimePicker`; `.number/.url` → `TextField` (inline-commit model — Enter + blur + `.onDisappear`, idempotent, `.fixedSize`; `Design.md`); `.checkbox` → `PropertyCheckbox`/`Toggle`; `.file`/`.lastEditedTime` → `PropertyCellDisplay` (read-only). Tier rows follow the SAME `(icon)(title)──(field)` shape, field = `ContextValueEditor(ids: $vm.draftTierN, scope: .contextTier(N), index: nexusManager.currentIndex, resolver: contextResolver)` — **`ids` is a `@Binding` (use `$`, plan-review fix)**; `nexusManager = @Environment(NexusManager.self)` (live index via A9); `contextResolver = @Environment(ContextDisplayResolver.self)`. In `inspectorColumn`: 3 tier slots at top (always; labels from `@Environment(TierConfigManager.self)`); let `promoted = TemplateResolver.promotedForField(type: vm.itemType, collection: vm.collection)`; then rows for each property where **`ItemWindowViewModel.isFilled(vm.draftProperties[id]) OR vm.surfaced.contains(id)`** AND **not pinned** (`!Set(promoted.map { $0.promotion.id }).contains(id)`); then an "Add Property" `Menu` over `ItemWindowViewModel.addableProperties(schema: vm.itemType.properties, filled: Set(vm.draftProperties.filter { ItemWindowViewModel.isFilled($0.value) }.map(\.key)), pinned: Set(promoted.map { $0.promotion.id }))` → `vm.addProperty`. Below a `Divider()`, a read-only meta section (`modified_at` via `PropertyCellDisplay`; `id`/`created_at` collapsed). Use `swiftui-expert-skill` + `Design.md`. **Quirk #12:** compute the pinned-set + filtered rows as plain-value helpers off-`body`; `first(where:)` not `contains` inside `@ViewBuilder`. **Ask-list (gate):** icon size/color; title width/alignment; field alignment; row spacing/divider. Build green; manually verify the row shows identity once (no double-name) + contexts always show (live index, names resolve) + assign persists + Add-Property surfaces→assigns + clear keeps the row this session. Commit.
...
- [ ] **⛔ DESIGN-GATED** (the two muted-state visuals): build the grouped-checkbox structure + write path + muting LOGIC first; ASK Nathan to confirm the cap-reached vs not-in-V1 muted visuals before finalizing. **Ask-list:** cap-reached indicator ('n/N') style; not-in-V1 lock/disabled treatment; group-header style. In `ItemTemplatePane`, add a new section: `Dictionary(grouping: itemType.properties, by: \.type)` rendered as section groups (header layout modeled on `StatusGroupSection`; per-property checkbox row modeled on `PropertyVisibilityPane`'s row — confirm both at edit time). Each property's checkbox = pinned (`promoted_properties` membership). **Muting:** compute `pinnedTypes` via `ItemWindowZoneConfig.pinnedTypes(promoted:schema:)`; for each unchecked row call `ItemWindowZoneConfig.muteReason(def.type, pinnedTypes:)` → disable with a **distinct** treatment: `.capReached` shows the pool's "n/N" count; `.notInV1` shows a muted/lock treatment. Checking writes `template_config.promoted_properties` via `updateTemplateConfig`; on the first pin write to a Set still carrying legacy `pinnedProperties` (`resolved.collection?.templateConfig?.promotedProperties == nil`), call the legacy collapse (`:379-396`). Keep cover + scope sections. Build green; manually verify select/multi checkable to cap then mute, others muted-as-disabled (distinct), pins reflect on the card. Commit.
...
### Task F2: post-functional UIX review (ALWAYS — standing gate)
- [ ] After the window functionally works + F1 is green, run a **post-functional, verified UIX review of the live Item Window** — this happens **no matter how clean the build** (Nathan, 2026-06-07). Drive Nathan's hands-on review + a UI pass (`impeccable` / `ui-ux-pro-max`) over every V1 interaction point (§11): header chrome, segmented bar, inspector rows, footer, toggle, drag, delete. Capture issues; fix as green commits; re-verify. Only after this passes do the docs close out. See `Guidelines/Review-Discipline.md`.

### Task F3: docs — Items.md + Paradigm-Decisions #15 + History
...
### Task F4: prose-level doc-sweep (final, per standard)
- [ ] Sweep ALL docs for stale Item-Window content — **not just grep** for terms (`archetype`, `LayoutArchetype`, `ItemWindowMode`, `commitItemEdits`, `editing: Bool`, "read-only stub", "popover"), **but READ the prose** of `Features/Items.md`, other `Features/*` that cross-reference the Item Window, `Guidelines/*`, `PommoraPRD.md`, `Framework.md`, `History.md`, and `CLAUDE.md` for outdated descriptions, and correct each against the shipped code (verify, don't assume). Commit `docs(items): prose-level Item-Window terminology + claims sweep`.

---

## Self-Review

- **Spec coverage:** §2 (no mode enum)→C1; §3 scope→D/E; §4 anatomy/sizing→D0; §4.1 header→D2; §4.2 chip-row+pooled cap→A5/A6/D4; §4.3 body+flush→B6/D3; §4.4 inspector (contexts/filled/surfaced/Add-Property/meta)→B8/D5; §4.5 footer (DetailFooterBar)→D6; §5 editors→D4/D5; §6 save model (.null at manager/debounce/tier-clear/title)→A2/B2/B3/B4/B6; §7 pane→E1/E2; §8 schema→A4; §9 reuse→file structure+C/D; §10 must-fixes→A1(rename)/A2(.null)/A3(icon-cap)/A4(property_layout)/A5(engine)/A6(promotedForField)/C1(editing+display)/E1(pane)/A8(placeholder)/D1(live index)/D6(footer)/F3(docs); §11 states→B2/B4/B6/B8/D5/D7/(VM lifecycle D1); §14 decisions→all; §15 blast radius→A1/A2/A7/C1/E1; §16 round-5 fixes→A2(audit)/A3/A5(pinnedTypes)/A6(paired)/C1(display path)/D1(live index)/D6(footer)/B-VM(concurrency). **Pre-execution amendments:** segmented bar→D4 (`PropertyFieldBar`); inline-commit model→B4/D1/D2/D5; inspector `(icon)(title)──(field)` no double-name→D5; §17.2 design-gate (gate EACH: D2/D4/D5/E1; ask-when-unsure; build-non-visual-first)→Conventions/D2/D4/D5/E1; `Design.md`→Conventions; per-task evidence-verify + report→adjust/PAUSE→Conventions; post-functional UIX review (always)→F2; prose doc-sweep→F4; Opus-4.8 implementers (Sonnet trivial)→Execution handoff.
...
**Model of execution — subagent-driven** (Nathan, 2026-06-07): a fresh subagent authors each task (`superpowers:subagent-driven-development`), handed the verified anchors + the relevant skill. Implementer agents run **Opus 4.8**, explicitly instructed to **avoid over-complicating mechanics** and to **leave zero design/coding ambiguity for a future agent** (document every decision inline; never force a guess). **Trivial/revisional fixes → Sonnet.** The orchestrator **hard-verifies every completion claim** (real background build with a non-zero executed test count — quirk #1; the actual diff; source/docs) before the green commit, then re-assesses the plan (adjust or PAUSE+ASK) before dispatching the next. Builder verification always via background `Agent` (quirk #13).
