...
>
> **Pre-execution amendments (2026-06-07, Nathan — folded before build):** (1) Property Field = **segmented-style row** (`PropertyFieldBar` CL asset; D4); (2) **§17.2 design-gated** — D2 header chrome signs off before build, other view tasks proceed on `Design.md`; (3) **inline text-field commit model** standard (Enter + blur + dismissal; idempotent; field-sized) — B4/D1/D2/D5; (4) **inspector row = `(icon)(title)──(field)`**, no double-name (D5); (5) `Design.md` added to conventions; (6) **per-task evidence-verification + report→adjust/PAUSE** protocol; (7) **F3 prose doc-sweep** added; (8) implementers = **Opus 4.8**, Sonnet for trivial.
>
...
- **Design-gate (§17.2) — gate EACH visual task** (D2 header, D4 property bar, D5 inspector row, E1 pane). Before building a step's visual, **ASK Nathan for the design when unsure** (no existing pattern fully dictates it). **To avoid stalling the plan, build the non-visual parts first** — each gated task's scaffolding, data wiring, and logic proceed; only the specific visual waits on Nathan, and smaller well-defined pieces never block on a gate. **A post-functional, verified UIX review of the working window always runs afterward** (Phase F2), no matter how clean the build.
