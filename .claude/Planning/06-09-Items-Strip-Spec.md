...
- SEAM-EDIT (lose item arm): `EntityKind` (`.item/.itemType/.itemCollection`), `Detail/ContentItem` (collapses to a `.page` wrapper → fold to `PageMeta`), `SidebarSelection` / `SelectionTag`, `Pages/AppGlobals` (item bridge + `presentItemAction`), `Nexus/NexusEnvironment` (item managers — **quirk #15:** every `@Environment(ItemTypeManager/ItemContentManager)` declarer must drop in the same commit or views SIGTRAP), `ViewSettings/*` panes (the **four** duplicate `SideKind` enums are **deleted**, not collapsed — single-case after the strip; `PropertyTypePickerPane` has no `SideKind`, only an item `@Environment` to drop), `NavDropdown/*`, `Ordering/OrderPersister`, `AtomicIO/NexusPaths`, `Nexus/NexusAdopter` (sidecar kinds + `Class` pass), `Settings/SettingsLabels` (drop `itemType`/`itemCollection`/`items` with `decodeIfPresent` tolerance).
