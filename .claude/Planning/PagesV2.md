...
**Architecture:** Page is the sole survivor; `Item*` code/index-tables/tests are deleted and every `item|page` shared seam collapses to page-only. The `EntityKind`/`SidebarSelection` enum-case removals are the compiler's exhaustiveness gate (P2). `{{ }}` is **retired entirely** (V7) — plain text, recognized nowhere; the chip visual survives only as a single dead-code design file staged in the Component Library. `Class` is dropped; the index rebuilds at schema v11 (no data migration).
...
3. **`{{ }}` is retired entirely (V7 — supersedes this decision's earlier retain-dormant form).** Connections use `[[` **only**; `{{` is plain text everywhere — no scanner regex, no tokenizer recognition, no render path, no gate, no autocomplete arm, no index rows. The chip **visual design** survives as exactly ONE dead-code design file staged in the Component Library (the Component-Library-as-source-of-design hard rule); ALL other chip-link machinery — MarkdownPM tokenizer/renderer/styler/selection-kind/config slots and app-side wiring — is deleted in P2.5. P0's rename-and-gate was a stepping stone; P2.5 finishes it off.
4. **`{{ }}` click — moot** (retired). Nothing to click; `{{` is plain text.
...
`P0` (MarkdownPM rename — stepping stone, finished off by P2.5) · `P1` (collapse leaf item arms) → `P2` (remove enum spine = compiler gate) → `P2.5` (retire `{{ }}` wholesale — V7) → `P4` (relocate survivors + `PageType.open_in`) → `P3` (delete item type bodies/managers/migration; `NexusEnvironment` keystone) → `P7` (schema v11) ; then `P5` (build `PagePreview` + open-in toggle) needs P3+P4 ; `P6` (settings/labels) alongside P5 ; `P8` (test reconciliation + new coverage) after P5/P6 — the lockstep rule pulls most surgery into P1–P7, and P8's new tests exercise P5's routing + P6's labels ; `P9` (band-3, last build) after P5 ; `P10` (doc-sweep + archival) after P5/P8.
...
### Phase P2.5 — Retire `{{ }}` wholesale (plain text again) — V7

**Goal:** `{{` carries ZERO special behavior anywhere — not a token, not a render, not a gate, not a trigger. Typing `{{Beta}}` in the editor produces the literal characters styled as plain body text. The chip **visual design** survives as exactly one dead-code design file staged in the Component Library; every other trace of the chip-link framework — MarkdownPM and app-side — is deleted. (Supersedes P0's rename-and-gate: that was the stepping stone, this is the destination.)

**Depends on:** P2 (enum spine gone; `ConnectionSyntax` already deleted).

- [ ] **MarkdownPM deletion sweep.** Live grep at execution: `grep -rln 'chipLink\|ChipLink\|{{' External/MarkdownPM/Sources/` (16 files at the V7 audit). Delete: the tokenizer's `{{` recognition (`MarkdownToken`/`MarkdownTokenizer`/`MarkdownDetection` chip arms), `Renderer/ChipLinkMetrics.swift` (fold its design values into the Component Library design file below), the styler chip arms (`MarkdownPMStyler` + `+Links`/`+TextStyling`), the `.chipLink` selection kind in `NativeTextViewSelectionTypes` + every coordinator arm (`NativeTextViewCoordinator` + `+Services`/`+TextDelegate`/`+Restyling`), the `MarkdownPMConfiguration` chip slots + the P0 gate field + its `.pommora()` factory param (the CR-1 threading dies here), the `MarkdownPMServices.chipLinks` slot + NoOp resolver, the `NativeTextViewWrapper`/`MarkdownTextLayoutFragment` chip arms. NO `default:` arms — delete dead arms.
- [ ] **App-side retirement.** `Pages/MarkdownEditorConfig.swift`: drop the chip wiring + gate threading. `Pages/PageEditorView.swift`: delete the `onItemLinkClick:` closure + drop `@Environment(\.itemConnectionResolver)` + its resolver arg; KEEP the `[[ ]]` `onLinkClick` path untouched as the sole page link path. Delete `Connections/ItemLinkOpener.swift` (CR-3, moved from P5 — the closure above is its only consumer). Decision #7 lands here: delete the `itemConnectionResolver` env key/@Entry; rename `pageConnectionResolver` → `ConnectionResolver` at injection + use sites. `Connections/AutoCompleteWiring.swift`: the `.chipLink` arms die with the `InlineSelectionKind` case; simplify `fragment`/`queryKind` signatures where the kind param becomes vestigial.
- [ ] **The one survivor — Component Library design file.** Rename `Properties/Chips/ItemChip.swift` to a page-native name (e.g. `Properties/Chips/ChipLink.swift`) and strip it to a pure visual primitive: the chip render (capsule/icon/title) + the design values folded in from `ChipLinkMetrics` — no engine hooks, no `{{` references, no resolver types. Keep/update its `ComponentLibraryView` entry so the design stays browsable (Component-Library-as-source-of-design hard rule). This file is intentionally dead code — design only.
- [ ] **Lockstep test surgery.** Delete/simplify the chip tests: `AutoCompleteWiringTests` `.chipLink` cases (added in P1 to pin suppression — the kind they reference dies here), `ConnectionConfigWiringTests` chip/gate assertions, and ALL chip/`{{` tests in `External/MarkdownPM/Tests/` (grep `chipLink\|ChipLink\|{{`). Tick off in P8's list.
- [ ] **Verify + commit.** Builder gate (build + `-only-testing:PommoraTests`) AND the retirement greps: `grep -rn 'chipLink\|ChipLink' Pommora/ External/MarkdownPM/` returns ONLY the design file + its Component Library entry; the tokenizer recognizes no `{{`. Manual spot-check: `{{Beta}}` typed in the editor renders literal. Commit: `refactor(pages): retire {{ }} wholesale — chip survives as Component Library design file`.

**Green gate:** build + tests green; retirement greps clean; `{{` is plain text in the live editor.
...
- [ ] **Delete `Validation/`:** `ItemValidator`, `ItemTypeValidator`, `ItemCollectionValidator`. (CR-3 resolved early: `ItemLinkOpener.swift` was already deleted in P2 — its consumer closure died with the enum spine; nothing remains here or in P5.)
- [ ] **`NexusEnvironment.swift` KEYSTONE STRIP (quirk #15):** remove stored props `itemTypeManager`/`itemContentManager`/`itemWindowPanelManager`; their construction/wiring/snapshot blocks; `AppGlobals.publish` item params; the `itemTypeMgr.loadAll` Task; the `.environment(...)` injects. (`itemConnectionResolver` already deleted in P2.5.)
- [x] **`ConnectionResolver.swift` (decision #7): ✓ landed in P2.5** — `itemConnectionResolver` @Entry deleted; `pageConnectionResolver` renamed `connectionResolver` (lowerCamelCase forced by @Entry convention); `PommoraConnectionResolver` de-kinded in P2.
...
> **P5 transcript reconciliation (Nathan's directive, 2026-06-10, pre-P10):** the implementation is reconciled with his "Final clarification" message from the planning session: **(a) the inspector defaults to OPEN** (that substitution is what justified removing pinned properties); **(b) unlocking REVEALS an "Open" affordance** beside the footer lock ("clicking an 'unlock' button reveals an 'open' button") — locked state shows only the lock, matching the frame; the context-menu "Open Page" remains as a secondary path. **(c) The close capsule uses ✕** (Nathan's ruling — the frame's square glyph was a placeholder).
...
**Goal — total erasure, three exceptions (amended by Nathan 2026-06-10).** Every doc must read **as if Items never existed** and Pages were always the only operational entity beside Agenda. Delete the Items feature doc entirely; reframe every other doc around Pages; drop all Item vocabulary, diagrams, schema rows, and wikilinks. **The ONLY three places "Items" may survive in the entire repo are:** (1) a **single one-line historical note in `CLAUDE.md`** (that an Items layer existed and was collapsed into Pages — pointer to History), (2) the **`History.md` record** (the collapse entry + the superseded ItemsV2 entries, append-only), and (3) a **dedicated historical section in `PommoraPRD.md`** keeping a DETAILED description of what Items WERE (Nathan's directive): the entity model (`.md` files with the capped body description inside Item Types, Item Collections, the Type/Set UI vocabulary), the Item Window floating panel, pinned properties, the `{{ }}` chip syntax, the `Class` stamp, the item index tables — and why/when it was collapsed into Pages (per-vault Compact/Window settings replacing the separate entity). Clearly framed as retrospective ("Items were…"), not current product. Anywhere else — Features, Architecture, Domain-Model, Framework, Guidelines, code comments, this plan's siblings — a surviving "Item" is a defect the final grep gate must catch. This is the last phase; it is not optional polish — it is the no-trace guarantee made real.
...
- [ ] **(Flagged by P3b — lands as a P8 test + a doc line here:)** legacy `_itemtype.json` folders are now UNPINNED under auto-tag/adoption — the old "sidecar wins" tests died with the deleted sidecar kind, and nothing asserts the new classify-as-sidecar-less behavior (IndexBuilder ignoring legacy item members IS pinned). P8 adds one test pinning what adoption does with a legacy item folder; the doc sweep then states the semantic in `Architecture.md`.
...
- [ ] **Heavy one-entity rewrites:** `Domain-Model.md`, `Architecture.md` (table count 11→8; schema v11 delete-and-rebuild; drop Class stamp + `_itemtype`/`_itemcollection` + `ItemTypeManager`/`ItemContentManager` rows), `PommoraPRD.md` (drop item DDL + recount; one operational entity in the CURRENT-product sections; **then ADD the sanctioned "What Items Were" historical section** — detailed retrospective per the amended goal above, the third permitted survivor), `Connections.md` (`[[` sole page-link path; `{{` retired — plain text, V7; chip visual = Component Library design file; drop Item Chip/Item Window), `Properties.md` (drop Item Type Settings + `_itemtype` schema-carrier; pinned_properties → Prospect), `Sidebar.md` (drop Items section; recast shape; band-3 note; rewrite the quirk-#8 mirror clause), `NavDropdown.md`.
...
- [ ] **`History.md`:** add a top collapse entry (survivor=Page; Item* deleted; Class dropped; `[[` declassed; `{{` retired to plain text with the chip visual preserved as a Component Library design file; `PageType.open_in`; `PagePreview` built / `PreviewWindow` eliminated; schema v10→v11; no migration); cross-note prior ItemsV2 entries as superseded.
...
- [ ] **Dead-code pass over seam-edited survivors (Nathan's gate, 2026-06-10: "no dead code is left").** The item-vocab greps are name-blind to orphaned helpers whose last caller died mid-migration (the class P2 caught with `AutoCompleteWiring.queryKind`). Walk every file the P1–P7 seam-edits touched but kept, and delete any helper/param/stored-property with zero remaining callers. The ONE sanctioned exception: the Component Library chip design file (intentionally dead by V7 decision).
...
- **Spec coverage:** every spec section maps to a phase — strip (P1–P3, P7, P8), `[[` declass + `{{` retirement (P0, P2, P2.5), `Class` drop (P3), `PagePreview` (Figma design captured in P5) + open-in + lock→open inspector-button transform (P4, P5), band-3 (P9), QuickCapture (spec-only, P10 doc), closeout (P8 tests, P10 docs). No pinned-property tasks (correctly absent — Prospect).
