...
**Status (2026-06-21):** Phases **A, B, C, D, E, H complete** (1,294 tests green); **F dropped**. The C-deferred magic-numbers → `PUI` + `.hoverFill()` and the `Core/Components/Domain/Features` grouping are done (plus a `Features/Toolbar` extraction + `NavDropdown`→`Navigation` rename). **H** closed by a simplification-review pass (Agenda title-sort DRY, a shared `InlineRenameFocus` responder-hop, a dropped `View.if`, a decorate-sort of `ViewSortComparator`). **D** built the `SidebarRow` primitive + re-skinned all 7 sidebar rows (eye-verified) — the drag-ghost `labelColor` patch is intentionally retained, not dissolved. **E** collapsed the SavedView scope→id switches + `currentView` hoist, shared `collisionSafeName`, and routed Page-CRUD through one scope-parameterized path. **G complete** — `ViewSurface` split into Cover/Rename/Delete extensions; GroupingPane's reusable rows extracted to shared `Components` (`SelectableOptionRow` + `LabeledToggleRow`, also adopted by the Sort/Layout panes); `NexusAdopter` + `PageTypeManager` **kept unified and internally cleaned** rather than split (splitting `PageTypeManager` would have regressed `private(set)` encapsulation on its observable state — recorded decision; `NexusAdopter` is a stateless enum, dead-code/duplication tidied; `PageTypeManager` error handling DRY'd through one `withPendingError` helper). **All phases A–E, G, H complete; F dropped — the refactoring program is done.** Separate deferred items (tracked in `Handoff.md`, not phases): `PropertyValue` datetime → `IndexDateFormat` (on-disk decode change, needs ratification) + the optional Page-CRUD caller migration onto the unified `in: parent:` API.
...
**Phase D — The `Row` Primitive** — **DONE (2026-06-21).**
A `Components`-style `SidebarRow` (icon · title · trailing menu, owning the inline-rename + selection state) now backs all 7 sidebar rows (4 leaf + 3 disclosure containers); chrome stays at row-file level via `.listRowBackground(SelectionChrome)` (quirks #8/#9 respected). Eye-verified live (`4c4c1d0` + `cb4e23d`). **The drag-ghost `labelColor` patch is KEPT, not dissolved** — `sidebarLabelColor` bakes the dark-mode `NSColor.labelColor` into a fixed color so the native `.onMove` drag-image (snapshotted in *light* appearance regardless of the dark window) doesn't flip the dragged row's text/icon to black. A real AppKit drag-snapshot workaround, not removable duplication.
...
**Phase E — DRY the Non-Divergent Families (TDD-Gated)** — **DONE — core (2026-06-21).**
The mechanical duplication is single-sourced: SavedView scope→typeID switches + `currentView` collapsed to `containerID`/`schemaTypeID` props on `ViewSettingsScope` + `resolvedActiveView` on `ActiveViewStore` (`468a0b6`, `ce7793e`); `collisionSafeName` hoisted to `Filesystem`, shared by both asset importers (`f7706d8`); Page-CRUD triplication routed through one `createPage`/`renamePage(in: parent:)` path + shared `deletePageCore`, all 9 public signatures preserved (`54464f0`). Scoped out: `schemaOptionValues` (didn't exist), the full asset-importer *merge* (`CoverAssetStore`/`AttachmentManager` legitimately diverge — only the shared helper extracted), fresh-token naming (declined — regresses filename legibility). Optional tail: migrate the CRUD callers onto the unified API + delete the thin shims.
