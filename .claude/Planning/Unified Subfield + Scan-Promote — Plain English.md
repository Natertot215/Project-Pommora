...
- **The scary-sounding "live word count in the preview" turned out to be cheap.** There was a worry that making the preview count words live would require rewiring a shared piece of plumbing that also feeds the tab-memory and thumbnail systems — a risky change for a small payoff. But it turns out the existing plumbing already ignores anything that isn't the page you're currently focused on. So the preview can just write into the *same* shared slot with its own page's name, and the other systems automatically look past it. No rewiring. (This only works because Pommora's rule is "most recent edit wins" — so even the weird case of the same page being open twice resolves itself by design.)
...
- **Phase B — the new-tab switch.** Add the List/Gallery switch to the footer's menu of possible items, make the new-tab screen show the footer, build its List view, and store the List/Gallery choice properly so it survives restarts (and so flipping it actually redraws the screen).
