...
- **Purpose:** Brainstorm → spec the **Gallery** view renderer for Collections and depth-1 Sets — the first of the five pending non-Table renderers (v0.6.0 opener) — plus its SurfacePM (view-embed) integration.
...
- **Success Criteria:** A Collection/Set view switched to Gallery draws cards (with or without covers), respects filter/group/sort, supports the Standard/Compact card layouts, and persists its layout options per-view.
...
- `src/shared/views.ts` — `VIEW_TYPES` includes `'gallery'` (:19); `SavedView` already carries `card_size?: 'small'|'medium'|'large'` (:114), `show_cover?`/`show_banner?`/`hide_page_icons?` (:115-117), `format?: 'standard'|'compact'` (:22-23, :125-127 — persisted, inert, documented as the *table density* knob), `collapsed_groups` (:112), `group_order`/`structural_order_mode`/`sub_group`/`ungrouped_placement` (:128-141). `mintVisibility` per-type switch (:287-303) is the mint seam.
- `Components/Detail/ViewSettings.tsx` — `IMPLEMENTED = new Set(['table'])` (:44) gates the type grid (:177-191); Gallery renders as an inert tile. The "Layout" leaf is `VisibilityList` (HiddenPane.tsx) + `LayoutToggles.tsx` (three Table-only switches, gated `view.type === 'table'` at :272). **No LayoutPane component exists** — "LayoutPane" for Gallery means new content in this leaf.
- `Detail/ContainerView.tsx:12-18` — hard-mounts `<TableView>`; **no renderer switch exists**. `ViewEmbedBlock.tsx:479` is the second hard mount — the switch must be a shared seam both consume (A-2).
- `Detail/Views/pipeline/` — `resolveView.ts:15-47` is renderer-agnostic (columns → filter → group → sort → `{columns, groups}`); `ResolvedGroup` (`shared/types.ts:505-513`) carries `items`, nested `children`, `isCollapsed`, `bucket`. `resolveColumns` (columns.ts:67) merges `property_order` + `hidden_properties` + schema — the card-field order source. **No sort-by-location exists** (sort.ts — per-property criteria only; Sorting pane offers Title/Modified/sortable properties).
- `NavWindow/NavGallery.tsx` + `navGallery.css` — the card precedent: pure CSS grid `repeat(auto-fit, minmax(var(--card-min), 1fr))`, knobs `--card-min`/`--card-gap-*`/`--cover-zoom`/`--thumb-share`, container-query typography (`5.5cqi` clamp), `hover-pop`, aspect-ratio card, thumb → divider → title+crumbs anatomy. **No runtime size slider exists anywhere** (`--card-min` is a static knob); range-input precedent: `MarkdownPM/ZoomSlider.tsx`.
- `styles.css:89-132` — the two inset regimes: `--content-inset`/`--gutter` (standard views) vs `--surface-inset`/`--surface-inset-right` (+ banner variants) — the block-surface regime NavView's gallery rides (`navView.css:64`); NavView's *list* deliberately rides `--content-inset` instead.
...
- **A-1:** [confirmed] Gallery currently exists as a blank, non-active choice in the ViewPane/ViewSettings type picker; this work activates it. *(Decision: Nathan. Evidence: `ViewSettings.tsx:44` — `IMPLEMENTED = new Set(['table'])`; the type grid renders Gallery as an inert tile.)*
- **A-2:** [confirmed] SurfacePM scope: Gallery renders inside the existing view embeds with no dedicated embed features — but two concrete obligations make that true: **(1)** TableView is hard-mounted in TWO places (`ContainerView.tsx:15` *and* `ViewEmbedBlock.tsx:479`) — the renderer switch is a shared `view.type` seam both consume, not a ContainerView-only branch; **(2)** embeds scale by the CSS `zoom` law (`Table.css:38` — `zoom: calc(var(--zoom) * var(--block-zoom, 1))`), which the gallery root must carry too or it renders full-size inside embeds. *(Decision: Nathan on scope; the obligations from adversarial review, verified.)*
...
- **D-2:** [confirmed] Image-bearing and imageless cards each have their own height treatment; one DRY `GalleryView.css` home for the CSS. *(Decision: Nathan.)*
- **D-3:** [confirmed] Card resize via a **LayoutPane slider**: default 1.00×, range 0.50×–1.50×, reflow like gallery-mode NavViews. *(Decision: Nathan. Evidence: no runtime size slider exists anywhere yet — `--card-min` is a static knob; range-input precedent `MarkdownPM/ZoomSlider.tsx`.)*
- **D-4:** [confirmed] `card_size` persistence reshapes from `'small'|'medium'|'large'` to a number (the slider factor), legacy strings mapped on read. **The codec changes in the same task:** `card_size` accepts `number | legacy-enum` with `.catch(undefined)` (its optional siblings' pattern) — today it's a bare `z.enum` and `parseViews` (`readNexus.ts`) silently **drops any view that fails to decode**, so a numeric write against the old codec deletes the view on next read. `card_size` has no other reader today (grep-verified), so the codec is the only break point. *(Decision: Nathan; codec requirement from adversarial review, verified.)*
- **D-5:** [confirmed] The Standard/Compact card layout reuses the existing `SavedView.format` field with per-renderer meaning (table: density; gallery: card layout) — no new key. *(Decision: Nathan.)*
- **D-6:** [confirmed] **Title Wrapping (on/off)** is a LayoutPane setting for the cards — on lets the title wrap; off keeps it single-line via the shared `OverflowScroll` (NavGallery's title treatment). *(Decision: Nathan.)*
- **D-7:** [confirmed] Gallery gets **its own card component**, with a precise DRY line: the **title-area, aspect ratio, non-image borders, and card-to-card padding are DRY-ed** (shared with NavGallery's card CSS); the **title/property inner area starts as a copy and is allowed to diverge**. Borrowed mechanics: the auto-fit grid, the `--card-min` knob family, container-query type, `hover-pop`, the shared tokens. *(Decision: Nathan.)*

#### E — Grouping, Location & Sorting

- **E-1:** [confirmed] **No sub-grouping** in Gallery — nested disclosure group headings look wrong in a card grid. Sub-*sort* is fine. *(Decision: Nathan.)*
- **E-2:** [confirmed] Location grouping renders **flattened**: top-level disclosure per location (like tables' bands) but no indenting of cards; sub-set nesting never indents cards. Planning detail: `structural()` emits a *nested* tree (`ResolvedGroup.children`) — the gallery defines the flatten traversal (descendants' pages roll up under the top-level band); structural bands never hide empties (`hide_empty_groups` is property-only), so an empty Set still renders its band. *(Decision: Nathan; traversal note from adversarial review.)*
...
- **E-5:** [confirmed] **No heading columns** in Gallery. *(Decision: Nathan — column mechanism is table-only anyway.)*
...
- **E-8:** [confirmed] Group By Location's **Order: Custom / Location** semantics copy from tables — Location mirrors the filesystem order (view `group_order` preserved-but-ignored for the flip back), Custom reads the view-owned `group_order`. *(Decision: Nathan. Evidence: `structural_order_mode`, `views.ts:44-45`, gated in `resolveView.ts:31-32` — renderer-agnostic, so the pipeline half is free.)* Known limit, deliberate: with band drag deferred (I-7), Custom order can't be *authored* from inside the gallery — it renders whatever `group_order` holds (authored via the table, or derived) until the band kit arrives.

#### F — Set Cards

- **F-1:** [confirmed] LayoutPane toggle **"Display Set Cards"**: the gallery's top row shows a larger card per Set (or per 1st-level sub-Set in a Set view). *(Decision: Nathan.)*
...
- **F-3:** [confirmed] Default gallery layout: Set Cards row on top, then per-location disclosures each holding their page cards. *(Decision: Nathan.)*
...
- **H-1:** [confirmed] Gallery does **not** use standard view insets (no gutter drag-lines needed); it uses the block-surface inset regime shared by gallery-mode NavView + Block Surfaces (`--surface-inset*`, `styles.css`; `navView.css` consumption). *(Decision: Nathan.)*
...
- **I-2:** [confirmed] Gallery gets the heading hover-"+" on structural headings, inactive on property buckets — exactly the table's rule. *(Decision: Nathan.)*
...
- **I-7:** [confirmed] **Gallery heading chrome is collapse + "+" only** — band collapse persists via `collapsed_groups`; `hide_empty_groups`, `ungrouped_placement`, and `group_order` carry over from the pipeline. Inline header rename does NOT apply (renames live in the sidebar/table); band drag, the native header menu, and the fuller band kit are table territory → follow-up. *(Decision: Nathan on rename; the narrowing is the joint call.)*
...
- **J-1:** [confirmed] `Views.md` — "only Table draws" / the Pending renderer list / the Renderers section go false on ship; Gallery gets its own feature doc, `Views.md` re-pointed.
- **J-2:** [confirmed] `ViewSettings.tsx` — `IMPLEMENTED` set, the Table-only gating on LayoutToggles (`:272`) and the Format footer row all grow gallery branches. **`setType` is the entry seam into gallery** — every mint path hardcodes table (`mintBase`/`mintNewView`/`mintDefaultView`), so a view becomes a gallery only via the type grid, preserving its existing visible-property set; no `mintVisibility` gallery case (it would be dead code).
- **J-3:** [confirmed] The Grouping pane is table-only by ratified design — Gallery gets its own Grouping surface (no Sub-Group row, per E-1); the Sorting pane gains the Location **entry** (the flatten mode, not a `SortCriterion` — E-4).
- **J-4:** [confirmed] DRY duties: chips/`OverflowScroll`/`hover-pop`/`PaneSlider`/`PropertyPicker`/`SubfieldBreadcrumb` are reused, never paralleled; `GalleryView.css` is the one CSS home (D-2), knobs aliased to design-system tokens.
- **J-5:** [confirmed] The `views.ts` header's Swift-parity claim ("two deliberate React-ahead supersets") goes stale — the gallery keys join its React-ahead superset list when they land.

#### K — Build Sequence & Options-Pane Spec

- **K-1:** [confirmed] **Visuals before mechanics, on a branch.** All code lands on a `gallery-view` branch, created before any code. Build order: (1) wire Gallery as a selectable type (ViewPane type grid + the SettingsPane Layout door) with its options pane, (2) get the card visuals right under live driving, (3) only then the mechanical renderer work (grouping, drag, editing). *(Decision: Nathan.)*
- **K-2:** [confirmed] The gallery options render **under the 3×2 view-selection grid** in ViewSettings — in BOTH doors: the full door directly under the grid (with the Group/Sort rows following, K-4), the flat door in the body (the table-toggles slot). *(Decision: Nathan.)*
- **K-4:** [confirmed] **Full-door structure for gallery:** type grid → the K-2 options block (the options are the gallery's Layout — no separate Layout row) → then **Group and Sort leaf rows exactly like tables**, sliding into the gallery Grouping variant (same logic, no Sub-Group row) and the Sorting pane. The one `view.type === 'table'` gate at `ViewSettings.tsx:272` splits per-type. Filter follows the table's current treatment (the leaf is a blank placeholder pending the pane redesign). *(Decision: Nathan — "Group + Sort stay separate like tables; same logic just no sub-groups.")*
- The gallery options block:

  ```
  Card Banner:    (Cover / Preview / None)   ‹picker›
  Card Style:     (Standard / Compact)       ‹picker›
  Hide Location:  Switch
  Wrap Titles:    Switch
  Hide Icons:     Switch
  Set Cards:      Switch
  ───────────────────────
  Scale:          (value)                    ‹slider row›
  ───────────────────────
  ```

  Field mapping: Card Banner → the new per-view banner-mode key (B-1) · Card Style → `format` (D-5) · Hide Location → the location-footing key (E-3) · Wrap Titles → the title-wrap key (D-6) · Hide Icons → `hide_page_icons` · Set Cards → the Display-Set-Cards key (F-1) · Scale → `card_size` as the slider factor (D-3/D-4), in the divider-separated footer region (the table Format row's slot). *(Decision: Nathan.)*

  Codec discipline for the new keys: the banner-mode enum ships `.optional().catch(undefined)` (the `format`/`structural_order_mode` pattern — a bare enum is the D-4 view-drop landmine); the new booleans follow the existing bare `z.boolean().optional()` convention. All land in the same task as the `card_size` reshape. The `views.ts` Swift-parity header adds the gallery keys to its React-ahead superset list (→ J-5). `show_cover` (a zero-consumer reserved bool) is superseded by `card_banner` and retires with it; `show_banner` stays reserved.

### Core (must-have)

- **The renderer seam:** a shared `view.type` switch consumed by BOTH TableView mounts — `ContainerView.tsx` and `ViewEmbedBlock.tsx` (A-2); `'gallery'` joins `IMPLEMENTED` in ViewSettings, with `setType` as the entry seam (J-2). The pipeline (`resolveView`) is consumed verbatim; the gallery root carries the embed `zoom` law.
- **GalleryView + its own card component** (D-7): image band + title + properties + optional breadcrumb footing, NavGallery's grid mechanics (`auto-fit`/`minmax`, container-query type, `hover-pop`), all CSS in one `GalleryView.css`, on the `--surface-inset` regime (H-1).
...
- **Gallery's own Grouping-pane variant** (no Sub-Group row) + the **Location entry** in the Sorting pane driving the flatten mode (E-4, J-3).
- **LayoutPane (gallery content), K-2's six-row set exactly:** Card Banner picker (B-1) · Card Style Standard/Compact (D-5) · Hide Location (standing switch, grouping-independent, E-3) · Wrap Titles (D-6) · Hide Icons (`hide_page_icons`) · Set Cards (F-1) · the Scale slider footer (`card_size` → number, D-3/D-4).
...
- **Full band chrome on gallery headings** — band drag, native header menu, inline rename parity with tables, if ever wanted.

#### Out of Scope (won't do)

- Sub-grouping in Gallery (E-1). Heading columns (E-5).
