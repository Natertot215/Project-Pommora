...
- [ ] **Step 3:** `TopicManager.swift` — `createTopic(name:parents:icon:)` → `createTopic(name:icon:)` (Topic init loses `parents:`); `renameTopic`'s validate call (:141-145) drops `parents`/`context`. The `contextProvider()` references disappear WITH those two validate-call rewrites (its only remaining uses after P1) — then delete the stored property (:14) + init param (:27-30), update the `TopicManager(...)` construction in `NexusEnvironment.swift`, AND `grep -rn "TopicManager(" Pommora/PommoraTests` to drop the `contextProvider:` argument from every test construction (the `TopicManagerTests` setup helper is one definition fixing all its call sites). The locked `@MainActor @escaping () -> NexusContext` pattern simply has one fewer consumer — `NexusContext` itself stays for page validation.
...
- [ ] **Step 5:** Tests: `SpaceFileTests` — title derives from folder name; `SpaceManagerTests` — assert `.nexus/spaces/<Title>/_space.json` paths in create/rename/delete/loadExisting fixtures; `NexusPathsTests.swift:160` — replace the `spaceFileURL` path test with `spaceFolderURL`/`spaceMetadataURL` coverage; `RenameAtomicityTests` — update its Space fixtures to the folder layout; `IndexBuilderTests` — update any `.space.json` fixture writes.
...
**ONE atomic commit + a verification gate — NOT split (third-pass finding).** Three enums with a `space` case are `String`-raw with IMPLICIT raw values — `EntityKind` (IndexQuery.swift:598-600, persisted via state.json refs + looked up through `kindTableMap`), `EntityStateRef`'s kind (NavDropdown, persisted in state.json), and `SidebarToast.ErrorSource` (:26-28, UI-only). Renaming `case space`→`case area` silently changes the STRING `"space"`→`"area"`, so a symbols-only commit desyncs `kindTableMap` keys and `RelationTargetKind` tier strings — compiles green, but tier-1 lookups return nil and ~10-20 tests go red until the string side lands. Symbol renames, file renames, raw-value string updates, labels, on-disk tokens, and the schema v13 bump therefore land in a SINGLE commit.
...
- [ ] **Step 3:** String side of the raw-value change: `kindTableMap` key `"space"`→`"area"` (IndexQuery.swift:13-26); `RelationTargetKind` tier-1 string `"space"`→`"area"` (:20-22); VERIFY no explicit `= "space"` raw assignment remains on any renamed enum; sweep every literal `"space"` comparison found by `grep -rn '"space"' Pommora/Pommora --include="*.swift"` (excluding IconCatalog).
