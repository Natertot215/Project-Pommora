...
3. **State-dependent root rows + Duplicate**: an unconfigured markdown block shows Link View › / Link Page ›; a configured embed shows **Source ›** instead — re-pick the page (search pane) or the view source; the type is fixed for the tile's life, so no embed→markdown or embed→embed conversion op exists (G-14 — the shipped native Type ▸ scaffolding retires here). **Duplicate** (G-16, confirmed v1): copies the tile — markdown mints a copy of its `.md`, embeds copy the payload — landing directly below via the attach logic.
