...
2. **The embed view scope** — the C-1 seam, and it gates the WRITES, not just the reads: an `Embeds/ViewEmbedScope` React context carrying `{ source, view, persistConfig }` (`persistConfig`, not `persistView` — TableView owns a local `persistView` with the opposite target, `TableView.tsx:418`). **Reads:** `useActiveView` gains a context-first read (four consumers: `TableView:151`, `ViewDropdown:44`, `PropertiesPane:166`, `HiddenPane:194`), and the resolution intercept lands at **`pickView`** (`TableView.tsx:92`; `SettingsPane.tsx:168-172` calls it three times to prop ViewSettings/GroupingPane/SortingPane) — swapping only the id at the direct slot reads (`ViewPane.tsx:68`, `SettingsPane.tsx:74`) would still resolve against `source.views`, where a payload config doesn't live. **Writes:** every `saveViewAdopting`/`views.save` call site routes `scope ? persistConfig : source` — the full sweep: `TableView.tsx:408/421` (column widths/styles/alignments + collapsed groups — config-drags that fire from everyday gestures inside the embed body), `ViewPane.tsx:87/106/188`, `ViewSettings.tsx:85/97`, `GroupingPane.tsx:109`, `SortingPane.tsx:129`, `PropertiesPane.tsx:271`, `HiddenPane.tsx:116/152`, `LayoutToggles.tsx:27`. **`activeViews.set` is scope-gated** to a payload `active` write — embeds never touch the per-machine slot (`viewMint.ts:51` and the switcher path `store.ts:363` both reach it). `[re-ground every listed call site at task start]` Outside a scope everything behaves exactly as today.
3. **`Blocks/ViewEmbedBlock.tsx`**: resolves `source_id` against the tree (Collection or depth-1 Set; dead ref → inert, E-2 — a deleted source *view* can't orphan the tile, the config is copied), renders the **H-5 header structurally** — optional title row (display title per G-6) over the switcher row with the config affordance right (opening ViewSettings INSIDE the scope); the switcher slot holds the single view for now (tabs/dropdown visuals ride the post-Task-6 design pass) — then TableView under the provider at the fixed embed zoom (G-10; the `--zoom` knob — resize is viewport-only, H-10). Never a banner (G-4).
