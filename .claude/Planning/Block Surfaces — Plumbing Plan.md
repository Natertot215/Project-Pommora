...
2. **The embed view scope** — the C-1 seam, and it gates by EFFECT, not by function name: an `Embeds/ViewEmbedScope` React context carrying `{ source, view, persistConfig }` (`persistConfig`, not `persistView` — TableView owns a local `persistView` with the opposite target, `TableView.tsx:418`). **Reads:** the scope is read where React can read it — `useActiveView` goes context-first (four consumers: `TableView:151`, `ViewDropdown:44`, `PropertiesPane:166`, `HiddenPane:194`); components calling the plain `pickView` helper (`TableView.tsx:92`; `SettingsPane.tsx:168-172` props ViewSettings/GroupingPane/SortingPane with it) read the scope in their own body and pass `scope.view` — `pickView` is not a hook and resolves only `source.views`, where a payload config doesn't live. **Writes — four IPC families, every site routing `scope ? payload : source`:**
   ...
   - **Container config** (`container.configure` for `open_in`/`view_style`/`view_button`: `SettingsPane.tsx:92`, `ViewDropdown.tsx:59`): per-tile payload overrides in scope (B-8/H-5), never the source's stored config.
...
3. **`Blocks/ViewEmbedBlock.tsx`**: resolves `source_id` against the tree (Collection or depth-1 Set; dead ref → inert, E-2 — a deleted source *view* can't orphan the tile, the config is copied), renders the **H-5 header structurally** — optional title row (display title per G-6) over the switcher row with the config affordance right. The affordance opens the **SettingsPane dropdown** (H-5: same surface, header referencing the *view*) — and SettingsPane's entire `node` derivation goes scope-first: it takes no props and resolves its target container from the global `selection` (`SettingsPane.tsx:57-75`), so scoping only the view id would leave rename/schema/Open In/Visibility pointed at whatever the sidebar happens to have selected — the wrong container entirely. The switcher slot holds the single view for now (tabs/dropdown visuals ride the post-Task-6 design pass) — then TableView under the provider at the fixed embed zoom (G-10; the `--zoom` knob — resize is viewport-only, H-10). Never a banner (G-4).
