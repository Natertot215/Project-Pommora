...
**Open unknowns + findings (why this whole area is flagged in flux):**
- **Inspector muting — native behavior we regressed (cause identified, fix unverified).** A standalone `ToolbarItem(.primaryAction)` inspector toggle, with the window-toolbar background present, is adopted + muted by the inspector automatically — the native macOS behavior (baseline: commit `454d153`, before any toolbar glass). Muting regressed once the toggle was folded into a glass-grouped cluster and `.toolbarBackground(.hidden, for: .windowToolbar)` was added. Strongly suspected, **not yet re-verified** (a native-restore attempt was reverted before it could be tested live): muting needs the toggle STANDALONE *and* the toolbar background UN-hidden. Next attempt: restore the `454d153` standalone-toggle pattern and confirm muting on-screen before re-layering the other controls.
...
- **Banner bleed ↔ inspector muting are coupled — mutually exclusive.** The container banner bleeds edge-to-edge under the toolbar via `backgroundExtensionEffect()`, which needs `.toolbarBackground(.hidden)` to show through. That same hidden background is what breaks native inspector muting (the inspector mutes adopted items *into* the toolbar background). So under-toolbar banner bleed and native inspector muting **cannot both hold** through `.toolbarBackground(.hidden)` — the next design picks one.
