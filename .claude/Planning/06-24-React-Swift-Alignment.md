...
- **`modified_at` is no longer a hard requirement (Swift, pending builder+commit on `main`):** the Swift cross-open of a React nexus was failing because `PageCollection.init(from:)` (and 7 peer models) hard-required `modified_at` (`try c.decode(Date.self)`), so a React-created/converted sidecar lacking it threw `keyNotFound` → the whole collection dropped → empty sidebar + "data couldn't be read because it is missing." Fix: `modified_at` decodes optional with a file-mtime fallback injected via `AtomicJSON`'s new `CodingUserInfoKey.fileModificationDate`, across all 8 models (PageCollection/PageSet/Area/Topic/Project/AgendaTask/AgendaEvent/Settings); still WRITTEN (encode untouched), just not required. New `ModifiedAtFallbackTests`. **This is the unblock for the Swift reverse-open.**
...
- After `modified_at` lands + a Swift rebuild: open a nexus React touched in **Swift** → collections populate, NO reseed/migration churn. (Prefer a *clean* React-created nexus over the frankenstein `~/test-2tier`, whose Set sidecars still carry the dead `type_id` field + `schema_version: 0`.)
