...
- **Accent palette mismatch (decision at impl):** Swift `accent_color` ∈ exactly `{red,orange,yellow,green,blue,purple,pink,gray}` (8), absent = system. React **today reads the wrong key** (`settings.accent`, [readNexus.ts:239](React/src/main/readNexus.ts#L239)) **and a different 10-color renderer palette** (`ACCENT_COLORS` has `lightBlue/cyan/lavender/grey`, `DEFAULT_ACCENT='lavender'`). For no-conflict the **persisted** accent vocabulary must be Swift's 8 (+ system) — the renderer may still map each to any hex, but it can't persist a value Swift drops on decode. → narrow the on-disk/settable set to Swift's 8, repoint the read to `accent_color`, and pick a `DEFAULT_ACCENT` ∈ Swift's 8 (this is a paradigm-solidifying on-disk vocabulary choice — surface options to Nathan at T11 start).
