...
- **D-2:** [confirmed] **Scope single-preview only** — one page-preview window; a new summon overtakes (swaps to the new origin's tab set, H-3). The not-foreclose seam for the multi-preview A-B is **already satisfied and stays this small**: the window reads its target as a prop and the store holds one window's state (the target slot today, the preview slice once tabs land) — pluralizing that one holding later is the entire upgrade. No plural rendering, z-arbitration, or N-window machinery ships now.
...
- **D-6:** [confirmed] Tree-push reconciliation, by page id: `applyTree` gains a preview branch — **re-path on rename/move; on delete, close the preview** (the pending write dies on the crud dead-path refusal — never written anywhere wrong). Accepted window: an **external** rename mid-edit loses at most the debounce-window keystrokes the same way (refusal, not misdirection) — unavoidable, since the old path is already dead when the push arrives; the **self-rename** flow (I-13) must flush the pending body *before* renaming (plan item). Shipped as a shell guard for `previewTarget`; the tab phases extend the same reconcile to the **active origin's tab set** on live pushes, and to **cold per-origin sets lazily at summon** (a `reconcileTabs`-equivalent runs on every restore before the set is shown).

- **D-9:** [confirmed] **Nexus adopt closes the preview** — `openVia` clears `previewTarget` at adopt START and **awaits the preview's registered flush** (the `pageFlush` pattern: PageEmbed's opt-in `registerFlush` prop → `flushPreviewPage()`), because the exit presence defers the unmount past the adopt — the unmount flush alone would bind the NEW root. The post-adopt state clear includes the field. Shipped as a shell guard.
