...
- `06-18-File-Watcher.md` (spec, **V4**) — the FSEvents file watcher: external + internal changes propagate live across **all entity kinds**, authority by recency (origin-blind), identity by **stamped ULID** (stamp-on-first-sight, a ratified paradigm decision → log to `History.md` on ratification). **V4 pending final verification** (V1–V3 failed on a content-diff echo filter, prior-state living in the index not memory, path-derived ids defeating rename-tracking, and the index database self-churning inside the watched root). Mechanism: one watcher-owned last-seen-mtime map (no write instrumentation; index-db excluded at intake), coarse per-scope reload + loader index-sync, surgical refresh for the open editor; broken-link UI and FTS5 out of scope.
