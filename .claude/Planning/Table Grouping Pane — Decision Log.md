...
#### E — Model & Persistence
- **E-1:** [confirmed] The structural-only settings live VIEW-LEVEL on the SavedView — `structural_order_mode?: 'custom' | 'location'` and `sub_group?: { property_id, order_mode, order?, date_granularity? }` — beside `group_order`, which is the binding precedent: it lives view-level for the same two reasons (the structural GroupConfig decoder drops extra fields, and the view has ONE `group` slot that a Group By switch replaces wholesale). `GroupConfig` and `decodeGroupConfig` stay completely untouched. Absent fields read as defaults (custom + location sub-group = today's behavior). Rejected alternatives → Considered & Rejected.
...
- **E-3:** [confirmed] A deleted/unmappable grouped property falls back to structural (pipeline already does); the pane displays Location in that state. `sub_group` and `structural_order_mode` survive Group By switches AUTOMATICALLY — the view-level home means switching to a property never touches them; flip back to Location and they're still there. Both are meaningful — and reachable in the pane — ONLY under location grouping; property grouping never offers them.
...
- Model: four view-level SavedView fields (`structural_order_mode` · `sub_group` · `ungrouped_placement` · `date_separator`), lenient decode to today's defaults; GroupConfig untouched.
...
- **Extending the structural GroupConfig variant in place** (`order_mode`/`sub_group` on the config object) — can't honor E-3: the view has one `group` field, so switching Group By to a property replaces the object and destroys the "preserved" settings; the property config can't carry them either (its own `order_mode` key collides). `group_order` already went view-level for exactly this; these fields follow it.
