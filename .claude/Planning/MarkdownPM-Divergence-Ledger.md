...
- **Phase 4:** the AST reconstruction reproduces legacy byte-for-byte for non-nested cases (`*a*`, `**b**`, `***c***`, intra-word asterisk) AND for nested-adjacent + unbalanced cases (`***bold** then italic*`, `**a*`, etc. — via the Phase-4-review clamped-bounds fix above). The rule-of-3 cases are NOT a reconstruction target — recon proved legacy's overlapping runs (`ruleOfThree_a` bold(0,11)+italic(5,10); `ruleOfThree_b` italic(0,10)+bold(4,11)) cannot exist in Apple's AST (one clean node) → they are re-pinned divergences (D-EMPH-3). Multi-backtick (D-CODE-1) + cross-line (D-EMPH-4) likewise logged. Delete `MarkdownTokenizer+Emphasis.swift` (173 lines) only after the swap commit lands green (parser goes dead first, then a terminal deletion commit).

**Phase-4 review fixes (adversarial review, 2026-06-03):**

- **Emphasis marker reconstruction was width-subtraction at the node's ABSOLUTE boundaries** (`appleEmphasisTokens` / `appendToken`) — review findings #1/#2. This is FALSE when an inner Strong/Emphasis ABUTS the outer emphasis's delimiter run: Apple reports the inner node's `.range` starting ON a delimiter char owned by the OUTER node. `***bold** then italic*` → inner Strong (0,9) starts at index 0 (the outer italic's `*`); width-2 subtraction yielded bold content (2,5)=`*bold` (a literal `*` STYLED + NOT hidden → a stray styled `*` mid-text). Same for `*italic then **bold***` (`bold*`), `***Warning:** read this carefully*` (`*Warning:`), and unbalanced `**a*`/`*a**` (stray edge `*`). NOT one of D-EMPH-1..4 (those are underscore / inside-code / rule-of-3 OVERLAP / cross-line — categorically different from these clean CommonMark nested sub-spans). **FIXED:** content boundaries now derive from TWO clamped signals — the node's own `.range` (reliable for its own delimiters) AND the child-span union (reliable when a child absorbed a delimiter), taking the tighter bound on each side (`max(childLower, full.start+width)` / `min(childUpper, full.end−width)`); markers are then exactly `width` chars adjacent to that content. Reproduces the retired stack parser byte-for-byte for every abutting/unbalanced ASTERISK case (oracle: `d2df39c`'s `MarkdownTokenizer+Emphasis.swift`); underscore equivalents pinned to the analogous geometry. NO residual — D-EMPH-5 NOT needed (every probed case fixed robustly).
- **Same-type identical-range nesting** (`****d****` → `Strong(Strong(Text))`) — review finding #4. The collapse only matched the OPPOSITE type, so it double-emitted two `.bold` tokens (harmless under the idempotent OR-merge, but sloppy). **FIXED:** added a same-type identical-range branch that DESCENDS without re-emitting; the inner Strong emits the single `.bold` over `d` (content (4,1), markers `**`/`**`). Renders bold, one token.
- **Invariant pinned:** `AppleEmphasisTokensTests` gains 7 cases (`***bold** then italic*`, `*italic then **bold***`, `***Warning:** read this carefully*`, `**a*`, `*a**`, `****d****`, underscore analogue) asserting exact markerRanges + contentRange, plus `assertNoStrayStyledDelimiter` — every `*`/`_` that lands in any token's contentRange MUST also be covered by some token's markerRange (styled ⇒ hidden), so the stray-delimiter regression can never recur. Package leg green: 109 tests / 10 suites / 0 failures (102 baseline + 7).

**Phase-4 recon (probe-verified against swift-markdown 0.8.0, 2026-06-02):**

- **D4.1-a CONFIRMED:** Emphasis/Strong/InlineCode/Link node ranges are **delimiter-inclusive** (marker width 1=Emphasis, 2=Strong). NOTE: naive width-subtraction at the node's absolute boundaries is INVALID for nested-adjacent emphasis — superseded by the Phase-4-review fix above (content from clamped node-range ∩ child-span). Converter to reuse is `SourceRangeConverter.nsRange(from:in:lineIndex:)` (`AppleASTSupplementalStyler.swift:277`), handles multi-line ranges.
