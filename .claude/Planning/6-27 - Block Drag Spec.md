...
2. **Move primitive** *(done)* — broaden the move to a plain range (`BlockRange`); add the blank-aware `blockMoveChanges` sibling (a block owns its trailing blank, so single-blank separation survives a move at mid-doc / EOF / top). List-drag's `moveBlockChanges` is untouched — its tests are the regression gate. Pure + tested. → review. *(Re-scope: the boundary collector moved to Phase 3 — a heading owns its section, so drop targets are siblings-at-a-level, which is gesture-coupled, not a flat list to pre-build here.)*
3. **Gesture + drop targets** — generalize listDrag's gesture to block handles; build the **sibling drop-target structure** (targets are siblings at a level: blocks within a heading's section, sections among themselves) + the coords mapping; rail `::before` grips for paragraph/code/list-item-1/callout/quote; the no-op rule + hover-reveal JS bridge. Top-level reorder working for non-heading blocks. → review (+ UIX).
