...
- **DRY for the other container views (Nathan, at pickup):** the property-editing surfaces live in `Detail/Views/PropertyEditing/` (sibling of `pipeline/` — the shared layer beside `Table/`) with table-agnostic props `(def, value, onCommit, anchor)` — no row/column concepts inside. Gallery/List/Cards mount the same components later; `Cell.tsx` is just the first consumer.
- **Visuals are token compositions only (Nathan, at pickup):** check `design-system/tokens` + the existing component library (`PickerMenu`, `Chip`/`chip.css.ts`, `Switch`, `Icon`) before rendering anything; NO net-new variants or values without Nathan's sign-off. **He's at his desk — stop and confirm any design fork that surfaces mid-execution.**
...
**Create:**
- `React/src/shared/columnStyles.ts` — the `ColumnStyle` type + zod (`look`, `date_format`, `time_format`, `number_format`) + `defaultStyleFor(declaredType)` (string-keyed — `shared/` can't import the renderer pipeline's `declaredType`).
- `React/src/shared/columnStyles.test.ts`
- `React/src/renderer/src/Detail/Views/Table/columnStyles.ts` + `.test.ts` — the schema-aware `styleFor(columnId, schema, view)` resolver (mirrors `columnAlign.ts`: `declaredType` + per-key merge of the view entry over the defaults).
- `React/src/renderer/src/Detail/Views/PropertyEditing/formatValue.ts` — `formatDate(iso, dateFormat, timeFormat)` + `formatNumber(n, numberFormat)` (pure; ports Swift's four date formats, ordinal-day, 12/24h, integer/decimal/percent/currency).
- `React/src/renderer/src/Detail/Views/PropertyEditing/formatValue.test.ts`
- `React/src/renderer/src/Detail/Views/PropertyEditing/statusCycle.ts` — `statusGroupOf(value, def): StatusGroupId | undefined` (created Task 3 — each look maps the fixed group id to its glyph) + `nextCycleValue(current, def)` (added Task 6) for the checkbox-status 3-group cycle (empty→minus→check→empty, first-in-order option, empty groups skipped).
- `React/src/renderer/src/Detail/Views/PropertyEditing/statusCycle.test.ts`
- `React/src/renderer/src/Detail/Views/PropertyEditing/PropertyPicker.tsx` — the PickerMenu-based value dropdown (status/select/multi), anchored at the cell, `useDismiss` + `useExitPresence`, chips as options; props `(def, value, onCommit, anchor)` — table-agnostic.
- `React/src/renderer/src/Detail/Views/PropertyEditing/PropertyEditor.tsx` — the inline text editor (number/link/file) implementing A-12; same table-agnostic prop shape.
...
**Files:** Create `shared/columnStyles.ts` + `.test.ts`, `Detail/Views/Table/columnStyles.ts` + `.test.ts` (the schema-aware resolver — renderer-side, mirroring `columnAlign.ts` exactly); Modify `shared/views.ts` (BOTH the hand-written `SavedView` interface at views.ts:75-93 AND the `savedView` zod codec at :165 — they're separate, not inferred), `Detail/Views/Table/viewMerge.ts`.
**Interfaces — Produces:** `type ColumnLook = 'pill'|'capsule'|'checkbox'|'switch'|'title'|'full'|'filename'|'path'` · `interface ColumnStyle { look?: ColumnLook; date_format?: 'short'|'full'|'dayMonthYear'|'monthDayYear'; time_format?: 'none'|'twelveHour'|'twentyFourHour'; number_format?: 'integer'|'decimal'|'percent'|'currency' }` · shared: `defaultStyleFor(declaredType: string | undefined): ColumnStyle` (string-keyed, pure — `shared/` CANNOT import the renderer's `pipeline/value.ts`, so the type-name key crosses the seam; status→`pill`, checkbox→`checkbox`, url→`full`, file→`filename`, datetime/number format defaults; select/multi are NOT style-addressable — Nathan, at pickup: no Style menu, chips always render pill by construction) · renderer: `styleFor(columnId, schema, view): ColumnStyle` (calls `declaredType` then merges `view.column_styles?.[columnId]` per-key over the defaults — the `alignFor` shape, columnAlign.ts:24-26).

- [ ] **Step 1: Failing tests** — `shared/columnStyles.test.ts`: `defaultStyleFor` per type (status → `{look:'pill', …}`, datetime → `{date_format:'full', time_format:'none'}`, number → `{number_format:'decimal'}`, unknown → `{}`); `Table/columnStyles.test.ts`: `styleFor` returns defaults with no view entry, a `view.column_styles[id]` entry overrides per-key (partial merge, not replace), unknown column → safe defaults.
- [ ] **Step 2: Run — FAIL** (`npx vitest run src/shared/columnStyles.test.ts src/renderer/src/Detail/Views/Table/columnStyles.test.ts`).
- [ ] **Step 3: Implement** — `shared/columnStyles.ts` (zod enums exactly as in Produces — Swift's sets verbatim; `columnStyle = z.looseObject({...each field optional enum with .catch(undefined)})`; imports NOTHING from views.ts — no cycle: views.ts imports the codec, the renderer resolver imports both); `views.ts` gains `column_styles?: Record<string, ColumnStyle>` on the interface + `column_styles: z.record(z.string(), columnStyle).catch({}).optional()` on the codec (lenient — one bad entry never sinks the view; unknown keys ride through); `mergeOverrides` gains a SIXTH param `styles: Record<string, ColumnStyle> = {}` folding `column_styles: { ...liveView.column_styles, ...styles }` BEFORE `...patch` (a patch still wins — the H-2 guard); `persistView` (TableView.tsx:158 — the ONLY call site) passes the `styleOverride` state once Task 4 adds it (until then the default `{}` means zero caller changes).
...
**Files:** Create `Detail/Views/PropertyEditing/formatValue.ts` + `.test.ts`.
...
**Files:** Modify `Cell.tsx`, `TableView.tsx` (thread `styleFor` results down as a per-column resolved style, computed beside `colAlign` — NOT via `liveView`); Create `PropertyEditing/statusCycle.ts` + `.test.ts` (the `statusGroupOf` half — Task 6 adds `nextCycleValue`); Modify fixtures + `properties.ts` rider comment; Test additions in `cellResolve.test.ts`-style pure tests where possible.
**Interfaces — Consumes:** Task 1 `styleFor`, Task 2 formatters. **Produces:** `statusGroupOf(value, def): StatusGroupId | undefined`; `Cell` accepts `style: ColumnStyle`; renders: status `pill` (today's Chip) / `capsule` (the icon-only chip — `chip + chipColor[color] + chipCapsule`, the named shape in `chip.css.ts` (landed at pickup), wrapping an `Icon` at 13; icon by the fixed group id: `upcoming`→`circle-dashed` · `in_progress`→`minus` · `done`→`check`) / `checkbox` (the `chipCheckbox` square, same group mapping rendered none/`minus`/`check`) · checkbox `checkbox` (today) / `switch` (the existing `Switch` component from `Components/Switches`, read-only visual) · url `full` (today's `<a>`) / `title` (same `<a>`, link-colored, showing the fetched-title-or-URL — URL until the fetch Prospect lands) · file `filename` (per-file chips — one `Chip color="default"` per `FileRef`, label = stripped basename) / `path` (per-file chips, label = full `ref.path`) · datetime/number via Task 2 formatters. **Both url looks render in the link color token.**
...
- [ ] **Step 2: Run — FAIL.**
- [ ] **Step 3: Implement** — the switch branches read `style.look`/formats; capsule/checkbox glyphs via `statusGroupOf` (the capsule shape is `chipCapsule` in `chip.css.ts` — already landed, consumed by the showcase Select row); delete the old local `formatDate`; drop `display_as` + `date_format` from `__fixtures__/registry.json` + `collection-with-status.json`; rewrite the `properties.ts` rider comment (formats now live per-view; riders stay inert foreign keys on disk).
...
**Interfaces — Consumes:** Task 1 types. **Produces:** `ColumnMenuContext` gains `style?: { type: PropertyType | 'title'; current: ColumnStyle }`; `ColumnMenuAction` gains `` `style:${key}:${value}` `` strings (e.g. `style:look:capsule`, `style:date_format:short`); the Style submenu renders per-type radio items: status Pill/Capsule/Checkbox · checkbox Checkbox/Switch · url Title/Full Link · file Filename/Full Path · number Integer/Decimal/Percent/Currency · datetime **Options ▸**. **Select/multi get NO Style submenu (Nathan, at pickup) — chips always render pill.** **Datetime menu labels use format-type NAMES, never rendered samples (Nathan, final):** "Short Date" · "Full Date" · "DD/MM/YYYY" · "MM/DD/YYYY" · separator · time names in the same convention ("None" · "12 Hour" · "24 Hour" — naming flavor like "Local Time"/"UTC" welcome where it clarifies) — Swift's `displayLabel` approach. Title gets no Style.
...
### Task 6: `PropertyPicker` — the PickerMenu value dropdown + checkbox-status cycle

**Files:** Create `PropertyEditing/PropertyPicker.tsx`; Extend `PropertyEditing/statusCycle.ts` + `.test.ts` (adds `nextCycleValue`); Modify `Cell.tsx`/`TableView.tsx` (mount state: `editing: {rowId, colId, mode:'picker'} | null` lives in TableView; row-drag arming untouched).
**Interfaces — Consumes:** `PickerMenu`/`PickerOption` (as-is), `useDismiss`, `useExitPresence`, `findOption`/`chipColorFor`, `applyPropertyValue`, store `mutate`. **Produces:** single-click on status/select opens `PropertyPicker` (options as `Chip`s, `selected` = current; props `(def, value, onCommit, anchor)` — the commit wiring stays the caller's); pick → optimistic `setValueOverride` + `mutate({op:'setProperty', path, propertyId, value})` → close; multi-select toggles values (stays open, chips reflect live). Checkbox-look status cells DON'T open the picker: click = `nextCycleValue` write (`statusCycle.ts`: groups in schema order, first-in-order option per group, empty groups skipped, value in Done→empty). `--dropdown-origin` set to the cell's anchor point.
...
### Task 7: `PropertyEditor` — inline editing (number, link, file) + title rename

**Files:** Create `PropertyEditing/PropertyEditor.tsx`; Modify `Cell.tsx`/`TableView.tsx` (mode `'editor'`), `shared/cellMenu.ts` (unhide `title:rename`).
**Interfaces — Consumes:** A-12 model, `isValidLink`/`normalizeLinkUrl` (`shared/links`), `mutate`. **Produces:** an input overlaying the cell (absolute within the cell, table-tokens type ramp): number → **keystroke-filtered (Nathan, at pickup: invalid characters just can't be typed)** — digits, one leading `-`, one `.`; commit parses float; an EMPTY commit clears the value (`setProperty` with `value: null` — `applyPropertyValue` deletes the key, `propertyValue.ts:132`); a lone `-`/`.` reverts; url → validates/normalizes on commit; file → edits the FIRST ref's `path` (multi-file editing = the picker Prospect; Edit on a multi-file cell edits ref 0 — documented in the code); title rename → `mutate({op:'rename', path, newName})`. Enter=commit · blur=commit · Esc=revert+exit. Single-click number cells enter the editor directly (A-8); link/file enter only via the menu's Edit (A-3/A-9).

- [ ] **Step 1: Failing tests** — jsdom: number cell click mounts editor; typing letters into the number input is a no-op (value unchanged); Enter commits `setProperty {kind:'number'}`; empty commit sends `value: null`; Esc calls no mutate; blur commits; url Edit normalizes (`example.com → https://example.com`).
- [ ] **Step 2: Run — FAIL.**
- [ ] **Step 3: Implement.**
- [ ] **Step 4: Run — PASS**; typecheck.
...
**Type consistency:** `ColumnStyle`/`ColumnLook` (T1) consumed by T3/T4/T5; `statusGroupOf` defined T3, consumed T3/T6; `setColumnStyle` defined T4, reused T5; `CellMenuContext` T5 only; `nextCycleValue` T6; `editing` state shape shared T6/T7.
**Deviations resolved with Nathan (at pickup):** select/multi get NO Style menu — chips always pills; their single-click value picker stays · Capsule = the icon-only chip, codified as `chipCapsule` in `chip.css.ts` at Nathan's direction (the showcase Select row consumes it), with the fixed-group glyph · number invalid input is keystroke-prevented, not commit-handled (empty commit clears via `value: null`) · the editing surfaces live table-agnostic in `Detail/Views/PropertyEditing/` for Gallery/List reuse.
