...
- **Body** — the Markdown-source description. This IS Items' description field (Shape A): the body is the single source of truth, with **no frontmatter `description` key and no mirror**. Capped at **250 Markdown-source chars** (`ItemValidator.maxDescriptionLength`, with an optional per-Type `description_cap` override; raw `.count` of the draft, markup counted). Validated on save (`ItemValidator`), never silently clamped.
