...
Items open in the **Item Window** — a native, draggable, chromeless floating scene (`WindowGroup(for: ItemRef.self)` + `.windowStyle(.plain)` + `.windowLevel(.floating)`, opened via `openWindow(value:)`), hosted by the reusable `PreviewWindow` primitive: a material card with custom 2-corner chrome (custom close + drag handle, no traffic lights), drag-to-move, and Esc-to-close. One config-driven `ItemWindowRenderer` draws every window from its resolved `template_config` (see § "Item Templates"); promoted properties render on the main panel, the rest in the per-archetype overflow surface (a side inspector for `banner_two_column`, a dropdown otherwise — disjoint from the main panel, no double-render).
...
- **Description** — the Markdown-source body (the Shape-A contract above), MarkdownPM-rendered: capped at **500 source characters** (`ItemValidator.maxDescriptionLength`, with an optional per-Type `description_cap` override), validated on save and rejected over-cap for in-app saves — never silently clamped; external/raw-Obsidian over-cap surfaces a non-blocking warning.
