...
**Item Type** — a folder at the nexus root carrying an `_itemtype.json` sidecar (`schema_version: 2`). The sidecar defines the property schema shared by every Item inside (the Type plus every Item Collection underneath). Folder name = title; renaming the folder renames the Type. Sidecar fields: `id`, `singular`, `icon`, `properties`, `views`, `template_config` (the per-Type template — see § "Item Templates"), `modified_at`, `schema_version`, `collection_order`, `item_order`, `default_sort`. Discovery is sidecar-driven: any root folder carrying `_itemtype.json` is an Item Type, regardless of name.
...
- **Body** — the Markdown-source description. This IS Items' description field (Shape A): the body is the single source of truth, with **no frontmatter `description` key and no mirror**. Capped at **500 Markdown-source chars** (`ItemValidator.maxDescriptionLength`, with an optional per-Type `description_cap` override; raw `.count` of the draft, markup counted). Validated on save (`ItemValidator`), never silently clamped.
...
- **Item** — row-shaped data without prose: contacts, wishlist, bookmarks, citations, music releases, recipes-as-rows, references. Created inside an Item Type; opens in the floating Item Window.
...
Items open in the **Item Window** — a **non-activating AppKit floating panel** (`FloatingItemPanel`, an `NSPanel` hosting SwiftUI via `NSHostingController` / `ItemWindowHost`), one per Item (several can be open at once), owned by `ItemWindowPanelManager`. It is a **fixed size** (`PUI.ItemWindow` ≈ 800×560), uses the **standard window background** (no custom fill), floats over the main window, is **draggable by its background**, and is **not** minimizable or resizable. Being non-activating with its content pinned to the active appearance (`.environment(\.controlActiveState, .active)`), it **never takes focus from the main window and does not dim when you click off it**. The native traffic-light buttons are hidden; a custom non-glass **✕** (top-left) and **Esc** both close it. One config-driven `ItemWindowRenderer` draws the main column (header · property bar · body · breadcrumb footer) plus a native trailing **`.inspector`** (`ItemInspector`) the system slides in/out — the panel being fixed, the inspector takes its width **from the body** (the body narrows) rather than growing the window.

**Pinned (promoted) properties** render in a **segmented property bar** below the title (a native-segmented-control look — `PropertyFieldBar`) for always-on access without the inspector; non-pinned properties live in the inspector. (During the design phase the bar shows placeholder "Label" cells when no template defines pinned properties, so its look is always visible.) Two Item-specific rules: items in a Type root (no Collection) get no pinning controls (the pinned set persists per Item Collection), and stale pinned IDs referencing deleted schema properties are filtered on render. Persistence shape + pin/unpin mechanics are canonical in [[Properties]] § "Where Properties Live" + § "Item Inspector → Pinned Properties".

- **Title** — the filename, editable in place (rename retitles the underlying `.md` file); styled as a window-title.
- **Icon** — optional SF Symbol via the native `IconPicker`, flush beside the title.
- **Inspector** — one **unified flat-hairline menu** (not a grouped Form): the Context tiers on top, then the schema properties, every row the same `[icon] [label] ··· [value]` shape, no section headers, no meta. Non-pinned properties are edited inline via per-type editors (`PropertyEditorRow` — select/status/number/date/relation/url/…); select-type pickers apply on click (a collapsed pill that opens a dropdown). A red **"Delete"** is pinned bottom-right.
- **Description** — the Markdown-source body (the Shape-A contract above), MarkdownPM-rendered, filling the panel below the property bar: capped at **500 source characters** (`ItemValidator.maxDescriptionLength`, with an optional per-Type `description_cap` override), validated on save and rejected over-cap for in-app saves — never silently clamped; external/raw-Obsidian over-cap surfaces a non-blocking warning.
- **Spaces / Topics / Projects (tier 1 / 2 / 3) relations** — pre-configured context-link properties (`relation_target` `{ kind: "context_tier", tier: N }`) merged onto the schema via `BuiltInContextLinkProperties`, edited inline as the top group of the inspector. Values render as the target's icon + title in plain styled colored text.
- **Footer** — a breadcrumb of the Item's Type (and Set, if any). The read-only meta block (`id` / `created_at` / `modified_at`) is no longer surfaced in the window.

Closed via the custom **✕** or **Esc**; the inspector's Delete removes the Item then closes the panel (both via `ItemWindowPanelManager`). Save commits via `ItemContentManager.updateItem` (drift-guarded). The body is the capped description only — no blocks, no `@Columns`; if the entry needs full prose, it should be a Page.
...
Each Item Type (and optionally each Item Collection) carries a `template_config` that one `ItemWindowRenderer` consumes to draw the window — layout is data, not per-layout view files. Fields: `layout` (the `LayoutArchetype` enum — on-disk `compact|standard|banner_two_column|gallery|wide|reserved`, tolerant `unknown`; only `standard` ships, the rest are selectable-but-muted in settings), `promoted_properties: [{id, display}]` (pinned to the main panel; `display` is the tolerant `PropertyDisplay` enum `inline|thumbnail|banner|chips|list` overriding the archetype default), `cover_property_id` (names an existing image-filtered `.file` property — no new property type), `description_cap`, `default_description`. Collection-level config overrides its parent Type; absent ⇒ Type default; absent everywhere ⇒ the standard window.

Editing happens in the **template**, not the live item: a "Templates" settings pane (Item scopes only; muted for Pages) embeds the renderer in a WYSIWYG mockup frame (`editing: true`) where you pick the archetype, pin/unpin + drag-reorder promoted properties, set per-property display, and choose the cover. Edits apply to every Item the Type/Collection governs. Registry decision #15 in [[Paradigm-Decisions]].
