...
Wikilinks resolve by ID. Disk format: plain `[[Title]]` (Obsidian-compatible) — Pommora never writes a piped form to disk. Rename-safe ID resolution comes from a derived `wikilinks: [<id>, ...]` frontmatter mirror, auto-maintained on save (v0.4.0). Renames update the displayed label at render time; the stored reference never changes. Untargeted `[[Title]]` (typed or pasted from another tool) resolves by current basename match; the editor underlines ambiguous matches.
