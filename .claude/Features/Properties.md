...
**Per-Type Editor Panes:** The remaining per-type property editors not yet built — the Number value-type pane, its number-format picker, and the relation (context) pickers. The Checkbox, Select, Multi-Select, Status (grouped / flat option lists, add · recolor · reorder · drag, right-click Rename · Remove · Clear), URL, and Date & Time editors have shipped; the rest follow on their patterns.
