...
Pommora's standard semantic icons — the canonical glyph for each pane, property type, and recurring concept. Icons come from **lucide-react** (named here by their lucide id); this doc names the assignment, the code holds the maps. Anything without an assigned glyph falls back to **`DashIcon`** (the dashed-square placeholder) until a symbol is chosen — a placeholder is intentional, not a gap to fill arbitrarily.
...
`eye-off` is the hidden-state variant, staged for the Visibility pane's per-property toggles.
