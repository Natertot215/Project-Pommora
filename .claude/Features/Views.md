...
- **Banner** — a `banner` field on the container sidecar (`_pagetype.json` / `_pagecollection.json`). When unset, the banner area does not exist; when set, a full-width image area renders above the container title (header grows taller) in both renderers, hideable per view via `show_banner`. **Add Banner is a small floating button that appears only when no banner is set** (the fullscreen page add-icon pattern) → file picker. Written via `PageTypeManager.setBanner`.
...
- A window-toolbar Views button sits in its own pill left of the ViewSettings capsule (same `.glassEffect()` + popover pattern). It has **two display modes** — icon-only (65×36pt) or glass icon + active-view title — toggled via right-click ("Show View Title"), persisted in `state.json` (`views_button_style`).
