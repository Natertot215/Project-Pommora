...
**View-Settings Editing Panes:** The Sort and Filter leafs, and the Layout leaf's order + visibility section (which folds the Visibility pane in, gated on its own design). The Format control persists Standard / Compact but is visually inert until the Compact table style lands. The pipeline already honors these configs from the sidecar, so the gap is authoring them — Properties, Visibility, and Group are reachable.
