...
- **Group** — `GroupResolver.resolve` produces `[ResolvedGroup]`. `.structural` (the default): vault scope groups by Collection with Sets nested as children; collection scope groups by Set plus an ungrouped root band (zero Sets → one ungrouped band, no header — the flat look). `.property` (groupable types = Select / Status / Checkbox / Date — single-value only) flattens to buckets ordered by `order_mode`: `configured` (schema-option order; Date by `date_granularity` bucket, Checkbox Unchecked-first), `reversed`, or `manual` (the `order` override). The "No [Property]" bucket is placed by `empty_placement` and dropped by `hide_empty_groups`; Checkbox routes an unset value → Unchecked (no nil bucket). A missing / non-groupable `property_id` falls back to `.structural`. `.flat` is a single group.
...
- **Banner** — a `banner` field on the container sidecar (`_pagetype.json` / `_pagecollection.json`), hideable per view via `show_banner`. *As of HEAD (actively changing — see the chrome-flux note under Views Dropdown):* with no banner set, the **22pt (`.title` bold) detail title** sits as plain chrome above the content; with one set, a **180pt full-width image bleeds edge-to-edge under the sidebar + inspector + toolbar** via Apple's `backgroundExtensionEffect()` (macOS 26 Liquid Glass; the Landmarks-sample pattern), and the **title overlays it at the bottom-leading corner**. Whether that under-toolbar bleed interacts with the toolbar chrome problems is **unknown and untested**. Add Banner is a hover-revealed button in the no-banner state → file picker; once set, a Change / Remove menu manages it. Written via `PageTypeManager.setBanner`.
...
- A window-toolbar Views button sits left of the settings·nav·inspector trio (popover pattern). As of HEAD it's a **fixed-width icon-only `Button` + `.popover`** showing the active view's icon (default `rectangle.3.group`), with **no right-click / context menu in code**; the old two-mode "Show View Title" toggle was dropped (`views_button_style` dormant). The `.toolbar` is hosted on the **detail column** as two glass capsules (Views pill | trio), each `.sharedBackgroundVisibility(.hidden)`.

> ⚠️ **Chrome flux — toolbar, Views button, and banner are NOT settled truth.** The Views button only **"looks" correct** *right now* — at **unknown cost**, and via methods we're not sure are the best (or even correct). Specifically: (1) both toolbar items are trailing, so the inspector **adopts the Views pill when toggled** — a position Apple-native apps don't use, cause not understood; (2) how the methods used to make the button look correct affect the rest of the toolbar is **assumed-impactful but unmapped**. Treat all of this as current-state observation, never as a foundation for future work. Open unknowns + next steps live in `// Planning//06-13-Views-UIX-Fixes.md`.
...
- **Group** — the redesigned `GroupingPane`: a **Grouping** toggle (off = `.structural` file-system default) that discloses an inline property picker (Select / Status / Checkbox / Date); a per-type **Order** popout (Default/Manual · Asc/Desc/Manual · Asc/Desc · Off/On), a **Date By** popout (Day/Week/Month/Year), a no-Add manual **Options** reorder list (Select/Status), and a secondary bottom-pinned empty-group footer (Hide empty groups · Empty group Top/Bottom). Full spec → `Planning/06-15-Grouping-Redesign.md`. *(View-side rendering of the groups + group-header manual drag is still pending — see the plan.)*
