...
- The **Sorting pane** (the Sort leaf, both doors) authors the sort on the Grouping pane's chassis: **Sort By** as the in-pane vertical disclosure (None + Title + Modified + the schema's sortable properties — only types the sorter genuinely ranks; Context and File are absent because they'd sort to a no-op), a per-type **Order** picker (Select/Status: Default / Reversed; dates, numbers, checkbox: Ascending / Descending; text: A → Z / Z → A), a **Sub-Sort** picker with its own Order (scoped to Default / Reversed), and the read-only **example order** — the primary property's chips in effective order, shown only for finite-ordered types.
