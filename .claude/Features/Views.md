...
- A window-toolbar Views button sits left of the ViewSettings capsule (popover pattern). It's a **fixed-width icon-only button** showing the active view's icon (default `rectangle.3.group`); the old two-mode "Show View Title" toggle was dropped (`views_button_style` now dormant). **Open issue:** under macOS 26's Liquid Glass toolbar (`NSGlassContainerView`) the primary-action controls (views / settings / nav / inspector) currently collapse into the `»` overflow — under troubleshooting.
