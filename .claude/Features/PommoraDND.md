...
The **table band surface** (`Table/bandDnd.tsx`) extends the same insertion-line treatment to the table's group headers: the **glyph** is the drag surface (the chevron and hover "+" isolate on pointerdown), the line + portal ghost are the chrome, a Set band's middle zone reads as a **nest-into** highlight (cycle-guarded), and all slot/parent/order math lives in a pure, unit-tested model (`Table/bandDndModel.ts`). A drop hands the caller a *classified* commit — reorder vs reparent, routed by the slot's implied parent against the dragged band's current parent — so the view never re-derives it. Its snapshot extends the measurement discipline one step: **the band list is snapshot state too** — a mid-drag tree swap goes stale together with the geometry, so both invalidate and re-measure as one. **Esc aborts an active drag on every surface** (sidebar, table rows, bands) through each gesture's existing cancel path.
