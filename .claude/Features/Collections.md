...
A Collection optionally subdivides into **Page Sets** — schema-less sub-folders; full spec → [[Sets]]. The Collection's sidecar holds `set_order` alongside `page_order`, and its detail view shows root pages plus each Set's pages as a flat concatenation.
