...
A Collection optionally subdivides into **Page Sets** — schema-less sub-folders, full spec → [[Sets]]. The Collection's sidecar holds `set_order` (its Sets' display order) alongside `page_order` (its root Pages' order). The Collection detail view shows root pages plus each Set's pages as a flat concatenation; its footer add menu offers New Page + New Set.
