...
- **Navigation · Agenda · Contexts · Collections · Settings** — below Homepage, in that default order, and **drag-to-reorder** (Homepage stays pinned). **Collections · Contexts · Agenda** switch the content column's mode; **Navigation** summons the NavPane (the floating navigation mini-shell → `Navigation.md`); **Settings** is a placeholder for its future glass-window surface. The mode icons reuse each kind's own entity icon, tracking any personalization override.
