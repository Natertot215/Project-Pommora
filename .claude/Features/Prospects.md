...
- **Page → Item (demote):** every frontmatter property in the destination Item Type's schema carries to `properties`; properties absent from the destination Item Type are **stripped**; the body carries over but is **clamped to the description cap** (250 source chars, or the Type's `description_cap` override): over-cap content is truncated, requiring explicit user confirmation — a deliberate exception to the save-time rule that never clamps silently. `id` preserved.
