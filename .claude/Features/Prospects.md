...
- **Page → Item (demote):** every frontmatter property in the destination Item Type's schema carries to `properties`; properties absent from the destination Item Type are **stripped**; the body carries over but is **clamped to the description cap** (500 source chars, or the Type's `description_cap` override): over-cap content is truncated, requiring explicit user confirmation — a deliberate exception to the save-time rule that never clamps silently. `id` preserved.
...
**Description:** SHIPPED (ItemsV2) — no longer a prospect. Per-Type/Collection `template_config` (layout archetype, promoted properties + per-property display, image-filtered cover, `description_cap` override, `default_description`) drives one config-driven Item Window renderer, edited in the Templates settings pane. Kept here only as a redirect — full spec → [[Items]] § "Item Templates"; schema → registry decision #15 in [[Paradigm-Decisions]].
