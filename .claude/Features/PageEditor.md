...
| **v0.2.7.x (later)** | QUEUED | Bullet glyph (`-` → `•` visual substitution — non-blocking cosmetic, deferred from v0.2.7.2). Blockquote rendering (Apple-Calendar-event-card chrome). Code & quote `Enter}` auto-completion. Code block → red text bug. Auto-format `←` / `↔` (the `->` transform works but `<-` / `<->` don't fire on typed input — paste works fine). Tables (ASAP but realistic estimate 10-15h after divider iteration experience). Sidebar + Vault/Collection drag-to-reorder. PreviewWindow primitive build. Phase 4.5 auto-pair polish. Phase 3 engine AST rewrite. No specific patch number assignments — pick what's next at session time. |
