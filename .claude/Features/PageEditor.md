...
| **v0.2.7.8** | ✅ SHIPPED 2026-05-23 | **Page-editor title area + structural divider + scrolls with body.** Two-phase ship in one row. **Phase 1 — divider visual.** `Rectangle().fill(Color(NSColor.separatorColor)).frame(height: 1)` between the title `TextField` and the body editor in `PageEditorView`, padded `.horizontal, 24` so its edges align with the body content gutters (not the window edges). Uses the system separator color — same hairline AppKit draws for `NSWindow.titlebarSeparatorStyle = .line` and `NSSplitView` dividers; rendered at explicit 1pt rather than SwiftUI's sub-pixel default `Divider`. Explicitly NOT the body HR style (this is structural chrome separating the title area from the body, not a markdown thematic break). Title bottom padding 14pt above divider; equidistant 14pt below before body's first line. **Phase 2 — title scrolls with body content.** The title used to be pinned at the top of the editor regardless of body scroll position. Restructured `PageEditorView` to a `ZStack(alignment: .topLeading)` with the body editor as the bottom layer and a SwiftUI `VStack { TextField; Divider }` overlay on top. The overlay tracks body scroll via `.offset(y: -min(max(0, scrollOffset), titleAreaHeight))`, fed by a new optional `onScrollOffsetChange: ((CGFloat) -> Void)?` callback on `NativeTextViewWrapper` (engine wrapper now publishes normalized scroll Y — 0 at rest, positive when scrolled down — by reading `scrollView.contentView.bounds.origin.y + scrollView.contentInsets.top` from inside the existing `boundsDidChangeNotification` observer). Body editor's `textContainerInset.vertical = titleAreaHeight = 90pt` reserves a scrollable empty zone at the top of the text container (NOT `safeAreaInsets`/`contentInsets`, which are fixed scroll-view padding that doesn't move with scroll — that was the failed first attempt). ZStack has `.clipped()` so the offset overlay doesn't bleed into the parent NavigationSplitView detail / toolbar region. **Why this architecture, not engine documentView restructure:** the engine's existing `NSScrollView`-wraps-`NSTextView` flow stays untouched; the only engine addition is the scroll-offset callback. Title scroll behavior lives entirely Pommora-side. Cover-image / banner support drops into the same overlay VStack with no further engine changes — just stack `Banner / Cover` above the `TextField` and bump `titleAreaHeight` to accommodate. Symmetric trade-off: `textContainerInset` is symmetric so the document bottom gets 90pt of trailing whitespace above the overscroll region (acceptable; matches Notes/Bear-style end-of-doc spacing). |
