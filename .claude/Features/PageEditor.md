...
| **v0.2.7.6** | ✅ SHIPPED 2026-05-22 | **Foldable headings.** Hover any heading line → chevron appears in left gutter; click toggles true zero-height collapse down to the next equal-or-higher heading (or document end). **Architecture: content-manager elision** — `NativeTextViewCoordinator` conforms to `NSTextContentStorageDelegate` and returns an empty `NSTextParagraph` from `textContentStorage(_:textParagraphWith:)` for paragraphs intersecting any folded range. TextKit 2's layout pipeline drops these paragraphs entirely → folded content takes zero vertical space AND is unreachable to caret, find, selection, and arrow-key navigation (no caret-skip patches needed). **Chevron rotation animates over 200ms ease-in-out**; the fold collapse itself is instant. **Persistence:** per-Page frontmatter as `folded_headings: ["## Foo", "## Notes [2]", ...]` — ordinal-disambiguated source-line keys (level + heading text + `[N]` for duplicates within the document). **Reconciliation on save:** `PageEditorViewModel.flushNow()` drops keys that no longer match any current heading (renaming a heading drops its entry; orphan keys never accumulate). **Conditional unfocus on chevron click** — caret preserved unless it would land inside the freshly-folded range. **Shared helpers:** `HeadingChevronGeometry` (rect computation, shared by renderer + hover handler), `NSTextLayoutFragment.nsRange` extension, AST-based code-block detection (replaces color comparison). Full architecture at `// Guidelines//Markdown.md` §9.11. 21/21 tests pass. |
