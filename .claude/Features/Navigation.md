...
**Navigation Popover:** The fuller history surface — a glyph opening a popover with two lists: a user-curated **Pinned** list and an auto-tracked **Recents** list (LRU, capped). Single-click selects a row, double-click opens it in the main pane, and right-click pins or unpins; the store persists per Nexus. Two placeholder entry points already sit in the UI for it — the toolbar trio's Navigation button and the sidebar ribbon's Navigation icon — both inert until it ships, so the eventual surface can be trialed as a window, a toolbar popover, or both. The Back/Forward history and the breadcrumb cover navigation today — the popover, its Pinned and Recents store, and any keyboard accelerators are unbuilt.
