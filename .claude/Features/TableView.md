...
- **Rich cells + in-cell editing** — type-aware chip/checkbox rendering and inline editing over the plain-text cells; tracked in `Views.md`'s Pending. The stable seams (resolved columns, resolved groups, per-cell value + display data) are what those route to.
