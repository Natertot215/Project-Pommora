...
> **Naming lineage.** The old three-tier model (Vault → Collection → Set) collapsed to two: a Collection nesting recursive Sets. Old `PageCollection` (middle) + `PageSet` (bottom) merged into one recursive `PageSet`; sidecar stays `_pageset.json` at every depth (→ `History.md`).
