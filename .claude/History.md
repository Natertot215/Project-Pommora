...
Group bands drag by their **glyph** on the sidebar's insertion-line gesture: vertical reorder persists per-view (`group_order` for structural bands, merged over the FULL tree so collapsed siblings survive; `group.order` + manual mode for property bands — its first UI writer), and a Set band's middle-zone nest or parent-changing slot commits a real `moveSet` (the destination's current children with the moved id appended — the order-leak guard). Esc now aborts every drag surface (bands, table rows, sidebar). Ships with the **no-"None"-band ruling**: property grouping's value-less rows render as a flattened, header-less tail pinned last, identical to structural loose rows (`empty_placement` stays decode parity, never read). Hardening: two adversarial plan rounds pre-build, a post-green build-breaker (3 findings fixed — region-owned hit-testing so hovering a group's data rows can't hand the slot to the next header; a failed move commits nothing; the drag ghost's label resolves once at activation) and a simplifier pass; verified LIVE against the real Nexus via CDP — structural reorder + sidecar/sidebar-independence proof, a reparent round-trip (real folder moves), and a property-band drop, all screenshot-evidenced. Suite 939 → 991. Decisions → the lock below; spec → `Features/TableView.md` §Groups.
