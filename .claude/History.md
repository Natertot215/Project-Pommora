...
#### ViewPane Properties Flow — the full assign surface (2026-07-02, branch `main`, React)

The Properties pane became the whole assign surface (the 7-2 ratified log, built T1→T7 attended): assigned rows on top, an **All Properties** disclosure pinned to the pane's bottom that **rises** open (an elastic spacer collapsing on the pane's beat) to list every unassigned registry def with a `+` promote; a **nexus-wide cosmetic property order** persisted inside `.nexus/properties.json` (the file grew to `{order, defs}` with a lenient legacy read; SQLite's `position` mirrors it); the two-region drag (region-owned classification — reorder-assigned / reorder-nexus / assign-at-slot / unassign with an area highlight + natural-slot snap; Esc aborts, swallowed from the dropdown's dismiss); a capped scrolling pane under the **hoisted scroll-edge fade** (now scroll-driven like the OverflowScroll eclipse — no fade at rest, Nathan's live correction); the header's `square-plus` create sliding into the type picker; row right-click **Rename** (a net-new `renamingProperty` store channel over `EditableInput` — properties are ids, not paths, so `RenamableTitle` stays untouched) **· Remove**; and the editor's `‹ Name ⋮` header carrying Remove + the **pane-gated Delete** behind main's native confirm — the footer Delete row died. Every pane push at every depth now slides (the subviews ride a nested PaneSlider). **REVERSAL of 07-01's Remove ruling:** Remove now **strips every member page's value and caches it restorably on the Collection's sidecar** (one atomic transaction); re-assigning restores per-value against the def's CURRENT type + options; Delete purges the caches everywhere — no dormant foreign values on pages (provenance: V1's "Clear Values," cut in the V2 review as leave-in-frontmatter; Nathan reversed it here). **Duplicate names now allowed on create AND rename** (flat policy — ids keep twins safe; the quirk lives in Properties.md); **Max Properties killed**. Spec → `Features/Properties.md`; the 07-01 entry below predates the Remove reversal.

#### Banner icons + title menu · notched glass dropdowns (2026-07-02, branch `main`, React)

Bannered container/context views carry the entity's glyph beside the title (banner ⇒ icon, kind-default when unassigned; banner-less stays text-only); **pages and the homepage stay title-only — Nathan's call**. The banner title hosts the shared `DetailTitleHeader`: right-click pops the native Rename / Change Icon menu (inline rename via `submitRename`, the IconPicker stub), its stopPropagation leaving the banner's own Change/Remove menu to the rest of the surface. The nav + settings dropdowns adopted the picker's beak via the hoisted **`NotchedPane`** (one path serves as frost clip AND SVG-stroked outline; the beak aims at its trigger from the trio's measured width), reshaped to the **Apple-popover silhouette** — tangent cubics, rounded crest, no tip vertex. Gotcha rerun worth its scar: the Bloom class must ride the pane + frame individually — an opacity-animated wrapper backdrop-roots the frost to nothing (Build-Gotchas §Glass) — a fix that also revived PickerMenu's silently-dead pure-glass frost.

#### PommoraIcons — the mixed Tabler + Lucide registry (2026-07-02, branch `main`, React)

The Lucide-only icon set became **PommoraIcons** (`design-system/symbols`): **Tabler** (`@tabler/icons-react`) is the default source at the registry's **1.75-stroke** default; **ratified Lucide keeps** stay at their library look (`gallery-vertical-end` Collections, the folder pair, `log-out` sidebar in-out, `link` + `link-2` — url vs Connections stays two glyphs — `layout-grid` tiers, `square-plus` Add Banner); `square-dashed` is the first first-party custom SVG in the same slot shape, and the Icon seam normalizes the libraries' differing stroke-prop spelling. Stray direct lucide imports folded into the registry (PropertyTypes, PropertiesPane, menu back-row, editor table grips — ViewPane deferred, in flight in a parallel session); the editor's hardcoded lucide path-data swapped to Tabler (fold chevron, checkbox ✓; the grip mask was already geometry-identical). Six dead registry entries dropped — unused in code, unreferenced in the Nexus (stored icons are all legacy Swift-era names that already fall back gracefully). `calendar` (Tabler `calendar-month`) is THE calendar glyph everywhere. Deliberately shipped pre-IconPicker so the picker enumerates PommoraIcons from day one. Registry mirror → `symbols/Symbols.md`; assignments → `Features/Icons.md`.

#### Conditional inspector removed — the inspector always compresses (2026-07-02, branch `main`, React)

The fitting-table-compresses / overflowing-table-keeps-width conditional went through three implementations, each CDP-verified correct — and still read wrong in real use; torn down entirely on Nathan's order, no dead code left. The surviving rule is one line: `overflowing` = the column sum exceeds the table's box (flatten + h-scroll); the inspector compresses the pane unconditionally and a tighter table scrolls within it. Durable lesson: `scrollWidth` floors at `clientWidth`, so any is-content-bigger comparison built on it can latch.

#### Tables Phase 2 — band drag shipped (2026-07-02, branch `main`, React)

Group bands drag by their **glyph** on the sidebar's insertion-line gesture: vertical reorder persists per-view, a Set band's nest (its whole region, one continuous span) or parent-changing slot commits a real `moveSet`, and Esc aborts every drag surface (bands, table rows, sidebar). Ships with the **no-"None"-band ruling** — value-less rows render as a flattened, header-less tail identical to structural loose rows. Hardened through two adversarial plan rounds, two post-green breaker rounds (6 findings fixed), two simplifier passes, and a live CDP pass against the real Nexus (structural reorder, a reparent round-trip with real folder moves, a property-band drop — all screenshot-evidenced). Suite 939 → 994. Decisions → the lock below; spec → `Features/TableView.md` §Groups.
...
Group-band ordering in table views is **manual-only** — no user-facing order mode. Bands *default* to the defined order (property options in defined sequence, status groups upcoming → in-progress → done; structural bands seed from fs `set_order`), and the **first drag snapshots that default into an owned sequence that never re-derives**. Homes: structural → the net-new view-level `group_order: string[]` (one flat set-id array covering every nesting level); property → the Swift-parity `group.order` + `order_mode: 'manual'` (`configured`/`reversed` stay decode-only parity). **Structural bands do not mirror the sidebar** — vertical reorder is per-view and never touches the filesystem; only a cross-tree Set reparent commits `moveSet`, carrying the destination's *current* fs child order with the moved id appended (the order-leak guard). **No "None" band exists** — value-less pages render as the loose-row tail (`empty_placement` stays decode-only parity). **Reversal seams (deliberate):** property = write `order_mode` back (the full three-mode machinery survives in `bucketOrder`); structural = remove the single `orderGroups` call (the `group_order` key rides as foreign data). No migration in either direction. Mechanics → `Features/TableView.md` §Groups.

#### PropertiesV2 — definitions go nexus-wide (2026-07-01, branch `main`, React)

Property **definitions** moved from per-Collection sidecars to one nexus-wide registry at `.nexus/properties.json` (zod-validated, lenient read); a Collection's sidecar `properties` is a flat **assignment array** of prop-ids, joined ids→defs in `readNexus` so the renderer and view pipeline never changed. Values stay `prop_<ulid>`-keyed in frontmatter — zero page migration. **Ratified decisions:** one fully-shared definition + options per property (divergent needs = a separate property, never forked options); Create validates names registry-wide while **Assign is an unvalidated, idempotent reference**; Remove = unassign (values survive as foreign frontmatter, restored by re-assigning); global **Delete** is the one fan-out op — a timestamped recovery snapshot to `.trash`, then one atomic transaction strips every assigner; registry mutations serialize through a single chain (a review-caught lost-update race). The `schema:*` IPC surface kept its exact pre-V2 names, so `PropertiesPane` shipped unmodified; **Agenda keeps its own `property_definitions`** (deliberate — future rework); SQLite v16 is a pure registry mirror. Cross-Collection moves adopt-don't-strip by construction; cross-Collection queries are unlocked. Net **+163 code lines** for the whole paradigm. The assign-surface UI (assign-existing picker, Remove/Delete labels, Max Properties) is the deferred Plan 2.
