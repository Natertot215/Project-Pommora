...
#### Refactoring program — Phase C reorg + shared primitives (2026-06-21, branch `refactoring-phase-b`)

Folder reorg gave the shared layer real homes: new `Core/` absorbs the one-file utility folders (CRUD / Ordering / Filesystem) and `Core/Formatters/` (IndexDateFormat + TimeFormat + DateFormat); `FlowLayout` extracted to `Components/Layout/`; misplaced singletons rehomed (`SavedConfig` → Configuration, `ReservedTypeID` → Agenda). Consolidations: the Crockford ULID alphabet single-sourced (`Core/ULIDAlphabet`, consumed by `ULID` + `ULIDValidator`); `AppState` + `NexusIdentity` persistence routed through `AtomicJSON`; `FilterBuilder` split out of the `IndexQuery` god-file. All behaviour-neutral + build-verified (1291 tests). **Deferred for review** (visual or paradigm-adjacent): magic-numbers → `PUI` + `.hoverFill()` (silent pixel risk the build can't catch), `PropertyValue` datetime → `IndexDateFormat` (adds fractional seconds = on-disk decode change), the full Domain/Features top-level grouping (subjective), and the `NexusAdopter` / `PageTypeManager` god-file splits (tangled — view-coupled `Identifiable` models + shared mutable state).
...
- **Option-value minting** unifies on `opt_<ULID>` going forward (Status already does; Select aligns — existing data untouched).
...
- **`loadAll` heal-on-read kept as-is** — opening a nexus keeps silently minting/rewriting missing sidecars (self-heal favored over read-purity).
- **Area color removed entirely.** The `AreaColor` palette + `Area.color` field + picker are cut — Areas are identified by **icon only**. (The `blue` swatch rendered as the app accent, not blue; `.accent` duplicated it; the controlling `TierConfig.taggingStyle` was already vestigial — its parent-Area indicator died with containment — and is removed too.) Existing `color` / `tagging_style` keys drop on next write.
...
**Locked paradigm — stamp-on-first-sight.** A `.md` Page authored outside Pommora (no `id` frontmatter) is minted a real ULID written into its own frontmatter — additive, foreign keys preserved — so an external rename tracks by id instead of degrading to delete-plus-create. Pages are stamped as they appear/change through the watcher **and bulk-stamped at index build** (folded into `IndexBuilder`'s page walk, so an Obsidian import / rebuild stamps every Page, correctly ordered). Deferred — and each net-negative to rush: Context/Agenda **sidecar stamping** (those entities are app-created with ids; an id-less hand-authored sidecar is dropped on load before any stamp could run) and **open-adopted-Page stamping** (covered by the index-build stamp for any normal vault; stamping an upgrader's untouched page on open would desync the index → a duplicate row on the next save). Spec → `Planning/06-18-File-Watcher.md`.
