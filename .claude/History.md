...
**v0.0 — Shell opens (scaffolded + polished + sidebar-scaffolded).** Two-column `NavigationSplitView(sidebar:detail:)` + `.inspector(isPresented:)` at the split-view level. Sidebar (240 default, drag-resizable) + main pane (`EmptyPane` — `windowBackgroundColor` fill) + pop-out inspector (280 default, hidden by default, `Color.clear` content). Inspector toggle: `sidebar.trailing` SF Symbol at `.primaryAction` inside the `.inspector { … }` closure so it anchors to the inspector's toolbar segment; wrapped in `withAnimation(.smooth(duration: 0.30))`. Sidebar collapse: system `≡` toggle, `NSSplitView` native animation (Mail / Notes / Finder pattern). View menu's "Show Inspector" + default keyboard shortcut via `InspectorCommands()`. Window 1440×810 default, 960×560 min, `.contentMinSize` resizability. Window title suppressed via `.windowToolbarStyle(.unified(showsTitle: false))`. Deployment target macOS 26.5.

**v0.0 sidebar scaffolding (added in commit `4431420`).** `List(selection:) + .listStyle(.sidebar) + .environment(\.sidebarRowSize, .medium) + .scrollContentBackground(.hidden)`. Three sections with placeholder rows: top Items (3 rows, no header), Spaces (3 rows, collapsible via `Section(isExpanded: $spacesExpanded)`), Collections (3 rows, collapsible via `Section(isExpanded: $collectionsExpanded)` with each Collection as a `DisclosureGroup` containing 3 member rows). DisclosureGroup `isExpanded` Binding wraps its `set` closure in `withAnimation(.smooth(duration: 0.25))` so chevron rotation animates (custom Bindings don't auto-animate). Section headers use the custom-header form with `Text("Title").foregroundStyle(.secondary)` because string-shorthand was rendering darker than Mail's reference tone on the `.scrollContentBackground(.hidden)` backdrop. `NSSearchField` via `NSViewRepresentable`, anchored to `.safeAreaInset(.top, spacing: 8)`. No `.listRowInsets` overrides — native sidebar geometry handles row insets correctly when uncustomized. Selection uses macOS default (accent-blue fill + white foreground); the gray-fill spec from Sidebar.md is deferred (intent, not built). Build verified via `xcodebuild`. Files: [PommoraApp.swift](Pommora/Pommora/PommoraApp.swift), [ContentView.swift](Pommora/Pommora/ContentView.swift).
