...
- **"Layer-confusion check"** (`CLAUDE.md` quirk #18). A wrong/empty UI surface ≠ a broken data layer — confirm the data directly (query the index / read the file) before blaming the store. Earned the hard way (a popover collapse read as an empty index for hours).
- **Relation-lifecycle hardening (Task 8, `f1d66f6`, 2026-05-30).** Both diagnosed bugs fixed. (A) The member-file value-strip crashed on a hand-authored frontmatter-less `.md` (`PageFrontmatter` requires `id` → `keyNotFound(.id)` → "data couldn't be read" toast + aborted mutation): hoisted the duplicated strip-loop resilience into one shared `MemberFileStrip.forEach` (skip + log an unreadable member — lossless) and routed all **8** strip sites through it (`DualRelationCoordinator.stageValueStrip` ×4 branches + `PageType`/`ItemTypeManager` delete+changeType — broader than the 3 originally scoped). (B) `upsertPage`/`upsertItem` FK-19 on an unindexed parent → catch the constraint, retry without the orphan collection (keep under type), else skip + log. Tests: `MemberFileStripResilienceTests` (3) + flipped `IndexPopulationReproTests` Test B; full suite green.
