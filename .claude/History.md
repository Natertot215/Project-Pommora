...
- Phase 6 — Ship: `5ceca94` 6.2 Handoff + History ship entry, `f2d42fe` 6.3 swift-format auto-fixes for lint gate (line-length wraps + import dedupe + `fileprivate` hoist), `1d72020` 6.3 doc-sync to exact tag commit — tag `flatlayout` lands on `1d72020`. 6.1 grep sweep was a no-op (production code already clean; legitimate `_schema.json` references inside NexusAdopterTests fixtures preserved per spec).
