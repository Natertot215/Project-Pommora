...
An FSEvents recursive watch on the Nexus root propagates external + out-of-band on-disk changes into the running app live, **authority by recency and origin-blind** — the newest write wins, internal-vs-external is irrelevant. v1 reconcile is coarse: a debounced, atomic index rebuild (`IndexBuilder.populate`) + manager reload, covering every kind and every change type (edit / rename / move / create / delete); per-scope surgical reconcile is the deferred optimization. The index database is excluded at watcher intake so reconciles can't self-feed, and a watcher-owned last-seen-`mtime` gate drops duplicate events and self-write echoes. The open editor live-reloads on an external in-place edit (clean only) and re-points by stable id on an external rename — **unflushed edits are protected** and a file deleted under a clean editor is never resurrected.

**Locked paradigm — stamp-on-first-sight.** A `.md` Page authored outside Pommora (no `id` frontmatter) is minted a real ULID written into its own frontmatter — additive, foreign keys preserved — so an external rename tracks by id instead of degrading to delete-plus-create. v1 stamps Pages as they appear/change through the watcher; the launch bulk-stamp of a pre-existing vault (must run before the index build), Context/Agenda sidecar stamping, and open-adopted-Page stamping are deferred. Spec → `Planning/06-18-File-Watcher.md`.
