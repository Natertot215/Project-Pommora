...
The fitting-table-compresses / overflowing-table-keeps-width conditional (Nathan's 07-01 visual idea, shipped behind a one-rule seam) never survived contact: three implementations deep (a scrollWidth one-way latch, the known-column-sum hypothetical, a two-signal box/pane split) it was CDP-verified correct each time and still read as broken in real use. Torn down entirely on Nathan's order — the `:has` rule, the pane signal, and every shell/inspector read in the table's overflow check are gone. The surviving rule is one line: `overflowing` = the column sum exceeds the table's current box (flatten + h-scroll); the inspector compresses the pane unconditionally and a tighter table scrolls within it. Durable lesson: `scrollWidth` floors at `clientWidth`, so any is-content-bigger comparison built on it can latch.
