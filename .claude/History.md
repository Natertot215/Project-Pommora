...
- **Three-pane shell**: sidebar (default 240px) / main (flex) / pop-out inspector (default 280px). Built on SwiftUI's two-column `NavigationSplitView(sidebar:detail:)` with the inspector pane attached via the `.inspector(isPresented:)` modifier (macOS 14+) at the split-view level. Apple's idiomatic pattern for main-pane + supplementary side panel (Mail / Notes / Pages); chosen over the three-column `NavigationSplitView` variant because the detail column was designed for selected-item drill-down, not for a contextual supplementary panel. Sidebar width via `.navigationSplitViewColumnWidth(min:ideal:max:)`; inspector width via `.inspectorColumnWidth(min:ideal:max:)`. `InspectorCommands()` registered in the App's `.commands` provides the View-menu toggle + default keyboard shortcut; a toolbar icon (`sidebar.trailing` SF Symbol at `.primaryAction`, **placed inside the `.inspector { content }` closure so it anchors to the inspector's segment of the unified toolbar**) gives the visible toggle affordance, wrapped in `withAnimation(.smooth(duration: 0.30))` to sync content + column geometry. **Sidebar collapse uses the system `≡` toggle and `NSSplitView`'s native animation** — no custom toggle (the system path matches Mail / Notes / Finder). **Inspector hidden by default in v0.0** (the property panel content lands in v0.6; the default-shown decision is revisited then). Sidebar drag-resizable from v0.0; inspector drag-resizable when shown; widths persist across launches.
...
**v0.0 — Shell opens (scaffolded + polished).** Barebones three-pane shell: two-column `NavigationSplitView(sidebar:detail:)` + `.inspector(isPresented:)` at the split-view level. Sidebar (240 default, drag-resizable, empty `List` with `.listStyle(.sidebar)`) + main pane (`EmptyPane` — `windowBackgroundColor` fill) + pop-out inspector (280 default, hidden by default, same `EmptyPane`). Inspector toggle: `sidebar.trailing` SF Symbol at `.primaryAction`, placed inside the `.inspector { … }` closure so it anchors to the inspector's toolbar segment; wrapped in `withAnimation(.smooth(duration: 0.30))`. Sidebar collapse: system `≡` toggle, `NSSplitView` native animation (Mail / Notes / Finder pattern; not replaced with custom). View menu's "Show Inspector" + default keyboard shortcut via `InspectorCommands()`. Window 1440×810 default, 960×560 min, `.contentMinSize` resizability. Window title suppressed via `.windowToolbarStyle(.unified(showsTitle: false))` (traffic lights render; no "Pommora" title text). Deployment target macOS 26.5. Build verified via `xcodebuild`. Files: [PommoraApp.swift](Pommora/Pommora/PommoraApp.swift), [ContentView.swift](Pommora/Pommora/ContentView.swift).
