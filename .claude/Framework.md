...
- **Vault-table display-only + creation-order.** Page/Item *Type* detail tables are display-only for row order (mirror the sidebar live); Collection/Set tables keep flat reorder; empty-state default order changed alphabetical → **creation order** (ULID-id ascending). The full per-view ordering system ships v0.4.0.
