...
- 🟡 **v0.2.7.2 — Page editor fixes (PARTIAL SHIP 2026-05-20)** — **HR / divider SHIPPED** via Obsidian-style dynamic syntax (full architecture documented in `Features/PageEditor.md → Dynamic-syntax pattern`). Established the locked architecture for paragraph-level dynamic-syntax constructs going forward: AST-backed detection in renderer + caret-awareness service as sole writer of construct-specific attributes + styler emits nothing for the construct. Three changes: `MarkdownTextLayoutFragment.swift` (renderer with `hasThematicBreak` + `caretIsInFragment` + `drawThematicBreak` wired into `draw(at:in:)`), `AppleASTSupplementalStyler.swift` (`visitThematicBreak` does nothing), `NativeTextViewCoordinator+HRVisibility.swift` (new — caret-awareness service). Bonus: legacy `MarkdownListHandler` HR expansion removed. **Blockquote DEFERRED** (will reuse dynamic-syntax pattern next session). **Tables DEFERRED** (~10-15h realistic estimate given divider's actual effort vs planned; "ASAP but not immediate"). Auto-transform on 3rd dash dropped from scope; cursor-atom workaround unnecessary (dynamic syntax obviates it). Right-click "Insert HR" out of scope.
- **Lists + Blockquotes via dynamic-syntax pattern** (next — likely v0.2.7.3) — Lists: Enter on bare `-` / `*` / `1.` lines commits as list item (currently only space triggers); Shift+Space inserts new list item below at same nesting. Blockquotes: Apple-Calendar-event-card chrome (grey rounded card + 3pt vertical bar inside) using the new dynamic-syntax architecture.
