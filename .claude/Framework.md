...
- **Relations unified.** One linking system replaces two — `tier1/2/3` and relation properties share a single pipeline; the `tier_links` table retired (one reverse-lookup path, `IndexQuery.incomingRelations`). Relations are always-multi (`[{"$rel": "<ULID>"}]`); a single-pane editor sets home side + reverse name + reverse icon, propagating to the target Type; values render as the target's live icon + title via `RelationChip`. Context-delete cascades the tier reference out of every Page / Item / Agenda entry. Entity `icon` denormalized into the SQLite index so relation + tier columns render icon + title everywhere (killed "(missing)"). Index resilience hardened (per-row insert, version-stamp-after-populate) + `MemberFileStrip.forEach` tolerates frontmatter-less member files.
...
##### v0.4.0 — Symbols + Settings + Trash + Wikilinks
...
##### v0.6.0 — Quick Capture + LLM Interface + global search
...
- **(folded) Context Linked-from surface.** The real `LinkedFromDropdown` (stub today at `Detail/LinkedFromDropdown.swift`): a dropdown listing every operational entity whose `tier1/2/3` or relations point at the Context, via `IndexQuery.incomingRelations(targetID:)` (shipped). Supporting bits: `EntityStateRef.iconName`, `EntityKind.displayLabel`, an `EntityStateRef → SidebarSelection` navigation resolver, and a host slot.
