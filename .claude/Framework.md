...
##### PagesV2 — the one-entity collapse (2026-06-09/10, through `c7f48c7`)
The second operational side deleted, not migrated — Page is the only operational entity beside Agenda. `[[` is the sole connection syntax; per-vault `open_in` (`compact` | `window`) routes page-taps to the new in-window **PagePreview** card (`PreviewStack` overlay) or the main detail pane; user-creatable sidebar sections group Vaults (navigation-only); index schema v10 → v11 (legacy tables dropped, delete-and-rebuild). Full record + retrospective pointers → [[History]] § "PagesV2".
...
- **Vault-table display-only + creation-order.** *Type* detail tables are display-only for row order (mirror the sidebar live); Collection tables keep flat reorder; empty-state default order changed alphabetical → **creation order** (ULID-id ascending). The full per-view ordering system ships v0.4.0.
...
- **(folded) Global search.** `⌘K` command palette + FTS5 search over Page bodies, Agenda titles, and frontmatter / properties (over the FTS5 tables wired at v0.4.0). Natural pairing with Quick Capture — both are global invocation surfaces.
