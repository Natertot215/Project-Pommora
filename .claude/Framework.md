...
> **Version buckets + priority follow Nathan's own [[Pommora Tasks]] doc** (the working intent ledger); Framework keeps the implementation detail under each. Entries Framework tracks that the Tasks doc doesn't name are *folded into their nearest bucket* and marked **(infra)** / **(folded)**. The Sort / Filter / Group panes muted in v0.3.1 + the multi-saved-view switcher land with the view system at **v0.5.0**.

##### v0.5.0 — Views + Symbols + Trash
- **Vault + Collection views.** The custom Table rebuild + the Gallery renderer over the per-container `SavedView` storage (Board / List / Cards stay enum-carried in the data model; their UI follows post-v0.5.0 — Board = kanban, cards grouped by a property's options); a toolbar **Views dropdown** switcher (saved-view list with inline per-view type switching); full per-view config — **order, sort, Group By, column selection** — plus **tier-link sort + filter** (`linked to` / `not linked to` relation operators); page **covers** + container **banners**. Also ships the deferred per-view **reorder engine** on the macOS 26 drag-session APIs — until now, Type detail tables are display-only and mirror the sidebar. Ratified spec → `Planning//06-11-Views-Spec.md`.
