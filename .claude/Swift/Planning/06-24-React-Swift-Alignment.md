...
- **`settings.json` keys (snake):** `{ version, defaults_version, accent_color?, labels, show_page_icon, excluded_folders, profile_image?, profile_subtitle, modified_at }`. React reads only what it needs but **any future settings WRITE (T13) must read-merge-write** — preserve `version`/`defaults_version`/`show_page_icon`/`modified_at` + the full `labels` block, or Swift's `defaults_version` auto-migration misfires (a churn/conflict source). Settings is foreign-by-value, like page frontmatter.
- **Accent — keep React's palette + tokens (Nathan-ratified).** React keeps its own 10-color spectrum, `resolveAccent`, `DEFAULT_ACCENT='lavender'`, and `color.css.ts` token DRY mechanism — **do NOT narrow to Swift's set or change the default.** The only alignment is the read: today React reads the wrong key (`settings.accent`, [readNexus.ts:239](React/src/main/readNexus.ts#L239)); repoint it to the shared `accent_color` and **map Swift's 8** (`red/orange/yellow/green/blue/purple/pink/gray`, absent = system) onto React's existing tokens (6 direct; `gray`→`grey`; `pink`→nearest/default) so a Swift-chosen accent renders in React. The reverse is graceful, not lossy: a React-only accent (`lavender`/`lightBlue`/`cyan`) Swift can't decode degrades to system display — accent is cosmetic per-nexus, the on-disk value isn't destroyed unless the user re-picks in Swift. No palette narrowing; the mapping lives in one place (DRY).
...
- **saved-config (camelCase):** `{ schemaVersion, items:[{key,label}] }` — React **today reads `savedConfig.labels` (a map)**, [readNexus.ts:250](React/src/main/readNexus.ts#L250); repoint to `items` keyed by `key`.
...
### Cleanup-stint detour (post-Phase-3, this session)

- **Agenda is config-driven, never name-reserved** (React `984ede2`/`285281a`, Swift+root docs `e050940`): Tasks/Events identified ONLY by `_taskconfig`/`_eventconfig` (no `Agenda/` wrapper, no `AGENDA_FOLDER_NAMES`); a user could even name a Collection "Tasks". The adopter also no longer fabricates a Collection from an empty sidecar-less folder. Per-file kind discriminator (`.task.json`/`.event.json`) is DEFERRED + documented in both `Architecture.md` + a code comment in both adopters.
- **Config-on-open runs on launch-restore too** (React `079ab3b`): `ensureIdentity`+`ensureSettings`+`stampAdopted` were only on explicit ⌘O; extracted to `prepareOpenedNexus`, now also on the reopen-last-nexus path.
- **`modified_at` is no longer a hard requirement (Swift, committed `main` `6317035` — build + tests green, `** TEST SUCCEEDED **`):** the Swift cross-open of a React nexus was failing because `PageCollection.init(from:)` (and 7 peer models) hard-required `modified_at` (`try c.decode(Date.self)`), so a React-created/converted sidecar lacking it threw `keyNotFound` → the whole collection dropped → empty sidebar + "data couldn't be read because it is missing." Fix: `modified_at` decodes optional with a file-mtime fallback injected via `AtomicJSON`'s new `CodingUserInfoKey.fileModificationDate`, across all 8 models (PageCollection/PageSet/Area/Topic/Project/AgendaTask/AgendaEvent/Settings); still WRITTEN (encode untouched), just not required. `ModifiedAtFallbackTests` (3/3) + History entry. **This is the unblock for the Swift reverse-open.**
- **Doc-alignment audit (both builds):** 4 read-only agents vs current code. React `Features/Architecture.md` rewritten off the 3-tier (Vault/PageType/vaults[]/roll-up/depth-cap) to 2-tier; Swift `PommoraPRD.md` (Page Types/per-vault → Collections), `Features/Architecture.md` (settings.json profile), `Features/Contexts.md` (tier labels: sidebar from `settings.labels`, tier-config feeds property names). All other docs verified accurate (audit-1 findings were false positives).

**Morning handoff — for Nathan (interactive, needs the Swift app):**
1. **Swift reverse-open (the one open verification).** `modified_at` (the blocker) is fixed + committed; the unblock is unit-proven (`ModifiedAtFallbackTests`: a sidecar with no `modified_at` now decodes). Still owed: a real GUI confirm — make a **clean** React-created nexus (⌘O an empty folder in React so `ensureIdentity`/`ensureSettings`/`stampAdopted` run, add a collection or two), then open that same folder in **Swift** and confirm Collections populate with NO reseed/migration churn. Do NOT use the frankenstein `~/test-2tier` (its Set sidecars carry the dead `type_id` + `schema_version: 0` — it'll mislead).
2. **Biome normalization (deferred, decision needed).** The Biome *setup* is committed (`main` `3a70ed2`); the one-time tree-wide `npm run check` (`biome check --write .`) was intentionally NOT run — on main's pre-alignment React it would conflict with the `pommora-react` worktree's current code on merge. Run it on the **worktree** once the biome config lands there (or after pommora-react merges to main), not on main now.

**Profile-header UIX pass: DONE** — Nathan verified live.
