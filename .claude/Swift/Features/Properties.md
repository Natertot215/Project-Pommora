...
**Not property types:**
- **Connections** — body-text inline `[[ ]]` links, indexed in SQLite with no frontmatter mirror. Not properties, not schema-creatable. Spec → [[Resources/II. Pommora/II. Swift/II. Features/Connections]].
...
Cross-property references in the schema use IDs: `default_sort.property_id`, `views[i].group.property_id`, `views[i].filter.rules[i].property_id` (→ [[Resources/II. Pommora/II. Swift/II. Features/Views]]). `_title` is reserved as the title column id in `property_order` (movable, never hideable).
...
**The `cover` field is not a property.** A page's cover is a root `cover` frontmatter field (nexus-relative image path), never surfaced in any properties UI — not Edit Properties, not the Layout visibility list, not the inspector. It's a per-view Gallery display concern only (→ [[Resources/II. Pommora/II. Swift/II. Features/Views]] § "Covers + Banners").
...
Canonical rule → [[Domain-Model]] § "Entity identity vs title". Connection disk format and rename cascade → [[Resources/II. Pommora/II. Swift/II. Features/Connections]].
...
Updates on any content or frontmatter edit — body, property value, title rename, icon, tier1/2/3. View-only actions never update it. External edits update file mtime but do NOT update frontmatter `modified_at` — so external edits don't move Last Edited; the stored stamp is Pommora-managed and survives sync, where mtime wouldn't. A Page that has never carried `modified_at` (legacy or externally-authored) derives Last Edited from file mtime at load, until Pommora next writes the page and persists the stamp.
...
The popover is active-view-scoped (resolved via `ActiveViewStore`). Full pane spec → [[Resources/II. Pommora/II. Swift/II. Features/Views]] § "View Settings Panes".
...
- `ChipLink` — **intentionally dormant design asset**: the chip-link visual, wired to nothing in production (showcased in the Component Library explorer only). Context → [[Resources/II. Pommora/II. Swift/II. Features/Connections]] § "Scope".
...
View definitions persist in the per-kind sidecar as `views[]` (SavedView v2). Multi-view CRUD + a toolbar Views dropdown switcher ship; Table and Gallery render, Board / List / Cards are muted. Full spec → [[Resources/II. Pommora/II. Swift/II. Features/Views]].
...
The three tier relations (Areas / Topics / Projects) surface in a Table view as pre-configured relation columns at the RIGHTMOST content positions — after every user-property column and immediately before the trailing Last Edited Time column. Order is Project, then Topic, then Area (`_tier3`, `_tier2`, `_tier1`). They render through `ContextChip` like any relation column and are reorderable + hideable like any column (via the Layout pane's visibility list → [[Resources/II. Pommora/II. Swift/II. Features/Views]]). A schema without tiers (e.g. a Type that doesn't carry them) gets no tier columns.

---

#### Out-of-scope boundaries

The full property data layer (all 10 types, ID-truth identity, schema CRUD on all four carriers, move-strip, file-attachment copy-on-attach, reserved-ID enforcement, the SQLite indexer) is in scope; the value editors, Settings sheet, and pickers all have a working UI path. Phasing of remaining UI polish lives in [[Resources/II. Pommora/Framework]]; per-feature deferrals live in [[Prospects]]. Design constraints that don't fit elsewhere in this doc:
...
- [[Resources/II. Pommora/II. Swift/II. Features/Agenda]] — Task + Event split; per-side schemas
- [[Resources/II. Pommora/II. Swift/II. Features/Contexts]] — Areas / Topics / Projects tier system
- [[Resources/II. Pommora/II. Swift/II. Features/Pages]] — on-disk shape, connection mechanics
- [[Prospects]] — post-v1 deferrals
- [[Resources/II. Pommora/Framework]] — version roadmap
