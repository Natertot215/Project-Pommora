...
- **Create is file-canonical.** A Page writes its `.md`; a Task / Event writes its `.task.json` / `.event.json`. The write is atomic and the index upsert is best-effort, so capture succeeds at the file level even if the index lags. (→ [[Resources/II. Pommora/II. Swift/II. Features/Architecture]].)
- **The property UI is host-agnostic.** The property panel takes a schema + value bindings, not the manager graph, so the same surface renders in a compact pane. (→ [[Resources/II. Pommora/II. Swift/II. Features/Properties]].)
