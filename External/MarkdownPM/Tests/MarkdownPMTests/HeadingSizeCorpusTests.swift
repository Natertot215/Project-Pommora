...
///
/// `styleHeadings` reads the multiplier for a level from
/// `HeadingStyle.fontMultiplier(for:)` (default `[2.0, 1.75, 1.5, 1.25,
/// 1.15, 1.0]`, H1...H6 — Phase-5 scale, see
/// MarkdownEditorConfiguration.swift) and emits a bold `.font` over the
/// heading CONTENT range sized `baseFont.pointSize * multiplier`.
///
/// These assertions lock the Phase-5 pointSizes (base fontSize 16): the new
/// scale anchors H6 = body size with no heading below body (D-HEAD-2 LANDED).
@MainActor
...
struct HeadingSizeCorpusTests {
    ...
    // Observed pointSizes at base fontSize 16 — the Phase-5 scale. Each equals
    // 16 × the default multiplier, i.e. the array [2.0,1.75,1.5,1.25,1.15,1.0].
    @Test("H1 content font is 32.0pt (16 × 2.0)")
    func h1() { #expect(headingContentPointSize(level: 1) == 32.0) }

    @Test("H2 content font is 28.0pt (16 × 1.75)")
    func h2() { #expect(headingContentPointSize(level: 2) == 28.0) }

    @Test("H3 content font is 24.0pt (16 × 1.5)")
    func h3() { #expect(headingContentPointSize(level: 3) == 24.0) }

    @Test("H4 content font is 20.0pt (16 × 1.25)")
    func h4() { #expect(headingContentPointSize(level: 4) == 20.0) }

    @Test("H5 content font is 18.4pt (16 × 1.15)")
    func h5() { #expect(headingContentPointSize(level: 5) == 18.4) }

    @Test("H6 content font is 16.0pt (16 × 1.0)")
    func h6() { #expect(headingContentPointSize(level: 6) == 16.0) }

    /// Whole-scale guard: the six pointSizes, divided by the base size, ARE the
    /// shipped multiplier array. Locks the scale as a set so a partial edit
    /// can't pass the per-level cases while drifting the array.
    @Test("Derived multiplier array is the shipped [2.0,1.75,1.5,1.25,1.15,1.0]")
    func derivedMultiplierArray() {
        ...
        let expected: [CGFloat] = [2.0, 1.75, 1.5, 1.25, 1.15, 1.0]
